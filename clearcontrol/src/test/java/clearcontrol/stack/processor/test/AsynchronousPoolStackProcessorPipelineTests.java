...
public class AsynchronousPoolStackProcessorPipelineTests
{
  ...
  public void test() throws InterruptedException
  {
    ...
    final AsynchronousPoolStackProcessorPipeline lAsynchronousPoolStackProcessorPipeline = new AsynchronousPoolStackProcessorPipeline("Test", lStackRecyclerManager, 10, 4);

    final ContiguousOffHeapPlanarStackFactory lOffHeapPlanarStackFactory = new ContiguousOffHeapPlanarStackFactory();

    final RecyclerInterface<StackInterface, StackRequest> lRecycler0 = new BasicRecycler<StackInterface, StackRequest>(lOffHeapPlanarStackFactory, 10);
    ...
    final StackProcessorInterface lStackProcessor1 = new StackProcessorInterface()
    {
      ...
      public StackInterface process(StackInterface pStack, RecyclerInterface<StackInterface, StackRequest> pStackRecycler)
      {
        ...
        final StackInterface lNewStack = pStackRecycler.getOrWait(1, TimeUnit.SECONDS, StackRequest.buildFrom(pStack));

        lNewStack.getContiguousMemory().setByteAligned(0, (byte) (pStack.getContiguousMemory().getByteAligned(0) + 1));
        pStack.release();
        return lNewStack;
      }
    };
    ...
    lAsynchronousPoolStackProcessorPipeline.addStackProcessor(lStackProcessor1, "recycler", 10, 10);

    lAsynchronousPoolStackProcessorPipeline.addStackProcessor(lStackProcessor2, "recycler", 10, 10);

    assertTrue(lAsynchronousPoolStackProcessorPipeline.open());

    lAsynchronousPoolStackProcessorPipeline.getOutputVariable().addListener(new VariableListener<StackInterface>()
    {

      @Override
      public void setEvent(StackInterface pCurrentValue, StackInterface pNewValue)
      {
        assertEquals(2, pNewValue.getContiguousMemory().getByteAligned(0), 0);
    ...
    {
      final StackInterface lStack = lRecycler0.getOrWait(100, TimeUnit.SECONDS, StackRequest.build(12, 13, 14));
      // System.out.println(lStack);
      lStack.getContiguousMemory().setByteAligned(0, (byte) 0);

      lAsynchronousPoolStackProcessorPipeline.getInputVariable().set(lStack);
