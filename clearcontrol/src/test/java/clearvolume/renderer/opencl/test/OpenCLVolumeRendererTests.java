...
public class OpenCLVolumeRendererTests
{
  ...
  {
    if (!OpenCLAvailability.isOpenCLAvailable()) return;

    final NativeTypeEnum lNativeType = NativeTypeEnum.UnsignedByte;
    final long lBytesPerVoxel = Size.of(lNativeType);

    final ClearVolumeRendererInterface lClearVolumeRenderer = new OpenCLVolumeRenderer("ClearVolumeTest", 512, 512, lNativeType, 512, 512, 1, false);
    ...
    final ByteBuffer lByteBuffer = ByteBuffer.allocateDirect(toIntExact(lLength * lBytesPerVoxel)).order(ByteOrder.nativeOrder());
    ...
    lClearVolumeRenderer.setVolumeDataBuffer(0, lByteBuffer, lResolutionX, lResolutionY, lResolutionZ);
    ...
    for (int z = 0; z < lResolutionZ; z++)
      for (int y = 0; y < lResolutionY; y++)
        for (int x = 0; x < lResolutionX; x++)
        {
          ...
          lOffHeapMemory.setByte(lIndex, (byte) (256 - (((byte) x ^ (byte) y ^ (byte) z))));
        }

    lClearVolumeRenderer.setVolumeDataBuffer(0, lOffHeapMemory, lResolutionX, lResolutionY, lResolutionZ);
  ...
  public void demoWith16BitGeneratedDataset() throws InterruptedException, IOException
  {
    ...
    for (int z = 0; z < lResolutionZ; z++)
      for (int y = 0; y < lResolutionY; y++)
        ...
        {
          final int lIndex = (x + lResolutionX * y + lResolutionX * lResolutionY * z);
          lOffHeapMemory.setShort(lIndex, (short) (256 - (((byte) x ^ (byte) y ^ (byte) z))));
        }
