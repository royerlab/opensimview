...
public class ClearCLBasicDemo
{
  ...
  private void demoImages(ClearCLContext lContext, ClearCLProgram pProgram)
  {

    ClearCLImage lImageSrc = lContext.createSingleChannelImage(HostAccessType.WriteOnly, KernelAccessType.ReadWrite, ImageChannelDataType.Float, 100, 100, 100);
    ...
    ClearCLImage lImageDst = lContext.createSingleChannelImage(HostAccessType.ReadOnly, KernelAccessType.WriteOnly, ImageChannelDataType.Float, 10, 10, 10);
  ...
  private void demoBuffers(ClearCLContext lCreateContext, ClearCLProgram pProgram) throws IOException
  {
    ...
    ClearCLBuffer lBufferA = lCreateContext.createBuffer(HostAccessType.WriteOnly, KernelAccessType.ReadOnly, NativeTypeEnum.Float, cFloatArrayLength);

    ClearCLBuffer lBufferB = lCreateContext.createBuffer(HostAccessType.WriteOnly, KernelAccessType.ReadOnly, NativeTypeEnum.Float, cFloatArrayLength);

    ClearCLBuffer lBufferC = lCreateContext.createBuffer(HostAccessType.ReadOnly, KernelAccessType.WriteOnly, NativeTypeEnum.Float, cFloatArrayLength);
    ...
    FloatBuffer lArrayC = ByteBuffer.allocateDirect(4 * cFloatArrayLength).order(ByteOrder.nativeOrder()).asFloatBuffer();
    ...
    for (int j = 0; j < cFloatArrayLength; j++)
    {
      ...
      {
        System.out.format("NOT EQUAL: (c[%d] = %g) != %g \n", j, lObservedValue, lTrueValue);
        assertTrue(false);
        break;
      }
      if (j % 100000 == 0) System.out.println(lObservedValue + " == " + lTrueValue);
    }
  }
