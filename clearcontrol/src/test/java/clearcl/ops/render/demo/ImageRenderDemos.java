...
public class ImageRenderDemos
{
  ...
  public void demoVolRenderMaxProj() throws InterruptedException
  {
    ...
    try (ClearCL lClearCL = new ClearCL(lClearCLBackend))
    {
      ...
      ClearCLImage l3DImage = lContext.createSingleChannelImage(HostAccessType.WriteOnly, KernelAccessType.ReadWrite, ImageChannelDataType.UnsignedInt8, lWidth, lHeight, lDepth);

      final byte[] lVolumeDataArray = new byte[lWidth * lHeight * lDepth];
      ...
      ClearCLBuffer lRGBABuffer = lContext.createBuffer(MemAllocMode.AllocateHostPointer, HostAccessType.ReadOnly, KernelAccessType.WriteOnly, 4, NativeTypeEnum.Byte, Region2.region(l3DImage.getDimensions()));

      ImageRender lImageRender = new ImageRender(lContext.getDefaultQueue(), Algorithm.MaximumProjection);
