...
public class ClearCLImageTests
{
  ...
  private void testReadFromFragmentedMemory(final ClearCLContext lContext)
  {
    ...
    final ClearCLImage lImage = lContext.createImage(HostAccessType.ReadWrite, KernelAccessType.ReadWrite, ImageChannelOrder.Intensity, ImageChannelDataType.Float, 10, 10, 10);

    OffHeapMemory lMemory = OffHeapMemory.allocateFloats(10 * 10 * 10);

    for (int i = 0; i < 10 * 10 * 10; i++)
      lMemory.setFloatAligned(i, i);

    FragmentedMemory lFragmentedMemory = FragmentedMemory.split(lMemory, 10);

    lImage.readFrom(lFragmentedMemory, Region3.originZero(), Region3.region(10, 10, 10), true);

    OffHeapMemory lMemory2 = OffHeapMemory.allocateFloats(10 * 10 * 10);

    lImage.writeTo(lMemory2, true);

    for (int i = 0; i < 10 * 10 * 10; i++)
      assertEquals((double) i, (double) lMemory2.getFloatAligned(i), 0.01);
