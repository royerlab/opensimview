...
public class VectorCellProperty extends CellProperty
{
   ...
   */
  public VectorCellProperty(TissueDynamics pTissueDynamics, int pDimension)
  {
   ...
   *
   * @param pDestId   destination particle vector id
   * @param pSourceId source particle vector id
   * @param pFactor   factor to multiply vector with
   */
  public void addVector(int pDestId, int pSourceId, float pFactor)
  {
    ...
    for (int i = 0; i < lDimension; i++)
      lMorphogenArrayWrite[pDestId * lDimension + i] = pFactor * lMorphogenArrayRead[pSourceId * lDimension + i];
