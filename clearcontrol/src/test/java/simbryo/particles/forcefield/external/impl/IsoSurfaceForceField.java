 ...
 * @author royer
 */
public class IsoSurfaceForceField extends ExternalForceFieldBase implements ExternalForceFieldInterface
{
   ...
   */
  public IsoSurfaceForceField(float pForceIntensity, IsoSurfaceInterface pIsoSurfaceInterface)
  {
  ...
  public void applyForceField(int pBeginId, int pEndId, float[] pForceFactor, ParticleSystem pParticleSystem)
  {
    ...
    for (int i = lIndexStart, id = pBeginId; id < pEndId; i += lDimension, id++)
    {
      ...
      final float lForceIntensityFinal = lForceSign * lForceIntensity * (pForceFactor != null ? pForceFactor[id] : 1);

      for (int d = 0; d < lDimension; d++)
      {
        float dx = mIsoSurfaceInterface.getNormalizedGardient(d);
        lVelocitiesWrite[i + d] = lVelocitiesRead[i + d] + dx * lForceIntensityFinal;
      }
