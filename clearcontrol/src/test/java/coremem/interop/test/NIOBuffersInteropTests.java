...
public class NIOBuffersInteropTests
{
  ...
  {
    final ByteBuffer lBuffer = ByteBuffer.allocateDirect(cBufferLength).order(ByteOrder.nativeOrder());
  ...
  {
    final DoubleBuffer lBuffer = ByteBuffer.allocateDirect(cBufferLength).order(ByteOrder.nativeOrder()).asDoubleBuffer();
    final ContiguousMemoryInterface lContiguousMemory = NIOBuffersInterop.getContiguousMemoryFrom(lBuffer);
    ...
    assertEquals(12345.54321, lContiguousMemory.getDoubleAligned(3), 0.001);
  }
