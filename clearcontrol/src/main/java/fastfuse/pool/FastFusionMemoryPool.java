...
public class FastFusionMemoryPool implements AutoCloseable, SizedInBytes
{
  ...
  private final Map<Pair<ImageChannelDataType, List<Long>>, Stack<ClearCLImage>> mImagesAvailable = new HashMap<>();
  ...
  private final LinkedHashSet<Pair<ImageChannelDataType, List<Long>>> mImageAccess = new LinkedHashSet<>();
  ...
  {
    return getInstance(pContext, pContext.getDevice().getGlobalMemorySizeInBytes(), false);
  }
   ...
   * @return pool
   */
  public static FastFusionMemoryPool getInstance(ClearCLContext pContext, long pPreferredPoolSizeInBytes)
  {
    if (mInstance == null) mInstance = new FastFusionMemoryPool(pContext, pPreferredPoolSizeInBytes, false);
    return mInstance;
  }
   ...
   * @return pool
   */
  public static FastFusionMemoryPool getInstance(ClearCLContext pContext, long pPreferredPoolSizeInBytes, boolean pDebug)
  {
    if (mInstance == null) mInstance = new FastFusionMemoryPool(pContext, pPreferredPoolSizeInBytes, pDebug);
    return mInstance;
  }
   ...
   * @param pDebug                  debug flag
   */
  private FastFusionMemoryPool(ClearCLContext pContext, long pMemorySizeLimitInBytes, boolean pDebug)
  {
    mContext = pContext;
  ...
  {
    ClearCLImage lImage = freeMemoryIfNecessaryAndRun(() -> mContext.createSingleChannelImage(HostAccessType.ReadWrite, KernelAccessType.ReadWrite, pDataType, pDimensions), String.format("Couldn't allocate image of type '%s' with dimensions %s", pDataType.toString(), Arrays.toString(pDimensions)));
  ...
  private void closeImage(ClearCLImage pImage)
  {
    ...
    pImage.close();
    debug("                  free:      %32s = %3.0f MB  ->  %s\n", getKey(pImage).toString(), pImage.getSizeInBytes() / (1024d * 1024d), toString());
  }
  ...
  private void freeMemIfNecessaryAndPossible(long pAdditionalMemoryInBytes)
  {
    if (notEnoughMemoryAvailable(pAdditionalMemoryInBytes) && freeMemIsPossible())
    {
      ...
      for (Pair<ImageChannelDataType, List<Long>> lKeyAccess : mImageAccess)
      {
        ...
        while (!lImageStack.isEmpty() && notEnoughMemoryAvailable(pAdditionalMemoryInBytes))
        {
   ...
   * @return image
   */
  public ClearCLImage requestImage(final String pName, final ImageChannelDataType pDataType, final long... pDimensions)
  {
    ...
    debug("%15s - %s  %32s = %3.0f MB  ->  %s\n", prettyName(pName, 15), allocated ? "allocate:" : "reuse:   ", lKey.toString(), getSizeInBytes(pDataType, pDimensions) / (1024d * 1024d), toString());
    return lImage;
  }
  ...
  public void releaseImage(String pName, ClearCLImage pImage)
  {
    ...
    debug("%15s - release:   %32s = %3.0f MB  ->  %s\n", prettyName(pName, 15), lKey.toString(), pImage.getSizeInBytes() / (1024d * 1024d), toString());
  ...
  {
    boolean foundIt = pThrowable instanceof OpenCLException && ((OpenCLException) pThrowable).getErrorCode() == -4;
    if (foundIt) return true;
    else return (pThrowable.getCause() == null) ? false : isMemoryAllocationFailure(pThrowable.getCause());
  }
  ...
  public void free(boolean pFreeImagesInUse)
  {
    ...
    for (Stack<ClearCLImage> lStack : mImagesAvailable.values())
      while (!lStack.isEmpty()) closeImage(lStack.pop());
  ...
  public long getCurrentAvailableMemorySizeInBytes()
  {
    ...
    for (Map.Entry<Pair<ImageChannelDataType, List<Long>>, Stack<ClearCLImage>> lEntry : mImagesAvailable.entrySet())
    {
   ...
   * @return
   */
  private Pair<ImageChannelDataType, List<Long>> getKey(final ImageChannelDataType pDataType, final long... pDimensions)
  {
  ...
  {
    if (pName == null) return "<unnamed>";
    if (pName.length() <= pLength) return pName;
  ...
  {
    if (mDebug) cDebugOut.printf(format, args);
  }
  ...
  {
    return String.format("MemoryPool(used = %2d, avail = %2d, memory = %4.0f | %.0f MB)", getNumberOfInUseImages(), getNumberOfAvailableImages(), getCurrentUsedMemorySizeInBytes() / (1024d * 1024d), getMemorySizeLimitInBytes() / (1024d * 1024d));
  }
}
