...
public class FastFusionEngine implements FastFusionEngineInterface
{
  ...
  private final ConcurrentHashMap<String, MutablePair<Boolean, ClearCLImage>> mImageSlotsMap = new ConcurrentHashMap<>();

  private final ArrayList<TaskInterface> mFusionTasks = new ArrayList<>();

  private final HashSet<TaskInterface> mExecutedFusionTasks = new HashSet<>();
   ...
   * copied over
   *
   * @param pFastFusionEngine fast fusion engine
   */
  ...
  public void reset(boolean pCloseImages)
  {
    ...
    if (pCloseImages) lMemoryPool.free();
  }
  ...
  @Override
  public void passImage(String pSlotKey, ContiguousMemoryInterface pImageData, final ImageChannelDataType pImageChannelDataType, long... pDimensions)
  {
    MutablePair<Boolean, ClearCLImage> lPair = ensureImageAllocated(pSlotKey, pImageChannelDataType, pDimensions);

    FastFusionMemoryPool.get().freeMemoryIfNecessaryAndRun(() -> lPair.getRight().readFrom(pImageData, true));
    lPair.setLeft(true);
  }
  ...
  {
    MutablePair<Boolean, ClearCLImage> lPair = ensureImageAllocated(pSlotKey, pImage.getChannelDataType(), pImage.getDimensions());

    FastFusionMemoryPool.get().freeMemoryIfNecessaryAndRun(() -> pImage.copyTo(lPair.getRight(), true));
    lPair.setLeft(true);
  }

  @Override
  public MutablePair<Boolean, ClearCLImage> ensureImageAllocated(final String pSlotKey, final ImageChannelDataType pImageChannelDataType, final long... pDimensions)
  {

    MutablePair<Boolean, ClearCLImage> lPair = getImageSlotsMap().get(pSlotKey);
    ...
    if (lImage == null)
    {
      lImage = lMemoryPool.requestImage(pSlotKey, pImageChannelDataType, pDimensions);
  ...
  @Override
  public void assignImageToAnotherSlotKey(final String pSrcSlotKey, final String pDstSlotKey)
  {
    MutablePair<Boolean, ClearCLImage> lDstPair = getImageSlotsMap().get(pDstSlotKey);
    ...
    MutablePair<Boolean, ClearCLImage> lSrcPair = getImageSlotsMap().get(pSrcSlotKey);
  ...
  {
    MutablePair<Boolean, ClearCLImage> lMutablePair = getImageSlotsMap().remove(pSlotKey);
  ...
  {
    MutablePair<Boolean, ClearCLImage> lMutablePair = getImageSlotsMap().get(pSlotKey);
    if (lMutablePair == null) return false;
  ...
  private int executeSeveralTasks(int pExecutedNumberOfTasks, int pMaxNumberOfTasks)
  {
    assert 0 <= pExecutedNumberOfTasks && pExecutedNumberOfTasks <= pMaxNumberOfTasks;
    if (pExecutedNumberOfTasks == pMaxNumberOfTasks) return pExecutedNumberOfTasks;
    ...
    for (TaskInterface lTask : mFusionTasks)
    {
      if (!mExecutedFusionTasks.contains(lTask))
      {
        if (lTask.checkIfRequiredImagesAvailable(lAvailableImageKeys))
        {
          ...
          return executeSeveralTasks(pExecutedNumberOfTasks + 1, pMaxNumberOfTasks);
        } else
        {
