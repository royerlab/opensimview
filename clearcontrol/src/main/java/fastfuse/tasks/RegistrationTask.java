 ...
 */
public class RegistrationTask extends TaskBase implements TaskInterface
{

  private SimpleExponentialSmoothing mSmoother = new SimpleExponentialSmoothing(6, 0.1);

  private CopyOnWriteArrayList<RegistrationListener> mListenerList = new CopyOnWriteArrayList<>();

  private String[] mInputImagesSlotKeys;
  private String mTransformedImageSlotKey;

  private RegistrationParameters mRegistrationParameters = new RegistrationParameters();
  private Registration mRegistration;

  /**
   * Instantiates a registered fusion task
   *
   * @param pImageProcessedReferenceSlotKey        first stack (reference volume for registration)
   * @param pImageProcessedToRegisterSlotKey       second stack (volume to be registered to reference volume)
   * @param pImageOriginalReferenceSlotKey         original/reference data for pImageProcessedReferenceSlotKey
   * @param pImageOriginalToRegisterSlotKey        original/reference data for pImageprocessedToRegisterSlotKey, to
   *                                               be transformed after registration has been found
   * @param pImageOriginalToRegisterTransformedKey transformed version of pImageOriginalToRegisterSlotKey
   */
  public RegistrationTask(String pImageProcessedReferenceSlotKey, String pImageProcessedToRegisterSlotKey, String pImageOriginalReferenceSlotKey, String pImageOriginalToRegisterSlotKey, String pImageOriginalToRegisterTransformedKey)
  {
    super(pImageProcessedReferenceSlotKey, pImageProcessedToRegisterSlotKey, pImageOriginalReferenceSlotKey, pImageOriginalToRegisterSlotKey);
    mInputImagesSlotKeys = new String[]{pImageProcessedReferenceSlotKey, pImageProcessedToRegisterSlotKey, pImageOriginalReferenceSlotKey, pImageOriginalToRegisterSlotKey};
   ...
   *
   * @param pSmoothingConstant smoothing constant
   */
  ...
  public boolean enqueue(FastFusionEngineInterface pFastFusionEngine, boolean pWaitToFinish)
  {
    ...
    assert TaskHelper.allSameDataType(ImageChannelDataType.Float, lImageA, lImageB, lImageC, lImageD);
    assert TaskHelper.allSameDimensions(lImageA, lImageB, lImageC, lImageD);

    if (mRegistration == null) mRegistration = new Registration(mRegistrationParameters, lImageA, lImageB);
    mRegistration.setImages(lImageA, lImageB);

    mRegistration.getParameters().setWaitToFinish(pWaitToFinish);

    double[] lBestTransform = mRegistration.getParameters().getInitialTransformation();
    try
    {
      ...
      double lBestScoreOriginalImages = mRegistration.computeScore(lBestTransform);
      ...
      lBestScoreOriginalImages = mRegistration.computeScore(lBestTransform);
      System.out.printf("---\nscore = %.6f: %s\nfor smoothed transformation on original images\n", lBestScoreOriginalImages, Arrays.toString(lBestTransform));
    ...
    MutablePair<Boolean, ClearCLImage> lFlagAndRegisteredImage = pFastFusionEngine.ensureImageAllocated(mTransformedImageSlotKey, lImageA.getChannelDataType(), lImageA.getDimensions());

    ClearCLImage lRegisteredImage = lFlagAndRegisteredImage.getRight();
    mRegistration.transform(lRegisteredImage, lImageD, lBestTransform);
    // notify listeners
   ...
   *
   * @param pRegistrationListener registration listener
   */
  public void addListener(RegistrationListener pRegistrationListener)
  {
    if (!mListenerList.contains(pRegistrationListener)) mListenerList.add(pRegistrationListener);
  }
