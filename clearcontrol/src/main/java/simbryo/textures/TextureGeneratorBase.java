 ...
 * @author royer
 */
public abstract class TextureGeneratorBase implements TextureGeneratorInterface
{
   ...
   *
   * @param pNormalizedTexture true -> normalized texture
   */
   ...
   *
   * @param pScale scales for al dimensions as an array of floats.
   */
    ...
    } else if (pDimensions.length == 3)
    {
      ...
      for (int z = 0; z < lDepth; z++)
        for (int y = 0; y < lHeight; y++)
          for (int x = 0; x < lWidth; x++)
          {
            int i = Math.toIntExact(x + y * lWidth + z * lWidth * lHeight);
  ...
  {
    if (!isNormalizedTexture()) return pTextureArray;
    ...
    if (lMax == lMin) return pTextureArray;
