...
public class SynthoscopySandBox
{
  ...
  public void demo() throws IOException, InterruptedException
  {
    ...
    try
    {
      ...
      File lDesktopFolder = new File(System.getProperty("user.home") + "/Temp/data");
      ...
      try (ClearCL lClearCL = new ClearCL(lBestBackend); ClearCLDevice lFastestGPUDevice = lClearCL.getFastestGPUDeviceForImages(); ClearCLContext lContext = lFastestGPUDevice.createContext())
      {
        ...
        LightSheetMicroscopeSimulatorOrtho lSimulator = new LightSheetMicroscopeSimulatorOrtho(lContext, lNumberOfDetectionArms, lNumberOfIlluminationArms, lMaxCameraResolution, lPhantomWidth, lPhantomHeight, lPhantomDepth);
        ...
        lSimulator.setNumberParameter(IlluminationParameter.Height, 0, 1f);
        lSimulator.setNumberParameter(IlluminationParameter.Height, 1, 0.2f);

        lSimulator.setNumberParameter(IlluminationParameter.Intensity, 0, 50f);
        lSimulator.setNumberParameter(IlluminationParameter.Intensity, 1, 0f);

        lSimulator.setNumberParameter(IlluminationParameter.Gamma, 0, 0f);
        lSimulator.setNumberParameter(IlluminationParameter.Gamma, 1, 20f);

        int i = 0;

        for (float z = -0.0f; z < 0.3 && lCameraImageViewer.isShowing(); z += 0.001)
        {

          lSimulator.setNumberParameter(IlluminationParameter.Z, 0, z);
          lSimulator.setNumberParameter(IlluminationParameter.Z, 1, z);
  ...
  private void simulate_single_beams(final int side, final float angle, final int n_planes, final String outdir) throws IOException, InterruptedException
  {
    ...
    try
    {
      ...
      System.out.format("side =  %d, angle = %f, outdir = %s", side, angle, outdir);
      ...
      try (ClearCL lClearCL = new ClearCL(lBestBackend); ClearCLDevice lFastestGPUDevice = lClearCL.getFastestGPUDeviceForImages(); ClearCLContext lContext = lFastestGPUDevice.createContext())
      {
        ...
        lSimulator.setNumberParameter(IlluminationParameter.Height, 0, 1.2f);
        lSimulator.setNumberParameter(IlluminationParameter.Height, 1, 1.2f);

        lSimulator.setNumberParameter(IlluminationParameter.Intensity, side, 50f);
        lSimulator.setNumberParameter(IlluminationParameter.Intensity, 1 - side, 0f);

        lSimulator.setNumberParameter(IlluminationParameter.Gamma, side, angle);
        lSimulator.setNumberParameter(IlluminationParameter.Gamma, 1 - side, 0f);

        int counter = 0;

        for (float z = -z_max; z <= z_max && lCameraImageViewer.isShowing(); z += 2.f * z_max / (n_planes - 1.f))
        {
   ...
   */
  @Test
  public void demo_single_arms() throws IOException, InterruptedException
  {
    ...
    for (int side = 0; side < 2; side++)
      for (int angle_mode = 0; angle_mode < 2; angle_mode++)
        simulate_single_beams(side, ((angle_mode < 1) ? -1.f : 1.f) * 20.f, 101, System.getProperty("user.home") + "/Tmp/xscope");
  }
}
