  ...
  private void test(int pNumberOfDetectionArms, int pNumberOfIlluminationArms)
  {
    try
    {
      ...
      try (ClearCL lClearCL = new ClearCL(lBestBackend); ClearCLDevice lFastestGPUDevice = lClearCL.getFastestGPUDeviceForImages(); ClearCLContext lContext = lFastestGPUDevice.createContext())
      {
        ...
        LightSheetMicroscopeSimulatorDrosophila lSimulator = new LightSheetMicroscopeSimulatorDrosophila(lContext, pNumberOfDetectionArms, pNumberOfIlluminationArms, 1024, 11f, lPhantomWidth, lPhantomHeight, lPhantomDepth);
