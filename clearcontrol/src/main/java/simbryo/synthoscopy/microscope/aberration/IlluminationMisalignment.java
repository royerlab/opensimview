 ...
 * @author royer
 */
public class IlluminationMisalignment extends AberrationBase implements AberrationInterface
{
  private volatile float mOffsetConstant = 1f;

  private HashMap<MutablePair<ParameterInterface<Number>, Integer>, Number> mOffsetMap = new HashMap<>();

  private float mAlpha, mBeta, mGamma, mHeight, mIntensity, mTheta, mX, mY, mZ;

  /**
   * Instantiates an illumination misalignment for a given offset in X, Y and Z
   *
   * @param pX X misalignment
   * @param pY Y misalignment
   * @param pZ Z misalignment
   * @return illumination misalignment
   */
  public static IlluminationMisalignment buildXYZ(float pX, float pY, float pZ)
  {
    IlluminationMisalignment lIlluminationMisalignment = new IlluminationMisalignment();
  ...
  {
    if (!(pParameter instanceof IlluminationParameter)) return pNumber;

    Number lNumber = pNumber;
    MutablePair<ParameterInterface<Number>, Integer> lKey = MutablePair.of(pParameter, pIndex);
