 ...
 * @author royer
 */
public class DetectionMisalignment extends AberrationBase implements AberrationInterface
{
  ...
  public Number transform(ParameterInterface<Number> pParameter, int pIndex, Number pNumber)
  {
    ...
    if (pParameter instanceof DetectionParameter)
    {
      if (lOffset == null)
      {
        float lOffsetConstant = adjustOffsetAmountDetection(pParameter);
    ...
    } else if (pParameter instanceof CameraParameter)
    {
      ...
      if (lOffset == null)
      {
        ...
        float lOffsetConstant = adjustOffsetAmountCamera(pParameter, pIndex);
  ...
  protected float adjustOffsetAmountCamera(ParameterInterface<Number> pParameter, int pIndex)
  {
    float lPixelSize = 1.0f / getMicroscope().getCameraRenderer(pIndex).getMaxWidth();
    float lOffsetConstant = 0;
