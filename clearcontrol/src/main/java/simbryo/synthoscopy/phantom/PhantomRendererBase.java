 ...
 * @author royer
 */
public abstract class PhantomRendererBase extends ClearCLSynthoscopyBase implements PhantomRendererInterface<ClearCLImage>
{
  private final TissueDynamicsInterface mTissue;
  protected boolean[] mPlaneAlreadyDrawnTable;

  private volatile float mSignalIntensity = 1, mNoiseOverSignalRatio = 0f;
   ...
   *
   * @param pContext                 OpenCL context to use.
   * @param pMaxParticlesPerGridCell max number of articles per cell
   * @param pTissueDynamics          tissue dynamics object
   * @param pStackDimensions         stack dimensions
   */
  public PhantomRendererBase(final ClearCLContext pContext, int pMaxParticlesPerGridCell, final TissueDynamicsInterface pTissueDynamics, final long... pStackDimensions)
  {
    super(pContext, true, ImageChannelDataType.Float, pStackDimensions);
    mTissue = pTissueDynamics;
    mPlaneAlreadyDrawnTable = new boolean[Math.toIntExact(getDepth())];

    mContext = pContext;

    int[] lGridDimensions = PhantomRendererUtils.getOptimalGridDimensions(pContext.getDevice(), pStackDimensions);

    mNeighborhoodGrid = new NeighborhoodGrid(pMaxParticlesPerGridCell, lGridDimensions);
  ...
  protected boolean isValid(int pZPlaneIndexBegin, int pZPlaneIndexEnd)
  {
    for (int i = pZPlaneIndexBegin; i < pZPlaneIndexEnd; i++)
      if (!mPlaneAlreadyDrawnTable[i]) return false;
    return true;
  }
  ...
  @Override
  public void render(int pZPlaneIndexBegin, int pZPlaneIndexEnd, boolean pWaitToFinish)
  {
   ...
   *
   * @param pZPlaneIndexBegin begin z index
   * @param pZPlaneIndexEnd   end z index
   * @param pWaitToFinish     true -> wait to finish
   * @return number of planes rendered
   */
  public int renderAndCount(int pZPlaneIndexBegin, int pZPlaneIndexEnd, boolean pWaitToFinish)
  {
    if (!isUpdateNeeded() && isValid(pZPlaneIndexBegin, pZPlaneIndexEnd)) return 0;
    ...
    {
      while (zi < pZPlaneIndexEnd && mPlaneAlreadyDrawnTable[zi]) zi++;

      int zj = zi;
      while (zj < pZPlaneIndexEnd && !mPlaneAlreadyDrawnTable[zj]) zj++;
    ...
    if (pWaitToFinish) mContext.getDefaultQueue().waitToFinish();
  ...
  protected void renderInternal(int pZPlaneIndexBegin, int pZPlaneIndexEnd, boolean pWaitToFinish)
  {

    // First we snap the rendering z bounds to the grid cell boundaries:
    pZPlaneIndexBegin = (int) (Math.floor((float) pZPlaneIndexBegin / mLocalSizeZ) * mLocalSizeZ);
    pZPlaneIndexEnd = (int) (Math.ceil((float) pZPlaneIndexEnd / mLocalSizeZ) * mLocalSizeZ);

    // Now we can render a possibly slightly larger chunck of the stack:

    if (pZPlaneIndexEnd == pZPlaneIndexBegin) pZPlaneIndexEnd += mLocalSizeZ;
    ...
    mRenderKernel.setGlobalSizes(getWidth(), getHeight(), pZPlaneIndexEnd - pZPlaneIndexBegin);
    ...
    mRenderKernel.setOptionalArgument("timeindex", (int) getTissue().getTimeStepIndex());
  ...
  public void close()
  {
    if (mRenderKernel != null) mRenderKernel.close();
    super.close();
  }
