...
public abstract class PhantomRendererUtils
{
   ...
   *
   * @param pDevice          device to adapt to
   * @param pImageDimensions image dimensions
   * @return possibly modified image dimensions
   */
  public static long[] adaptImageDimensionsToDevice(ClearCLDevice pDevice, long[] pImageDimensions)
  {
    int[] lOptimalGridDimensions = getOptimalGridDimensions(pDevice, pImageDimensions);

    long[] lNewDimensions = new long[pImageDimensions.length];

    for (int i = 0; i < pImageDimensions.length; i++)
      lNewDimensions[i] = (pImageDimensions[i] / lOptimalGridDimensions[i]) * lOptimalGridDimensions[i];
   ...
   * @param pStackDimensionsLong stack dimensions
   * @return array of grid dimensions
   */
  public static int[] getOptimalGridDimensions(ClearCLDevice pDevice, long... pStackDimensionsLong)
  {
   ...
   * @return array of grid dimensions
   */
  public static int[] getOptimalGridDimensions(ClearCLDevice pDevice, int... pStackDimensions)
  {
    int lDimension = pStackDimensions.length;
    int[] lGridDimensions = new int[lDimension];

    long lMaxWorkGroupSize = pDevice.getType().isCPU() ? 1 : pDevice.getMaxWorkGroupSize();

    int lMaxGridDim = excludeNonOneOdd((int) floor(max(1, min(8, Math.pow(lMaxWorkGroupSize, 1.0 / 3)))));
