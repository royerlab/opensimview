...
public abstract class HistoneFluorescence extends PhantomRendererBase implements PhantomRendererInterface<ClearCLImage>
{
   ...
   * @param pContext                 OpenCL context
   * @param pTissueDynamics          tissue dynamics
   * @param pMaxParticlesPerGridCell max number of particles/nuclei/cells per work group. This depends
   *                                 on the density of the cells and on the capabilities of the OpenCL
   *                                 device.
   * @param pNucleiRadius            nuclei radius
   * @param pNucleiSharpness         nuclei sharpness
   * @param pNucleiRoughness         nuclei roughness
   * @param pNucleiTextureContrast   nuclei texture contrast
   * @param pNoiseOverSignalRatio    noise over signal ratio
   * @param pStackDimensions         stack dimensions
   * @throws IOException thrown in OpenCL kernels cannot be read.
   */
  public HistoneFluorescence(ClearCLContext pContext, TissueDynamicsInterface pTissueDynamics, int pMaxParticlesPerGridCell, float pNucleiRadius, float pNucleiSharpness, float pNucleiRoughness, float pNucleiTextureContrast, float pNoiseOverSignalRatio, long... pStackDimensions) throws IOException
  {
    ...
    final int lMaxParticlesPerGridCell = mNeighborhoodGrid.getMaxParticlesPerGridCell();
  ...
  private void setupBuffersAndImages(TissueDynamicsInterface pTissueDynamics, final int lMaxParticlesPerGridCell)
  {
    final int lDimension = getTissue().getDimension();

    final int lNeighboorsArrayLength = mNeighborhoodGrid.getVolume() * lMaxParticlesPerGridCell;

    mNeighboorsBuffer = mContext.createBuffer(HostAccessType.WriteOnly, KernelAccessType.ReadOnly, NativeTypeEnum.Int, lNeighboorsArrayLength);

    mPositionsBuffer = mContext.createBuffer(HostAccessType.WriteOnly, KernelAccessType.ReadOnly, NativeTypeEnum.Float, lDimension * pTissueDynamics.getMaxNumberOfParticles());

    mPolaritiesBuffer = mContext.createBuffer(HostAccessType.WriteOnly, KernelAccessType.ReadOnly, NativeTypeEnum.Float, lDimension * pTissueDynamics.getMaxNumberOfParticles());
  ...
  protected void setupProgramAndKernel(final int pMaxParticlesPerGridCell) throws IOException
  {
    ...
    lProgram.addSource(HistoneFluorescence.class, "kernel/HistoneFluoRender.cl");
    ...
    lProgram.addDefine("NUCLEITEXTURECONTRAST", getNucleiTextureContrast());
   ...
   *
   * @param pClearCLProgram program to add source to.
   * @throws IOException thrown if source code canot be read.
   */
  ...
  private void setupNoiseBuffers(ClearCLContext pContext) throws IOException
  {
    ...
    FractalNoise lFractalNoise = new FractalNoise(lSimplexNoise, 1f, 0.5f, 0.25f, 0.125f, 0.0625f);
    // lFractalNoise.setScales(1000f);

    float[] lTexture = lFractalNoise.generateTexture(cNoiseDim, cNoiseDim, cNoiseDim);

    mPerlinNoiseImage = pContext.createSingleChannelImage(ImageChannelDataType.Float, cNoiseDim, cNoiseDim, cNoiseDim);
  ...
  protected void updateBuffers()
  {
    ...
    ElapsedTime.measure("data copy", () ->
    {
      ...
      mPositionsMemory.copyFrom(getTissue().getPositions().getCurrentArray(), 0, 0, lDimension * lNumberOfCells);

      if (mHasPolarity)
      {
        HasPolarity lHasPolarity = (HasPolarity) getTissue();
        float[] lPolarityArray = lHasPolarity.getPolarityProperty().getArray().getCurrentArray();

        mPolaritiesMemory.copyFrom(lPolarityArray, 0, 0, lDimension * lNumberOfCells);
      }
      ...
      mPositionsBuffer.readFrom(mPositionsMemory.subRegion(0, lNumberOfCells * 3 * Size.FLOAT), false);

      mPolaritiesBuffer.readFrom(mPolaritiesMemory.subRegion(0, lNumberOfCells * 3 * Size.FLOAT), false);

      mRadiiBuffer.readFrom(mRadiiMemory.subRegion(0, lNumberOfCells * Size.FLOAT), true);
    });
    /**/
  }
   ...
   *
   * @param pNucleiTextureContrast new contrast of nuclei texture
   */
  ...
  {
    if (isUpdateNeeded()) updateBuffers();
