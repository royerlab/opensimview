...
public class StrogatzWaveOperator extends OperatorBase<CellProperty>
{
   ...
   */
  public StrogatzWaveOperator(float pIncrement, float pCouplingConstant, float pNeighboorhoodRadiusDilationFactor)
  {
    super();
  ...
  @Override
  public void apply(int pBeginId, int pEndId, TissueDynamics pEmbryo, CellProperty... pCellProperty)
  {
    ...
    final float[] lVelocities = pEmbryo.getVelocities().getCurrentArray();
    ...
    final float[] lCellPropertyArrayRead = lCellProperty.getArray().getReadArray();
    final float[] lCellPropertyArrayWrite = lCellProperty.getArray().getWriteArray();
    ...
    for (int idu = pBeginId; idu < pEndId; idu++)
    {
      ...
      float lNewValue = lOldValue + lIncrement + detectNeighboringEvent(lDimension, lNeighborhood, lCouplingConstant, lNeighborhoodRadiusFactor, lPositions, lVelocities, lRadii, lCellPropertyArrayRead, lNeighboors, lNumberOfNeighboors, idu);

      boolean lEvent = (int) lNewValue > (int) lOldValue;

      lNewValue = (lEvent ? (int) lNewValue : lNewValue);

      lCellPropertyArrayWrite[idu] = eventHook(lEvent, idu, lPositions, lVelocities, lRadii, lNewValue);
   ...
   *
   */
  private float detectNeighboringEvent(int pDimension, NeighborhoodGrid pNeighborhood, final float pCouplingConstant, final float pNeighborhoodRadiusFactor, final float[] pPositions, final float[] pVelocities, final float[] pRadii, final float[] pMorphogenArrayRead, final int[] pNeighboors, final int pNumberOfNeighboors, int idu)
  {
    ...
    for (int k = 0; k < pNumberOfNeighboors; k++)
    {
      ...
      if (idu != idv) //
      {
        float lDistance = GeometryUtils.computeDistance(pDimension, pPositions, idu, idv);
        float lGap = lDistance - (1 + pNeighborhoodRadiusFactor) * ru - (1 + pNeighborhoodRadiusFactor) * rv;

        boolean lBoundaryPassed = ((int) pMorphogenArrayRead[idv]) > ((int) pMorphogenArrayRead[idu]);
  ...
  protected float eventHook(boolean pEvent, int pId, float[] pPositions, float[] pVelocities, float[] pRadii, float pNewMorphogenValue)
  {
