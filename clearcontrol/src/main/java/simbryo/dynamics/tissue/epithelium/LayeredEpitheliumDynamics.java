 ...
 * @author royer
 */
public class LayeredEpitheliumDynamics extends TissueDynamics implements TissueDynamicsInterface, HasLayers, Serializable
{
  private static final long serialVersionUID = 1L;

  private ArrayList<IsoSurfaceInterface> mEpitheliumLayerIsoSurfaceList = new ArrayList<>();
  private ArrayList<ForceFieldInterface> mEpitheliumLayerForceFieldList = new ArrayList<>();
   ...
   * @param pCohesionForce                   cohesion force
   ...
   */
  public LayeredEpitheliumDynamics(float pCollisionForce, float pCohesionForce, float pDrag, int pMaxNumberOfParticlesPerGridCell, int... pGridDimensions)
  {
    super(pCollisionForce, pDrag, pMaxNumberOfParticlesPerGridCell, pGridDimensions);

    mCohesionForceField = new CohesionForceField(pCohesionForce, pDrag);

    mEpitheliumLayerCellForceProperty = addCellProperty();

    mEpitheliumLayerCellForceProperty.set(0, getMaxNumberOfParticles(), 0f);
  ...
  @Override
  public void addLayer(IsoSurfaceInterface pLayerIsoSurface, float pLayerForceIntensity)
  {
    mEpitheliumLayerIsoSurfaceList.add(pLayerIsoSurface);

    ForceFieldInterface lForceField = new IsoSurfaceForceField(pLayerForceIntensity, pLayerIsoSurface);
   ...
   *
   * @param pCellId  cell id
   * @param pLayerId layer id
   */
  public void assignCellToLayer(int pCellId, float pLayerId)
  {
    float[] lCurrentArray = mEpitheliumLayerCellForceProperty.getArray().getCurrentArray();
  ...
  public void simulationSteps(int pNumberOfSteps)
  {
    for (int i = 0; i < pNumberOfSteps; i++)
    {
      for (int layer = 0; layer < getNumberOfLayers(); layer++)
      {
        ...
        applyForceField(mEpitheliumLayerForceFieldList.get(layer), 0, getNumberOfParticles(), mMask);
      }
