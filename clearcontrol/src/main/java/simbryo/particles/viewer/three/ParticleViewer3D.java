 ...
 * @author royer
 */
public class ParticleViewer3D extends Stage implements ParticleViewerInterface
{
  ...
  private final PerspectiveCamera mPerspectiveCamera = new PerspectiveCamera(true);
  private final CameraGroup mCameraGroup = new CameraGroup();
  private final ParticleViewerGroup mParticleViewerGroup;

  private volatile double mousePosX, mousePosY, mouseOldX, mouseOldY, mouseDeltaX, mouseDeltaY;
   ...
   * @return viewer.
   */
  public static ParticleViewer3D view(ParticleSystem pParticleSystem, String pWindowTitle, int pWindowWidth, int pWindowHeight)
  {
    return JavaFXUtil.runAndWait(() ->
    {
      return new ParticleViewer3D(pParticleSystem, pWindowTitle, pWindowWidth, pWindowHeight);
    });
  }
   ...
   * @param pWindowHeight   window height
   */
  public ParticleViewer3D(ParticleSystem pParticleSystem, String pWindowTitle, int pWindowWidth, int pWindowHeight)
  {
    super();
    ...
    Scene lScene = new Scene(mRoot, pWindowWidth, pWindowHeight, true);
  ...
  private void handleMouse(Scene pScene)
  {
    ...
    pScene.setOnMouseDragged((MouseEvent e) ->
    {
      ...
      {
        mPerspectiveCamera.setTranslateX(mPerspectiveCamera.getTranslateX() - mouseDeltaX);
        mPerspectiveCamera.setTranslateY(mPerspectiveCamera.getTranslateY() - mouseDeltaY);
      }
    });
    ...
    pScene.setOnScroll((e) ->
    {
      ...
      mPerspectiveCamera.setTranslateZ(mPerspectiveCamera.getTranslateZ() + lDeltaZ);
