...
public class ParticleViewerGroup extends Group
{
  ...
  private volatile boolean mDisplayGrid, mDisplayElapsedTime, mDisplayRadius = true;
  ...
  private final PhongMaterial mBoxMaterial, mParticleMaterial, mParticlePolarityMaterial;

  private HashMap<Color, PhongMaterial> mMaterialForColorMap = new HashMap<>();
  ...
  public ParticleViewerGroup(int pWidth, int pHeight, int pDepth)
  {
    ...
    getChildren().addAll(lBoundingBox, mParticlesGroup, mParticlesPolarityGroup);
  }
   ...
   *
   * @param pDisplayGrid display-grid flag
   */
   ...
   *
   * @param pDisplayElapsedTime true -> display ellapsed time, false otherwise.
   */
   ...
   *
   * @param pDisplayRadius true -> display radius, false otherwise
   */
   ...
   */
  public void updateDisplay(ParticleSystem pParticleSystem, boolean pBlocking)
  {
    boolean lHasPolarity = pParticleSystem instanceof HasPolarity;
    VectorCellProperty lPolarityProperty = null;
    if (lHasPolarity) lPolarityProperty = ((HasPolarity) pParticleSystem).getPolarityProperty();

    long lNow = System.nanoTime();
    float lNewElapsedTime = (float) (1e-6 * (lNow - mLastUpdateTimeInNanos));

    mLastUpdateTimeInNanos = lNow;

    mElapsedTime = (float) (0.99 * mElapsedTime + 0.01 * lNewElapsedTime);
    ...
    final int lNumberOfParticles = pParticleSystem.getNumberOfParticles();
    ...
    if (mPositions == null || mPositions.length != lNumberOfParticles * lDimension)
    {
      ...
      if (lHasPolarity) mPolarities = new float[lNumberOfParticles * lDimension];
    }

    pParticleSystem.copyPositions(mPositions);
    pParticleSystem.copyVelocities(mVelocities);
    pParticleSystem.copyRadii(mRadiis);
    if (lHasPolarity) lPolarityProperty.getArray().copyCurrentArrayTo(mPolarities, mPolarities.length);

    Platform.runLater(() ->
    {

      getChildren().remove(mParticlesGroup);
      if (lHasPolarity) getChildren().remove(mParticlesPolarityGroup);

      final ObservableList<Node> lParticlesSpheres = mParticlesGroup.getChildren();

      final ObservableList<Node> lParticlesPolaritySpheres = mParticlesPolarityGroup.getChildren();
      ...
      while (lParticlesSpheres.size() > pParticleSystem.getNumberOfParticles())
      {
        ...
        if (lHasPolarity) lParticlesPolaritySpheres.remove(lParticlesPolaritySpheres.size() - 1);
        // System.out.println("REMOVING");
      }

      for (int id = 0, i = 0; id < lNumberOfParticles; id++, i += lDimension)
      {
        ...
        if (mColorClosure != null) lSphere.setMaterial(getMaterialForColor(getColor(id)));

        if (lHasPolarity)
        {

          Sphere lPolaritySphere = (Sphere) lParticlesPolaritySpheres.get(id);
          ...
          double lPolarityWorldX = lSphereWorldX + lRadius * lPolarityX;
          double lPolarityWorldY = lSphereWorldY + lRadius * lPolarityY;
          double lPolarityWorldZ = lSphereWorldZ + lRadius * lPolarityZ;
      ...
      if (lHasPolarity) getChildren().add(mParticlesPolarityGroup);
