...
public class ParticleViewerCanvas extends Canvas
{
   ...
   *
   * @param pDisplayGrid true -> display grid, false otherwise
   */
   ...
   * @param pBlocking       true -> bloaking call, fals -> asynchronous call
   */
  public void updateDisplay(ParticleSystem pParticleSystem, boolean pBlocking)
  {
    ...
    Platform.runLater(() ->
    {
      ...
      for (int id = 0, i = 0; id < lNumberOfParticles; id++, i += lDimension)
      {
        ...
        double lRadius = mDisplayRadius ? lMinWidthHeight * r : lMinWidthHeight * 0.005f;
        ...
        lSaturation = lSaturation > 1 ? 1 : (lSaturation < 0 ? 0 : lSaturation);

        /*if (id == 0)
          gc.setFill(Color.WHITE);
        else/**/
        gc.setFill(Color.hsb((223 * id) % 359, lSaturation, lBrightness));
      ...
      {
        gc.setFill(Color.WHITE);
        String lFrameRateText = String.format("%.3f ms", mElapsedTime);
