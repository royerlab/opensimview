 ...
 */
public class RepulsionForceField extends InteractionForceFieldBase implements InteractionForceFieldInterface
{
   ...
   * @param pNumberOfInteractionPartners number of interaction partners
   */
  public RepulsionForceField(float pForceIntensity, int pNumberOfInteractionPartners)
  {
  ...
  {
    if (mRandom == null) mRandom = new SplittableRandom();
    ...
    pParticleSystem.getVelocities().copyDefault(pBeginId * lDimension, pEndId * lDimension);

    final int lNumberOfInteractionPartners = mNumberOfInteractionPartners;
    final float lForceIntensity = mForceIntensity / lNumberOfInteractionPartners;
    ...
    for (int idu = pBeginId, i = idu * lDimension; idu < pEndId; idu++, i += lDimension)
    {
      ...
      for (int k = 0; k < lNumberOfInteractionPartners; k++)
      {
        ...
        float lInvDistanceWithForce = lForceIntensity * (pForceFactor != null ? pForceFactor[idu] * pForceFactor[idv] : 1) / lDistance;
