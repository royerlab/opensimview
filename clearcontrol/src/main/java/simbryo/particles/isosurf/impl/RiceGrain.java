 ...
 * @author royer
 */
public class RiceGrain extends Ellipsoid implements IsoSurfaceInterface
{
   ...
   * @param pCenterAndAxis center and axis parameters
   */
  public RiceGrain(float pRadius, float... pCenterAndAxis)
  {
    super(pRadius, pCenterAndAxis);

    if (pCenterAndAxis.length / 2 != 3) throw new IllegalArgumentException("Dimension must be 3");
  ...
  public void addCoordinate(float pValue)
  {
    ...
    {
      float lFunction = (float) (pow(x, 2) / pow(a, 2) + pow(y, 2) * (1 / pow(b, 2) + s / (pow(x, 2) + 1)) + pow(z, 2) * (1 / pow(c, 2) + s / (pow(x, 2) + 1)));

      mDistance = (float) (Math.sqrt(lFunction) - mRadius);

      float dx = (float) ((2 * x * (-pow(a, 2) * s * (pow(y, 2) + pow(z, 2)) + pow(x, 4) + 2 * pow(x, 2) + 1)) / pow(pow(a, 2) * (pow(x, 2) + 1), 2));
      float dy = (float) (2 * y * (1 / pow(b, 2) + s / (pow(x, 2) + 1)));
      float dz = (float) (2 * z * (1 / pow(c, 2) + s / (pow(x, 2) + 1)));

      float lGradientSquaredLength = dx * dx + dy * dy + dz * dz;

      float lGradientlength = (float) Math.sqrt(lGradientSquaredLength);
