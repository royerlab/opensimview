...
{
  protected StackTraceElement[] mAllocationStackTrace = new StackTraceElement[]{new StackTraceElement("NULL", "NULL", "NULL", -1)};
   ...
   * resource.
   *
   * @param pParent        parent reference to prevent the parent's garbage collection.
   * @param pAddress       address
   * @param pLengthInBytes length in bytes
   * @return off-heap memory object
   */
  public static final OffHeapMemory wrapPointer(final Object pParent, final long pAddress, final long pLengthInBytes)
  {
   ...
   * resource.
   *
   * @param pName          memory region name
   * @param pParent        parent reference to prevent the parent's garbage collection.
   * @param pAddress       address
   * @param pLengthInBytes length in bytes
   * @return off-heap memory object
   */
  public static final OffHeapMemory wrapPointer(final String pName, final Object pParent, final long pAddress, final long pLengthInBytes)
  {
  ...
  public static OffHeapMemory wrapPointer(com.sun.jna.Pointer pJNAPointer, long pTargetSizeInBytes)
  {
    ...
    return wrapPointer(pJNAPointer.toString(), pJNAPointer, lAddress, pTargetSizeInBytes);
  }
   ...
   *
   * @param pBridJPointer BridJ pointer
   * @return off-heap memory object
   */
  public static OffHeapMemory wrapPointer(org.bridj.Pointer<Byte> pBridJPointer)
  {
    ...
    return wrapPointer(pBridJPointer.toString(), pBridJPointer, lAddress, lTargetSizeInBytes);
  }
   ...
   * byte array.
   *
   * @param pBuffer buffer to copy contents from
   * @return off-heap memory object
   */
   ...
   *
   * @param pNumberOfBytes number of bytes
   * @return off-heap memory object
   */
   ...
   *
   * @param pNumberOfChars number of chars
   * @return off-heap memory object
   */
   ...
   *
   * @param pNumberOfShorts number of shorts
   * @return off-heap memory object
   */
   ...
   *
   * @param pNumberOfInts number of ints
   * @return off-heap memory object
   */
   ...
   *
   * @param pNumberOfLongs number of longs
   * @return off-heap memory object
   */
   ...
   *
   * @param pNumberOfFloats number of floats
   * @return off-heap memory object
   */
   ...
   *
   * @param pNumberOfDoubles number of doubles
   * @return off-heap memory object
   */
   ...
   * Allocates off-heap memory that can hold a given number of bytes.
   *
   * @param pName          name (can be used to track allocation origin)
   * @param pNumberOfBytes number of bytes
   * @return off-heap memory object
   */
   ...
   * @param pName          name (can be used to track allocation origin)
   * @param pNumberOfBytes number of bytes
   * @param pAlignment     byte boundary to align to.
   * @return off-heap memory object
   */
  public static OffHeapMemory allocateAlignedBytes(String pName, long pNumberOfBytes, long pAlignment)
  {
    if (pAlignment == 0) return allocateBytes(pName, pNumberOfBytes);

    long lNumberOfBytesWithPadding = pNumberOfBytes + pAlignment;

    OffHeapMemory lAllocatedBytesWithPadding = allocateBytes(pName, lNumberOfBytesWithPadding);

    long lOffset = pAlignment - (lAllocatedBytesWithPadding.getAddress() % pAlignment);

    OffHeapMemory lAlignedRegion = lAllocatedBytesWithPadding.subRegion(lOffset, pNumberOfBytes);
  ...
  {
    this(pName, null, OffHeapMemoryAccess.allocateMemory(pLengthInBytes), pLengthInBytes);
  }
   ...
   * @param pLengthInBytes length in bytes
   */
  public OffHeapMemory(final Object pParent, final long pAddress, final long pLengthInBytes)
  {
   ...
   * @param pLengthInBytes length in bytes
   */
  public OffHeapMemory(final String pName, final Object pParent, final long pAddress, final long pLengthInBytes)
  {
    ...
    if (mParent == null) RessourceCleaner.register(this);
  }
   ...
   */
  @Override
  public OffHeapMemory subRegion(final long pOffset, final long pLenghInBytes)
  {
    if (mAddressInBytes + pOffset + pLenghInBytes > mAddressInBytes + mLengthInBytes)
      throw new InvalidNativeMemoryAccessException(String.format("Cannot instanciate OffHeapMemory on subregion staring at offset %d and length %d  ", pOffset, pLenghInBytes));
  ...
  public long resize(long pNewLength)
  {
    ...
    try
    {
      mAddressInBytes = OffHeapMemoryAccess.reallocateMemory(mAddressInBytes, pNewLength);
      mLengthInBytes = pNewLength;
    } catch (final Throwable e)
    {
      final String lErrorMessage = String.format("Could not resize memory region from %d to %d ", mLengthInBytes, pNewLength);
  ...
  {
    if (mParent != null) return new OffHeapMemoryCleaner(null, mSignature, mName, mAllocationStackTrace);
    return new OffHeapMemoryCleaner(mAddressInBytes, mSignature, mName, mAllocationStackTrace);
  }
  ...
  {
    return "OffHeapMemory [mParent=" + mParent + ", mAddressInBytes=" + mAddressInBytes + ", mLengthInBytes=" + mLengthInBytes + ", mIsFree=" + mIsFree + ", getMemoryType()=" + getMemoryType() + "]";
  }
