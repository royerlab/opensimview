...
public class OffHeapMemoryCleaner implements Cleaner
{
   ...
   *
   * @param pAddress              address to free
   * @param pSignature            corresponding signature
   * @param pName                 name of memory
   * @param pAllocationStackTrace allocation stack trace at allocation point
   */
  ...
  public void run()
  {
    try
    {
      ...
      if (OffHeapMemoryAccess.isAllocatedMemory(mAddressToClean, mSignature))
      {
        format("Cleaning memory: name=%s, address=%s, signature=%d \n", mName, mAddressToClean, mSignature);/**/
        try
        {
      ...
      } else if (OffHeapMemoryAccess.getSignature(mAddressToClean) != mSignature)
      {
        ...
        formatWarning("Stack:\n %s \n", Arrays.toString(mAllocationStackTrace).replaceAll(", ", "\n"));/**/
