...
public class BridJInterop
{
   ...
   * releaser
   *
   * @param pTargetClass target class
   * @param pAddress     address
   * @param pSizeInBytes size in bytes
   * @param pReleaser    releaser
   * @return BridJ pointer
   */
  public static <T> Pointer<T> getBridJPointer(Class<T> pTargetClass, long pAddress, long pSizeInBytes, Releaser pReleaser)
  {

    PointerIO<T> lPointerIO = PointerIO.getInstance(pTargetClass);

    Pointer<T> lPointerToAddress = Pointer.pointerToAddress(pAddress, pSizeInBytes, lPointerIO, pReleaser);
  ...
  public static Pointer getBridJPointer(MemoryBase pMemoryBase, Class pTargetClass)
  {
    ...
    final Pointer<?> lPointerToAddress = BridJInterop.getBridJPointer(pTargetClass, pMemoryBase.getAddress(), pMemoryBase.getSizeInBytes(), lReleaser);
