...
public interface RecyclerInterface<R extends RecyclableInterface<R, P>, P extends RecyclerRequestInterface> extends Freeable
{
   ...
   *
   * @param pNumberofPrealocatedRecyclablesNeeded number of preallocated recyclables
   * @param pRecyclerRequest                      request to use for instanciating the recyclable objects
   * @return effective number of recyclable objects allocated.
   */
  public abstract long ensurePreallocated(final long pNumberofPrealocatedRecyclablesNeeded, final P pRecyclerRequest);
   ...
   * @param pTimeUnit        wait time unit
   * @param pRecyclerRequest request
   * @return null if fails to get a new recyclable object before timeout
   */
  public abstract R getOrWait(final long pWaitTime, final TimeUnit pTimeUnit, final P pRecyclerRequest);

  /**
   * Requests a recyclable object (all other get methods delegate to this one).
   *
   * @param pWaitForLiveRecyclablesToComeBack as it names suggest, this flag causes this call to wait until an
   *                                          object is available.
   * @param pWaitTime                         wait time
   * @param pTimeUnit                         wait time unit
   * @param pRecyclerRequest                  request
   ...
   */
  public abstract R get(final boolean pWaitForLiveRecyclablesToComeBack, final long pWaitTime, final TimeUnit pTimeUnit, final P pRecyclerRequest);
   ...
   *
   * @param pObject recyclable object to release
   */
   ...
   *
   * @param pRecyclerListener recycler listener
   */
