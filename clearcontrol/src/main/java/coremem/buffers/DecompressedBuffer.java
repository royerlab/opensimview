...
public class DecompressedBuffer extends ContiguousBuffer
{
  ...
  private long writeDecompressedMemorySingleChunk(ContiguousMemoryInterface pContiguousMemory)
  {
    ...
    IBloscDll.blosc_decompress_ctx(JNAInterop.getJNAPointer(pContiguousMemory), JNAInterop.getJNAPointer(getRemainingContiguousMemory()), new NativeLong(lDecompressedSize), mNumThreads);
