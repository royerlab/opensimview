...
public final class MemoryMappedFileUtils
{
   ...
   * @param pFilePosition       file position in bytes
   * @param pMappedRegionLength mapped region length
   * @param pExtendIfNeeded     extend file length if needed (file not as long as request mapping
   *                            length).
   * @return mapped address
   * @throws MemoryMapFileException thrown if problem occurs while mapping.
   */
  public static final long map(FileChannel pFileChannel, MemoryMappedFileAccessMode pAccessMode, final long pFilePosition, final long pMappedRegionLength, final boolean pExtendIfNeeded) throws MemoryMapFileException
  {
    ...
    try
    {
      if (!pFileChannel.isOpen()) throw new ClosedChannelException();

      if ((pFilePosition % cPageSize) != 0)
        throw new IllegalArgumentException("File position must be page aligned (4096 byte boundaries)");
      if (pFilePosition < 0L) throw new IllegalArgumentException("Negative position");
      if (pMappedRegionLength < 0L) throw new IllegalArgumentException("Negative size");
      if (pFilePosition + pMappedRegionLength < 0L) throw new IllegalArgumentException("Position + size overflow");
      if (pMappedRegionLength > Long.MAX_VALUE) throw new IllegalArgumentException("Size exceeds Long.MAX_VALUE");
      ...
      if (pExtendIfNeeded)
      {
        if (pFileChannel.size() < pFilePosition + pMappedRegionLength)
        {
          ...
          pFileChannel.position(pFilePosition + pMappedRegionLength - 1);
      ...
      lMemoryMapMethod = pFileChannel.getClass().getDeclaredMethod("map0", Integer.TYPE, Long.TYPE, Long.TYPE);
      lMemoryMapMethod.setAccessible(true);
      final Object lReturnValue = lMemoryMapMethod.invoke(pFileChannel, pAccessMode.getValue(), pFilePosition, pMappedRegionLength);

      final Long lAddressAsLong = (Long) lReturnValue;

      final long lAddress = lAddressAsLong.longValue();
      OffHeapMemoryAccess.registerMemoryRegion(lAddress, pMappedRegionLength);

      lMappedAddress = lAddressAsLong;

    } catch (final Throwable e)
    {
      final String lErrorMessage = String.format("Cannot memory map file: %s at file position %d with length %d (%s)", pFileChannel.toString(), pFilePosition, pMappedRegionLength, e.getLocalizedMessage() != null ? e.getLocalizedMessage() : e.getCause().getLocalizedMessage());
   ...
   *
   * @param pFileChannel        file channel
   * @param pMemoryMapAddress   mapped address
   * @param pMappedRegionLength mapped region length
   * @return return code
   * @throws MemoryMapFileException thrown if problem occurs while unmapping
   */
  public static final int unmap(FileChannel pFileChannel, final long pMemoryMapAddress, final long pMappedRegionLength) throws MemoryMapFileException
  {
    int lIntReturnValue = 0;
    try
    {

      final Method lMemoryUnMapMethod = pFileChannel.getClass().getDeclaredMethod("unmap0", Long.TYPE, Long.TYPE);
      lMemoryUnMapMethod.setAccessible(true);
      final Object lReturnValue = lMemoryUnMapMethod.invoke(null, pMemoryMapAddress, pMappedRegionLength);
    ...
    {
      final String lErrorMessage = String.format("Cannot unmap memory at address %d with length %d (%s)", pMemoryMapAddress, pMappedRegionLength, e.getLocalizedMessage() != null ? e.getLocalizedMessage() : e.getCause().getLocalizedMessage());
      // e.printStackTrace();
      new MemoryMappedFileUtils().error("Native", lErrorMessage);
      throw new MemoryMapFileException(lErrorMessage, e);
    }
   ...
   *
   * @param pFileChannel file channel
   * @param pLength      truncation length
   * @throws IOException thrown if problem while truncating file
   */
  public static final void truncate(FileChannel pFileChannel, final long pLength) throws IOException
  {
    try
    {
    ...
    {
      final String lErrorMessage = String.format("Cannot truncate file %s at length %d (%s)", pFileChannel, pLength, e.getLocalizedMessage() != null ? e.getLocalizedMessage() : e.getCause().getLocalizedMessage());
      new MemoryMappedFileUtils().error("Native", lErrorMessage);
      throw new IOException(e);
    }
  ...
  public static MemoryMappedFileAccessMode bestMode(StandardOpenOption[] pStandardOpenOption)
  {
    ...
    if (lWrite) return MemoryMappedFileAccessMode.ReadWrite;

    if (lRead) return MemoryMappedFileAccessMode.ReadOnly;
