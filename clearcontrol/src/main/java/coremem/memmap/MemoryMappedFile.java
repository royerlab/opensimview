...
public class MemoryMappedFile implements AutoCloseable, Cleanable
{
   ...
   * @param pFilePosition       file position
   * @param pMappedRegionLength region length
   * @param pExtendIfNeeded     true -> extend if needed
   */
  public MemoryMappedFile(FileChannel pFileChannel, MemoryMappedFileAccessMode pAccessMode, final long pFilePosition, final long pMappedRegionLength, final boolean pExtendIfNeeded)
  {
    super();
    ...
    mMappingPointerAddress = MemoryMappedFileUtils.map(mFileChannel, mAccessMode, mActualMappingFilePosition, mActualMappingRegionLength, mExtendIfNeeded);
  ...
  {
    MemoryMappedFileUtils.unmap(mFileChannel, mMappingPointerAddress, mActualMappingRegionLength);
  ...
  {
    return new MemoryMappedFileCleaner(mFileChannel, mMappingPointerAddress, mActualMappingRegionLength, mSignature);
  }
