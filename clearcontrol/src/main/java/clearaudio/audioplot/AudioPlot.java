...
public class AudioPlot implements AutoCloseable
{
   ...
   * frequency.
   *
   * @param pSlowPeriod slow period
   * @param pFastPeriod fast period
   * @param pLowFreqHz  low frequency
   * @param pHighFreqHz high frequency
   */
  public AudioPlot(double pSlowPeriod, double pFastPeriod, double pLowFreqHz, double pHighFreqHz)
  {
   ...
   * @param pLowFreqHz   low frequency
   * @param pHighFreqHz  high frequency
   * @param pInvertRange if true the audio range is inverted with respect to the default
   */
  public AudioPlot(double pSlowPeriod, double pFastPeriod, double pLowFreqHz, double pHighFreqHz, boolean pInvertRange)
  {
    super();
    ...
    final Synthesizer lSynthesizer = new Synthesizer(lLowPassFilter, mSoundOut);
        ...
        try
        {
          ...
          while (!mStopSignal)
          {
            ...
            while ((lTimeNow = System.nanoTime()) < mNewDeadline)
            {
              ...
              {
                if (mReentrantLock.isHeldByCurrentThread()) mReentrantLock.unlock();
              }
              Thread.yield();
            }
  ...
  public void start()
  {
    synchronized (mStartStopLock)
    {
      ...
      mThread = new Thread(mDeamonThreadRunnable, AudioPlot.class.getSimpleName() + ".PlayThread");
  ...
  {
    while (!mThreadStoppedSignal) try
    {
      Thread.sleep(10);
   ...
   * [0,1].
   *
   * @param pValue size within range [0,1]
   */
  public void setValue(double pValue)
  {
    pValue = max(min(pValue, 1), 0);
    if (Double.isNaN(pValue) || Double.isInfinite(pValue)) pValue = 0;

    if (isInvertRange()) pValue = 1 - pValue;
   ...
   *
   * @param pInvertRange true to invert range
   */
