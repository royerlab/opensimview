 ...
 */
public class ShepardRissetGlissando extends SourceBase implements Source
{
   ...
   *
   * @param pBottomFrequency  bottom frequency of the support.
   * @param pTopFrequency     top frequency of the support.
   * @param pFrequencySpacing frequency spacing.
   */
  public ShepardRissetGlissando(float pBottomFrequency, float pTopFrequency, float pFrequencySpacing)
  {
    super();
   ...
   *
   * @param pDeltaFrequencyHertz change in frequency.
   */
  ...
  {
    if (pFrequency < mBottomFrequency || pFrequency > mTopFrequency) return 0;
    float lNormalizedFrequency = (pFrequency - mBottomFrequency) / (mTopFrequency - mBottomFrequency);
    float lAmplitude = (float) (getAmplitude() * pow(sin(PI * lNormalizedFrequency), 2));
    return lAmplitude;
  }
