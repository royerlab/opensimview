...
public class ReverbFilter extends FilterBase
{
   ...
   *
   * @param pAmplitude             reverb amplitude
   * @param pReverbPeriodInSeconds reverb period in seconds
   */
  ...
  public void setDefaulKernel()
  {
    ...
    for (int r = 0; r < mReverbBuffer.length / 4; r++)
    {
      ...
      for (int j = 1; j < mReverbBuffer.length - 1; j++)
        mReverbBuffer[j] = 0.99f * (mReverbBuffer[j - 1] + 2 * mReverbBuffer[j] + mReverbBuffer[j + 1]) / 4;/**/
   ...
   *
   * @param pReverbKernel reverb kernel
   */
