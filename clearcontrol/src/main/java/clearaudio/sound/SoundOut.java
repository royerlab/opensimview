...
public class SoundOut implements AutoCloseable
{
   ...
   * @param pSampleSizeInBits sample size in bits
   * @param pNumberOfChannels numebr of channels
   */
  public SoundOut(float pSampleRate, int pSampleSizeInBits, int pNumberOfChannels)
  {
    super();
    mAudioFormat = new AudioFormat(pSampleRate, pSampleSizeInBits, pNumberOfChannels, true, false);
  }

  /**
   * Starts to process incoming samples.
   *
   * @throws LineUnavailableException thrown if line unavailable
   */
   ...
   *
   * @param pBuffer double buffer
   * @param pLength length of buffer to use
   */
   ...
   * @param pBuffer        double buffer
   * @param pLength        length of buffer to use
   * @param pReentrantLock lock to release
   */
  public void play(final double[] pBuffer, final int pLength, ReentrantLock pReentrantLock)
  {
   ...
   * @param pReentrantLock lock to release
   */
  public void play(final float[] pBuffer, final int pLength, ReentrantLock pReentrantLock)
  {
    mTemporaryBuffer.reset();
    for (int i = 0; i < pBuffer.length; i++)
    {
  ...
  public void play(final byte[] pBuffer, final int pLength, ReentrantLock pReentrantLock)
  {
    int lLength;
    ...
    if (pReentrantLock != null && pReentrantLock.isHeldByCurrentThread()) pReentrantLock.unlock();
    synchronized (mLock)
    {
