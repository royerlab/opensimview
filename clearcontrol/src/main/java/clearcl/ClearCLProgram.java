...
public class ClearCLProgram implements AutoCloseable
{
   ...
   */
  public void addSource(Class<?> pClassForRessource, String pIncludeRessourceName) throws IOException
  {
    StringBuilder lStringBuilder = new StringBuilder();

    lStringBuilder.append("\n\n");
    lStringBuilder.append(" //###########################################################################\n");
    lStringBuilder.append("// Source: '" + pIncludeRessourceName + "' relative to " + pClassForRessource.getSimpleName() + "\n");
    ...
    if (lResourceAsStream == null)
    {
      ...
      String lAbsoluteKernelFilename = ("/" + String.join("/", lCutPath) + "/" + pIncludeRessourceName).replace("/./", "/");
    ...
    if (lResourceAsStream == null)
    {
      String lMessage = String.format("Cannot find source: [%s] %s", pClassForRessource.getSimpleName().toString(), pIncludeRessourceName);
  ...
  public void addDefine(String pKey, Number pValue)
  {
    ...
    else if (pValue instanceof Float) mDefinesMap.put(pKey, String.format(Locale.US, "%ef", pValue.floatValue()));
    else if (pValue instanceof Double) mDefinesMap.put(pKey, String.format(Locale.US, "%ef", pValue.doubleValue()));
  ...
  public BuildStatus build() throws IOException
  {
    ...
    if (!mModified) return getBuildStatus();
    ...
    ClearCLPeerPointer lProgramPeerPointer = mContext.getBackend().getProgramPeerPointer(mContext.getPeerPointer(), mLastBuiltSourceCode);

    String lOptions = concatenateOptions();
    mContext.getBackend().buildProgram(lProgramPeerPointer, lOptions);

    mCompiledProgram = new ClearCLCompiledProgram(mDevice, mContext, this, mLastBuiltSourceCode, lProgramPeerPointer);
  ...
  {
    InputStream lResourceAsStream = OCLlib.class.getResourceAsStream("preamble/preamble.cl");

    if (lResourceAsStream == null)
    {
      String lMessage = String.format("Cannot find preamble file at 'preamble/preamble.cl'");
      throw new IOException(lMessage);
    }
  ...
  private String insertDefines(String pSourceCode)
  {
    ...
    for (Map.Entry<String, String> lDefinesEntry : mDefinesMap.entrySet())
    {
      ...
      if (lDefinesEntry.getValue().length() == 0) lStringBuilder.append("#define " + lDefinesEntry.getKey() + "\n");
      else lStringBuilder.append("#define " + lDefinesEntry.getKey() + " \t" + lDefinesEntry.getValue() + "\n");
    }
    lStringBuilder.append(pSourceCode);
  ...
  private String insertIncludes(String pSourceCode) throws IOException
  {
    ...
    while ((lIncludeIndex = pSourceCode.indexOf("#include", lBeginIndex)) >= 0)
    {
      ...
      String lClassName = lClassNameBegin == -1 ? "" : lIncludeLine.substring(lClassNameBegin + 1, lClassNameEnd).trim();

      int lIncludeNameBegin = lIncludeLine.indexOf('"');
      int lIncludeNameEnd = lIncludeLine.indexOf('"', lIncludeNameBegin + 1);

      String lIncludeName = lIncludeLine.substring(lIncludeNameBegin + 1, lIncludeNameEnd).trim();

      Class<?> lReferenceClass = Object.class;
      if (lClassName.isEmpty()) lIncludeName = "/" + lIncludeName;
      else lReferenceClass = findClassByName(lClassName);
  ...
  {
    if (mCompiledProgram == null || mModified) throw new ClearCLProgramNotBuiltException();

    BuildStatus lBuildStatus = mContext.getBackend().getBuildStatus(getDevice().getPeerPointer(), mCompiledProgram.getPeerPointer());
    return lBuildStatus;
  }
  ...
  public String getBuildLog()
  {
    ...
    String lBuildLog = mContext.getBackend().getBuildLog(getDevice().getPeerPointer(), mCompiledProgram.getPeerPointer()).trim();
    return lBuildLog;
  }
  ...
  {
    return String.format("ClearCLProgram [mOptions=%s, mDefinesMap=%s, lines of code:%d]", mBuildOptionsList.toString(), mDefinesMap.toString(), getNumberLinesOfCode());
  }
