...
public class FastestDeviceSelector implements DeviceSelector
{
  ...
  public void init(ArrayList<ClearCLDevice> pDevices)
  {
    try
    {
      ...
      File lFastestDeviceFile = new File(lClearCLFolder, String.format("fastestdevice_%s.txt", mBenchmarkTest));

      if (lFastestDeviceFile.exists())
      {
        String lFastestDeviceName = StringUtils.readFileToString(lFastestDeviceFile).trim();
        mFastestDevice = getDeviceWithName(pDevices, lFastestDeviceName);
      } else
      {
        mFastestDevice = Benchmark.getFastestDevice(pDevices, mBenchmarkTest);
        if (mFastestDevice != null) StringUtils.writeStringToFile(lFastestDeviceFile, mFastestDevice.getName());
      }
    } catch (Throwable e)
    {
      e.printStackTrace();
  ...
  private ClearCLDevice getDeviceWithName(ArrayList<ClearCLDevice> pDevices, String pDeviceName)
  {
    for (ClearCLDevice lDevice : pDevices)
    {
      if (lDevice.getName().trim().toLowerCase().equals(pDeviceName.toLowerCase())) return lDevice;
    }
    return null;
  }
  ...
  public boolean selected(ClearCLDevice pClearCLDevice)
  {
    ...
    if (mFastestDevice == null) return true;
