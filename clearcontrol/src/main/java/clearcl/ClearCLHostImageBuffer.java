...
public class ClearCLHostImageBuffer extends ClearCLMemBase implements ClearCLMemInterface, ClearCLImageInterface
{
   ...
   *
   * @param pClearCLImage image template
   * @return newly allocated image.
   */
  public static ClearCLHostImageBuffer allocateSameAs(ClearCLImageInterface pClearCLImage)
  {
    ClearCLHostImageBuffer lClearCLHostImage = new ClearCLHostImageBuffer(pClearCLImage.getContext(), allocate(pClearCLImage.getSizeInBytes()), pClearCLImage.getNativeType(), pClearCLImage.getNumberOfChannels(), pClearCLImage.getDimensions());
  ...
  {
    return OffHeapMemory.allocatePageAlignedBytes("ClearCLHostImageBuffer", pSizeInBytes);
  }
   ...
   */
  public ClearCLHostImageBuffer(ClearCLContext pClearCLContext, NativeTypeEnum pNativeType, long pNumberOfChannels, long... pDimensions)
  {
    this(pClearCLContext, allocate(pNumberOfChannels * Size.of(pNativeType) * getVolume(pDimensions)), pNativeType, pNumberOfChannels, pDimensions);
   ...
   * @param pDimensions                dimensions
   */
  public ClearCLHostImageBuffer(ClearCLContext pClearCLContext, ContiguousMemoryInterface pContiguousMemoryInterface, NativeTypeEnum pNativeType, long pNumberOfChannels, long... pDimensions)
  {
    super(pClearCLContext.getBackend(), pClearCLContext.getBackend().wrap(pContiguousMemoryInterface), null, null, null);
  ...
  {
    return String.format("ClearCLHostImageBuffer [mContiguousMemory=%s, mNativeType=%s, mDimensions=%s, mNumberOfChannels=%s, getMemAllocMode()=%s, getKernelAccessType()=%s, getBackend()=%s, getPeerPointer()=%s]", mContiguousMemory, mNativeType, Arrays.toString(mDimensions), mNumberOfChannels, getMemAllocMode(), getKernelAccessType(), getBackend(), getPeerPointer());
  }
