...
public class ClearCLBackends
{
  ...
  public static final boolean isFunctionalBackend(ClearCLBackendInterface pClearCLBackend)
  {
    ...
    lFunctional &= Benchmark.getFastestDevice(pClearCLBackend, BenchmarkTest.Buffer, 2) != null;
  ...
  {
    ArrayList<ClearCLBackendInterface> lBackendList = getBackendList();

    for (ClearCLBackendInterface lClearCLBackend : lBackendList)
    {
      if (isFunctionalBackend(lClearCLBackend)) return lClearCLBackend;
    }
    return null;
  }
  ...
  public static final ClearCLBackendInterface getBestBackend()
  {
    ...
    if (lClearCLBackend == null) lClearCLBackend = new ClearCLBackendJOCL();

    println(" --> Using backend: " + lClearCLBackend.getClass().getSimpleName());
  ...
  {
    if (sStdOutVerbose) System.out.print(pString);
  }
