...
public class ClearCLImagePanel extends BorderPane
{
  ...
  private final BooleanProperty mIsActive = new SimpleBooleanProperty(true);
  ...
  private final ObjectProperty<RenderMode> mRenderMode = new SimpleObjectProperty<>(RenderMode.Slice);
  ...
  public ClearCLImagePanel(ClearCLImageInterface pClearCLImage)
  {
    ...
    try
    {
      mProgramFloat = lContext.createProgram(OCLlib.class, "render/img2D.cl", "render/ortho/avgproj3D.cl", "render/ortho/colproj3D.cl", "render/ortho/maxproj3D.cl", "render/ortho/slice3D.cl");

      mProgramFloat.addBuildOptionAllMathOpt();
      mProgramFloat.addDefine("FLOAT");
      mProgramFloat.buildAndLog();

      mProgramUint = lContext.createProgram(OCLlib.class, "render/img2D.cl", "render/ortho/avgproj3D.cl", "render/ortho/colproj3D.cl", "render/ortho/maxproj3D.cl", "render/ortho/slice3D.cl");

      mProgramUint.addBuildOptionAllMathOpt();
      mProgramUint.addDefine("UINT");
      mProgramUint.buildAndLog();

      mProgramInt = lContext.createProgram(OCLlib.class, "render/img2D.cl", "render/ortho/avgproj3D.cl", "render/ortho/colproj3D.cl", "render/ortho/maxproj3D.cl", "render/ortho/slice3D.cl");
    ...
    backgroundProperty().set(new Background(new BackgroundFill(Color.BLACK, CornerRadii.EMPTY, Insets.EMPTY)));/**/
    ...
    mNumberOfSteps.set((int) Math.min(mNumberOfSteps.get(), pClearCLImage.getDepth()));
  ...
  {
    if (mDirectWritableImage != null && (int) mDirectWritableImage.getWidth() == (int) pClearCLImage.getWidth() && (int) mDirectWritableImage.getHeight() == (int) pClearCLImage.getHeight())
      return;

    if (mDirectWritableImage != null) getChildren().remove(mDirectWritableImage);

    mDirectWritableImage = new DirectWritableImage((int) pClearCLImage.getWidth(), (int) pClearCLImage.getHeight());
  ...
  {
    if (mRenderRGBBuffer == null || mRenderRGBBuffer.getWidth() != pNewImage.getWidth() || mRenderRGBBuffer.getHeight() != pNewImage.getHeight())
    {
      if (mRenderRGBBuffer != null) mRenderRGBBuffer.close();

      mRenderRGBBuffer = pNewImage.getContext().createBuffer(MemAllocMode.AllocateHostPointer, HostAccessType.ReadOnly, KernelAccessType.WriteOnly, 4, NativeTypeEnum.Byte, Region2.region(pNewImage.getDimensions()));
    }

    if (mClearCLHostImage == null || mClearCLHostImage.getWidth() != pNewImage.getWidth() || mClearCLHostImage.getHeight() != pNewImage.getHeight())
    {
      if (mClearCLHostImage != null) mClearCLHostImage.close();
      ...
      try
      {
        ...
        if (mAuto.get() || mTrueMin == null)
        {
          ...
          else
          {
            ...
            mTrueMin = (1 - cSmoothingFactor) * lMinValue + cSmoothingFactor * mTrueMin;
            mTrueMax = (1 - cSmoothingFactor) * lMaxValue + cSmoothingFactor * mTrueMax;
        ...
        if (mClearCLImage.getDimension() == 2)
        {
          if (mClearCLImage instanceof ClearCLImage)
          {
            ...
            ImageChannelDataType lDataType = lImage.getChannelDataType();

            if (lDataType.isNormalized() || lDataType.isFloat())
              mRenderKernel = mProgramFloat.getKernel("image_render_2d");
            else if (lDataType.isInteger() && lDataType.isUnSigned())
              mRenderKernel = mProgramUint.getKernel("image_render_2d");
            else if (lDataType.isInteger() && lDataType.isSigned())
              mRenderKernel = mProgramInt.getKernel("image_render_2d");

          } else if (mClearCLImage instanceof ClearCLBuffer)
            mRenderKernel = mProgramFloat.getKernel("buffer_render_2df");
          ...
          switch (getRenderModeProperty().get())
          {
            case AvgProjection:
              if (mClearCLImage instanceof ClearCLImage)
              {
                ...
                if (lDataType.isNormalized() || lDataType.isFloat())
                  mRenderKernel = mProgramFloat.getKernel("image_render_avgproj_3d");
                else if (lDataType.isInteger() && lDataType.isUnSigned())
                  mRenderKernel = mProgramUint.getKernel("image_render_avgproj_3d");
                else if (lDataType.isInteger() && lDataType.isSigned())
                  mRenderKernel = mProgramInt.getKernel("image_render_avgproj_3d");

              } else if (mClearCLImage instanceof ClearCLBuffer)
                mRenderKernel = mProgramFloat.getKernel("buffer_render_avgproj_3df");
              break;
            case ColorProjection:
              if (mClearCLImage instanceof ClearCLImage)
              {
                ...
                if (lDataType.isNormalized() || lDataType.isFloat())
                  mRenderKernel = mProgramFloat.getKernel("image_render_colorproj_3d");
                else if (lDataType.isInteger() && lDataType.isUnSigned())
                  mRenderKernel = mProgramUint.getKernel("image_render_colorproj_3d");
                else if (lDataType.isInteger() && lDataType.isSigned())
                  mRenderKernel = mProgramInt.getKernel("image_render_colorproj_3d");

              } else if (mClearCLImage instanceof ClearCLBuffer)
                mRenderKernel = mProgramFloat.getKernel("buffer_render_colorproj_3df");
              break;
            case MaxProjection:
              if (mClearCLImage instanceof ClearCLImage)
              {
                ...
                if (lDataType.isNormalized() || lDataType.isFloat())
                  mRenderKernel = mProgramFloat.getKernel("image_render_maxproj_3d");
                else if (lDataType.isInteger() && lDataType.isUnSigned())
                  mRenderKernel = mProgramUint.getKernel("image_render_maxproj_3d");
                else if (lDataType.isInteger() && lDataType.isSigned())
                  mRenderKernel = mProgramInt.getKernel("image_render_maxproj_3d");

              } else if (mClearCLImage instanceof ClearCLBuffer)
                mRenderKernel = mProgramFloat.getKernel("buffer_render_maxproj_3df");
              break;
            default:
            case Slice:
              if (mClearCLImage instanceof ClearCLImage)
              {
                ...
                if (lDataType.isNormalized() || lDataType.isFloat())
                  mRenderKernel = mProgramFloat.getKernel("image_render_slice_3d");
                else if (lDataType.isInteger() && lDataType.isUnSigned())
                  mRenderKernel = mProgramUint.getKernel("image_render_slice_3d");
                else if (lDataType.isInteger() && lDataType.isSigned())
                  mRenderKernel = mProgramInt.getKernel("image_render_slice_3d");

              } else if (mClearCLImage instanceof ClearCLBuffer)
                mRenderKernel = mProgramFloat.getKernel("buffer_render_slice_3df");
              break;
          }
        ...
        final int lZStep = (int) (1.0 * mClearCLImage.getDepth() / mNumberOfSteps.get());
        mRenderKernel.setOptionalArgument("zstep", lZStep);

        mRenderKernel.run(true);

        ElapsedTime.measure("mRenderRGBBuffer.copyTo(mClearCLHostImage, true);", () -> mRenderRGBBuffer.copyTo(mClearCLHostImage, true));
        ...
        Platform.runLater(() ->
        {
          ...
          if (lTryLock2)
          {
            try
            {
              if (mClearCLImage.getWidth() != lWidth || mClearCLImage.getHeight() != lHeight) return;
