...
public class ClearCLImageViewer
{
   ...
   * @return image viewer
   */
  public static ClearCLImageViewer view(ClearCLImageInterface pImage, String pWindowTitle)
  {
    ClearCLImageViewer lViewImage = new ClearCLImageViewer(pImage, pWindowTitle);
    return lViewImage;
  }
   ...
   * @return image viewer
   */
  public static ClearCLImageViewer view(ClearCLImageInterface pImage, String pWindowTitle, int pWindowWidth, int pWindowHeight)
  {
    ClearCLImageViewer lViewImage = new ClearCLImageViewer(pImage, pWindowTitle, pWindowWidth, pWindowHeight);
    return lViewImage;
  }
   ...
   * @param pWindowTitle  window title
   */
  public ClearCLImageViewer(ClearCLImageInterface pClearCLImage, String pWindowTitle)
  {
    this(pClearCLImage, pWindowTitle, (int) (Math.max(pClearCLImage.getWidth(), 512)), (int) (Math.max(pClearCLImage.getHeight(), 512)));
  }
   ...
   * @param pWindowHeight window height
   */
  public ClearCLImageViewer(ClearCLImageInterface pClearCLImage, String pWindowTitle, int pWindowWidth, int pWindowHeight)
  {
    super();
    ...
    Platform.runLater(() ->
    {
      ...
      lStackPane.backgroundProperty().set(new Background(new BackgroundFill(Color.TRANSPARENT, CornerRadii.EMPTY, Insets.EMPTY)));/**/
      ...
      mControlPane.getRowConstraints().addAll(row1, row2, row3, row4, row5);/**/
      ...
      mControlPane.getColumnConstraints().addAll(col1, col2, col3, col4, col5, col6, col7);/**/

      mControlPane.backgroundProperty().set(new Background(new BackgroundFill(Color.TRANSPARENT, CornerRadii.EMPTY, Insets.EMPTY)));/**/
      ...
      if (pClearCLImage.getDimension() == 3)
      {
        ...
        lRenderModeComboBox.valueProperty().bindBidirectional(mImagePanel.getRenderModeProperty());
      ...
      lAutomaticMinMaxToggleButton.selectedProperty().bindBidirectional(mImagePanel.getAutoProperty());
      GridPane.setColumnSpan(lAutomaticMinMaxToggleButton, 3);
      GridPane.setHalignment(lAutomaticMinMaxToggleButton, HPos.CENTER);
      ...
      lAutomaticMinMaxToggleButton.selectedProperty().bindBidirectional(lMin.disableProperty());
      lMin.valueProperty().bindBidirectional(mImagePanel.getMinProperty());
      ...
      lAutomaticMinMaxToggleButton.selectedProperty().bindBidirectional(lMax.disableProperty());
      lMax.valueProperty().bindBidirectional(mImagePanel.getMaxProperty());
      ...
      {
        mImagePanel.getGammaProperty().set((float) (Math.pow(2, 2 * (-lGamma.getValue()))));
      });
      ...
      if (pClearCLImage.getDimension() == 3)
      {
        ...
        getZSlider().valueProperty().bindBidirectional(mImagePanel.getZProperty());
        ...
        {
          RenderMode lRenderMode = mImagePanel.getRenderModeProperty().get();
          getZSlider().setDisable(lRenderMode != RenderMode.Slice);
        });
        getZSlider().setDisable(mImagePanel.getRenderModeProperty().get() != RenderMode.Slice);
      ...
      PanZoomScene lPanZoomScene = new PanZoomScene(mStage, lStackPane, mImagePanel, pWindowWidth, pWindowHeight, Color.BLACK);

      double lInitialScale = 0.90 * Math.pow(Math.min(((double) pWindowWidth) / pClearCLImage.getWidth(), ((double) pWindowHeight) / pClearCLImage.getHeight()), 0.9);
  ...
  {
    Platform.runLater(() -> mImagePanel.getRenderModeProperty().set(pRenderMode));
