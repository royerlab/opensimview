 ...
 * @author royer
 */
public class ClearCLKernel extends ClearCLBase implements Runnable, Cleanable
{
  ...
  private final ConcurrentHashMap<Integer, Boolean> mUpdatedArgumentsMap = new ConcurrentHashMap<>();

  private volatile boolean mArgumentCaching = true;

  private long[] mGlobalOffsets = new long[]{0, 0, 0};
   ...
   * @param pClearCLCompiledProgram program
   * @param pKernelPointer          kernel peer pointer
   * @param pKernelName             kernel name
   * @param pSourceCode
   */
  ClearCLKernel(final ClearCLContext pClearCLContext, final ClearCLCompiledProgram pClearCLCompiledProgram, final ClearCLPeerPointer pKernelPointer, final String pKernelName, final String pSourceCode)
  {
   ...
   *
   * @param pGlobalOffsets new global offsets
   */
   ...
   *
   * @param pArgumentCaching true for cacging, false otherwise
   */
  ...
  public void setArgument(final int pIndex, final Object pObject)
  {
    ...
    {
      Argument lExistingArgumentValue = mIndexToArgumentMap.get(pIndex);

      if (lExistingArgumentValue == null || lExistingArgumentValue.argument == null || lExistingArgumentValue.argument != pObject || !lExistingArgumentValue.argument.equals(pObject))
      {
   ...
   *
   * @param pArguments list of arguments
   */
   ...
   * @param pObject       argument
   */
  public void setArgument(final String pArgumentName, final Object pObject)
  {
    final Integer lArgumentIndex = mNameToIndexMap.get(pArgumentName);

    if (lArgumentIndex == null) throw new ClearCLUnknownArgumentNameException(this, pArgumentName, pObject);
  ...
  public Object getArgument(final String pArgumentName)
  {
    ...
    if (lArgumentIndex == null) throw new ClearCLUnknownArgumentNameException(this, pArgumentName, null);
   ...
   *
   * @param pArgumentName     argument name
   * @param pNativeTypeEnum   native type for elements
   * @param pNumberOfElements number of elements
   */
  public void setLocalMemoryArgument(final String pArgumentName, final NativeTypeEnum pNativeTypeEnum, final long pNumberOfElements)
  {
    setArgument(pArgumentName, new ClearCLLocalMemory(pNativeTypeEnum, pNumberOfElements));
  }
   ...
   */
  public boolean setOptionalArgument(final String pArgumentName, final Object pObject)
  {
    final Integer lArgumentIndex = mNameToIndexMap.get(pArgumentName);

    if (lArgumentIndex == null) return false;
  ...
  private void setArgumentsInternal()
  {
    ...
    for (final Map.Entry<String, Integer> lEntry : mNameToIndexMap.entrySet())
    {
      ...
      if (!isArgumentCaching() || mUpdatedArgumentsMap.get(lArgumentIndex) == null || mUpdatedArgumentsMap.get(lArgumentIndex))
        try
        {
          ...
          if (lArgument == null)
          {
            ...
            if (lDefaultValue != null) lArgument = new Argument(lDefaultValue);
          }

          if (lArgument == null) throw new ClearCLArgumentMissingException(this, lArgumentName, lArgumentIndex);
   ...
   *
   * @param pWaitToFinish if true the call is blocking, false otherwise
   */
   ...
   * @param pWaitToFinish if true the call is blocking, false otherwise
   */
  public void run(final ClearCLQueue pClearCLQueue, final boolean pWaitToFinish)
  {
    ...
    ElapsedTime.measure(isLogExecutionTime(), "kernel " + getName(), () ->
    {
      ...
      getBackend().enqueueKernelExecution(pClearCLQueue.getPeerPointer(), getPeerPointer(), getGlobalSizes().length, getGlobalOffsets(), getGlobalSizes(), getLocalSizes());

      if (pWaitToFinish) pClearCLQueue.waitToFinish();
  ...
  public String toString()
  {
    return String.format("ClearCLKernel [mName=%s, mClearCLCompiledProgram=%s]", mName, mClearCLCompiledProgram);
  }
  ...
  {
    final ConcurrentHashMap<String, Number> lNameToDefaultArgumentMapMap = new ConcurrentHashMap<String, Number>();

    final String lSourceCode = getSourceCode();

    int lBeginOfDefault = 0;
    while ((lBeginOfDefault = lSourceCode.indexOf("//default " + pKernelName, lBeginOfDefault)) != -1)
    {
      final int lEndOfDefault = lSourceCode.indexOf('\n', lBeginOfDefault);
      final String lSubStringKernel = lSourceCode.substring(lBeginOfDefault, lEndOfDefault);

      // System.out.println(lSubStringKernel);

      final String[] lTwoPointsAndEqualSplit = lSubStringKernel.split("(\\s|=)+");
      // System.out.println(Arrays.toString(lTwoPointsAndEqualSplit));

      final String lArgumentName = lTwoPointsAndEqualSplit[2].trim().toLowerCase();
      String lArgumentValue = lTwoPointsAndEqualSplit[3].trim().toLowerCase();

      final char lArgumentType = lArgumentValue.charAt(lArgumentValue.length() - 1);

      lArgumentValue = lArgumentValue.substring(0, lArgumentValue.length() - 1);

      switch (lArgumentType)
      {
        case 'b':
          lNameToDefaultArgumentMapMap.put(lArgumentName, Byte.parseByte(lArgumentValue));
          break;

        case 's':
          lNameToDefaultArgumentMapMap.put(lArgumentName, Short.parseShort(lArgumentValue));
          break;

        case 'i':
          lNameToDefaultArgumentMapMap.put(lArgumentName, Integer.parseInt(lArgumentValue));
          break;

        case 'l':
          lNameToDefaultArgumentMapMap.put(lArgumentName, Long.parseLong(lArgumentValue));
          break;

        case 'f':
          lNameToDefaultArgumentMapMap.put(lArgumentName, Float.parseFloat(lArgumentValue));
          break;

        case 'd':
          lNameToDefaultArgumentMapMap.put(lArgumentName, Double.parseDouble(lArgumentValue));
          break;
      }
  ...
  {
    final ConcurrentHashMap<String, Integer> lNameToIndexMap = new ConcurrentHashMap<String, Integer>();
  ...
  private String[] getKernelSignature(final String pKernelName)
  {
    final String lSourceCode = getSourceCode();
    {
      ...
      if (lMatcher.find())
      {
        ...
        final int lEndOfKernelSignature = lSourceCode.indexOf('{', lBeginOfKernelSignature);
        final String lSubStringKernel = lSourceCode.substring(lBeginOfKernelSignature, lEndOfKernelSignature);

        final String lSubStringSignature = lSubStringKernel.substring(lSubStringKernel.indexOf('(') + 1, lSubStringKernel.indexOf(')'));

        // System.out.println("[[[" + lSubStringSignature + "]]]");

        final String[] lKernelSignature = lSubStringSignature.split(",", -1);
  ...
  public void close()
  {
    ...
    {
      if (mKernelCleaner != null) mKernelCleaner.mClearCLPeerPointer = null;
  ...
  private static class KernelCleaner implements Cleaner
  {
    ...
    public KernelCleaner(ClearCLBackendInterface pBackend, ClearCLPeerPointer pClearCLPeerPointer)
    {
      mBackend = pBackend;
    ...
    public void run()
    {
      try
      {
        ...
        {
          if (ClearCL.sDebugRGC) System.out.println("Releasing kernel: " + mClearCLPeerPointer.toString());
  ...
  {
    mKernelCleaner = new KernelCleaner(getBackend(), getPeerPointer());
