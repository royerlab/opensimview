 ...
 * @author royer
 */
public abstract class TaskDevice extends SignalStartStopDevice implements Runnable, StartStopSignalVariablesInterface, IsRunningTaskInterface, OpenCloseDeviceInterface, AsynchronousExecutorFeature, LoggingFeature
{
   ...
   *
   * @param pDeviceName     device name
   * @param pThreadPriority thread priority
   */
  public TaskDevice(final String pDeviceName, int pThreadPriority)
  {
    ...
    mIsRunningVariable = new Variable<Boolean>(pDeviceName + "IsRunning", false);

    mLastExceptionVariable = new Variable<Throwable>(pDeviceName + "LastException", null);

    ClearControlExecutors.getOrCreateThreadPoolExecutor(this, pThreadPriority, 1, 1, Integer.MAX_VALUE);
  }
  ...
  {
    if (mTaskFuture != null && !mTaskFuture.isDone()) return false;
  ...
  public boolean waitForStarted(long pTimeOut, TimeUnit pTimeUnit)
  {
    try
    {
      if (mStartedLatch != null) return mStartedLatch.await(pTimeOut, pTimeUnit);
  ...
  public boolean waitForStopped(int pTimeOut, TimeUnit pTimeUnit)
  {
    try
    {
      ...
      if (mStoppedLatch != null) lResult = mStoppedLatch.await(pTimeOut, pTimeUnit);
      else lResult = true;
