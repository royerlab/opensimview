 ...
 * @author royer
 */
public class VariableAsFile<O> extends Variable<O> implements AutoCloseable

{
  ...
  public VariableAsFile(final String pVariableName)
  {
    ...
    this(pVariableName, MachineConfiguration.get().getPersistentVariableFile(pVariableName), null);
  }
   ...
   *
   * @param pVariableName variable name
   * @param pFile         file o use
   * @param pValue        initial value
   */
  public VariableAsFile(final String pVariableName, final File pFile, final O pValue)
  {
  ...
  public O get()
  {
    if (mValue != null && mFile != null && mFile.exists() && mFile.lastModified() < mCachedReferenceFileSignature)
    {
      return mValue;
    }
  ...
  private O readFromFile() throws FileNotFoundException, IOException, ClassNotFoundException
  {
    O lReference = null;
    synchronized (mLock)
    {
      ...
      } finally
      {
        try
        {
          if (lObjectInputStream != null) lObjectInputStream.close();
  ...
  private void saveToFile(O pReference)
  {
    synchronized (mLock)
    {
      ...
      } finally
      {
        try
        {
          if (lObjectOutputStream != null) lObjectOutputStream.close();
        } catch (final IOException e)
        {
          e.printStackTrace();
        }
