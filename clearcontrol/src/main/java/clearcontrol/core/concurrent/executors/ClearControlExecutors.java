...
public class ClearControlExecutors
{
  ...
  private static ConcurrentHashMap<Object, CompletingThreadPoolExecutor> cThreadPoolExecutorMap = new ConcurrentHashMap<>(100);
  private static ConcurrentHashMap<Object, ScheduledThreadPoolExecutor> cScheduledThreadPoolExecutorMap = new ConcurrentHashMap<>(100);

  /**
   * Returns a completing thread pool executor given a key
   *
   * @param pObject key to use
  ...
  {
    final CompletingThreadPoolExecutor lExecutor = cThreadPoolExecutorMap.get(pObject);
  ...
  {
    final ScheduledThreadPoolExecutor lExecutor = cScheduledThreadPoolExecutorMap.get(pObject);
   ...
   * @param pMaxPoolSize    max pool size
   * @param pMaxQueueLength max queue length
   * @return scheduld thread pool executor
   */
  public static final CompletingThreadPoolExecutor getOrCreateThreadPoolExecutor(final Object pObject, final int pPriority, final int pCorePoolSize, final int pMaxPoolSize, final int pMaxQueueLength)
  {
    CompletingThreadPoolExecutor lThreadPoolExecutor = cThreadPoolExecutorMap.get(pObject);
    ...
    if (lThreadPoolExecutor == null)
    {
      ...
      lThreadPoolExecutor = new CompletingThreadPoolExecutor(pCorePoolSize, pMaxPoolSize, 1, TimeUnit.MINUTES, lNewQueue, getThreadFactory(lThreadName, pPriority));
   ...
   * @return scheduld thread pool executor
   */
  public static final ScheduledThreadPoolExecutor getOrCreateScheduledThreadPoolExecutor(final Object pObject, final int pPriority, final int pCorePoolSize, final int pMaxQueueLength)
  {

    ScheduledThreadPoolExecutor lScheduledThreadPoolExecutor = cScheduledThreadPoolExecutorMap.get(pObject);

    if (lScheduledThreadPoolExecutor == null)
    {
      String lThreadName = getThreadName(pObject);

      lScheduledThreadPoolExecutor = new ScheduledThreadPoolExecutor(pCorePoolSize, getThreadFactory(lThreadName, pPriority));
      ...
      cScheduledThreadPoolExecutorMap.put(pObject, lScheduledThreadPoolExecutor);
    }
  ...
  private static String getThreadName(final Object pObject)
  {
    ...
    if (lName.isEmpty()) lName = pObject.getClass().getName();
    return lName;
  }
   ...
   *
   * @param pName     name
   * @param pPriority thread priority
   * @return thread factory
   */
  public static final ThreadFactory getThreadFactory(final String pName, final int pPriority)
  {
    if (pName.isEmpty()) throw new IllegalArgumentException("Thread name cannot be empty");
