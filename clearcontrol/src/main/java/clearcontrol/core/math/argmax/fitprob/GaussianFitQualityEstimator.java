...
public class GaussianFitQualityEstimator
{
  ...
  {
    this(Executors.newFixedThreadPool(max(1, Runtime.getRuntime().availableProcessors() / 2)));
  }
  ...
  private File getFile(int pLength)
  {
    ...
    if (!lStatsFolder.exists()) lStatsFolder.mkdirs();
    final File lFile = new File(lStatsFolder, String.format("%s_l=%d.obj", this.getClass().getSimpleName(), pLength));

    return lFile;
  }

  private void writeToFile(NormalDistribution pNormalDistribution, File pFile) throws IOException
  {
    final FileOutputStream lFileOutputStream = new FileOutputStream(pFile);
    final BufferedOutputStream lBufferedOutputStream = new BufferedOutputStream(lFileOutputStream, cBufferLength);
  ...
  private NormalDistribution readFromFile(File pFile) throws IOException, ClassNotFoundException
  {
    final FileInputStream lFileInputStream = new FileInputStream(pFile);
    final BufferedInputStream lBufferedInputStream = new BufferedInputStream(lFileInputStream, cBufferLength);
  ...
  private NormalDistribution computeNullHypothesisDistribution(int lLength)
  {
    ...
    for (final FutureTask<Double> lFutureTask : lTaskList)
    {
      try
      {
        final Double lRMSD = lFutureTask.get(200, TimeUnit.MILLISECONDS);
        if (lRMSD != null) lIRMSDList.add(lRMSD);
      } catch (final Throwable e)
      {
      }
    }
    ...
    final NormalDistribution lNormalDistribution = new NormalDistribution(lCenterValue, lStandardDeviation);
    return lNormalDistribution;
  ...
  public Double probability(double[] pX, double[] pY)
  {
    ...
    final RandomizedDataGaussianFitter lDataGaussianFitter = new RandomizedDataGaussianFitter(pX, lNormY);
    try
    {
      mRealDataRMSD = lDataGaussianFitter.computeRMSD();

      if (mRealDataRMSD == null) return 0.0;
  ...
  public double[] getFit(double[] pX, double[] pY)
  {
    ...
    {
      lFittedY[i] = lMin + (lMax - lMin) * (mUnivariateDifferentiableFunction == null ? 0 : mUnivariateDifferentiableFunction.value(pX[i]));
    }
    return lFittedY;
  }
