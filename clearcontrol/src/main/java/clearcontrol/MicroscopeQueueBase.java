 ...
 * @author royer
 */
public class MicroscopeQueueBase<M extends MicroscopeBase<M, Q>, Q extends MicroscopeQueueBase<M, Q>> implements QueueInterface
{
  ...
  HashMap<QueueDeviceInterface<?>, QueueInterface> mDeviceToQueueMap = new HashMap<>();
  ...
  public MicroscopeQueueBase(M pMicroscope)
  {
    ...
    @SuppressWarnings("rawtypes") ArrayList<QueueDeviceInterface> lQueueableDevices = mMicroscope.getDeviceLists().getDevices(QueueDeviceInterface.class);

    for (QueueDeviceInterface<?> lQueueableDevice : lQueueableDevices)
    {
      QueueInterface lRequestedQueue = lQueueableDevice.requestQueue();
   ...
   *
   * @param pCameraIndex device index
   * @return queue
   */
   ...
   * @return queue
   */
  public QueueInterface getDeviceQueue(Class<?> pClass, int pDeviceIndex)
  {
    Object lDevice = getMicroscope().getDevice(pClass, pDeviceIndex);
    if (lDevice == null) throw new IllegalArgumentException("Device not found for class: " + pClass.getSimpleName());

    if (!(lDevice instanceof QueueDeviceInterface))
      throw new IllegalArgumentException("Should be an instance of " + QueueDeviceInterface.class.getSimpleName());
    @SuppressWarnings("rawtypes") QueueDeviceInterface<?> lQueueableDevice = (QueueDeviceInterface) lDevice;
  ...
  public void clearQueue()
  {
    ...
    for (final Entry<QueueDeviceInterface<?>, QueueInterface> lEntry : mDeviceToQueueMap.entrySet())
    {
      ...
      if (lDevice instanceof QueueDeviceInterface) if (mMicroscope.isActiveDevice(lDevice))
      {
        lQueue.clearQueue();
      }
    }
