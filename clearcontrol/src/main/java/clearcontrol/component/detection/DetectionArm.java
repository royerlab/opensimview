 ...
 * @author royer
 */
public class DetectionArm extends QueueableVirtualDevice<DetectionArmQueue> implements DetectionArmInterface, LoggingFeature
{

  private final Variable<UnivariateAffineFunction> mZFunction = new Variable<>("DetectionZFunction", new UnivariateAffineFunction());

  private final Variable<Double> mPixelSizeInMicrometerVariable = new Variable<>("PixelSizeInMicrometers", MachineConfiguration.get().getDoubleProperty("device.lsm.detection." + getName() + ".pixelsize", 0.406));
  ...
  {
    MachineConfiguration.get().getBoundsForVariable("device.lsm.detection." + getName() + ".z.bounds", getZVariable(), -200, 200);
  }
  ...
  public void resetFunctions()
  {
    ...
    mZFunction.set(MachineConfiguration.get().getUnivariateAffineFunction("device.lsm.detection." + getName() + ".z.f"));
