...
public class VariableRangeSlider<T extends Number> extends HBox
{
   ...
   * @param pLowVar      low value
   * @param pHighVar     high value
   * @param pMin         min value
   * @param pMax         max value
   * @param pGranularity granularity
   * @param pTicks       ticks
   */
  public VariableRangeSlider(String pLabelText, Variable<T> pLowVar, Variable<T> pHighVar, T pMin, T pMax, T pGranularity, T pTicks)
  {
    this(pLabelText, pLowVar, pHighVar, new Variable<T>("min", pMin), new Variable<T>("max", pMax), new Variable<T>("granularity", pGranularity), pTicks);
   ...
   * @param pLowVar      low value
   * @param pHighVar     high value
   * @param pMinVar      min value
   * @param pMaxVar      max value
   * @param pGranularity granularity
   * @param pTicks       ticks
   */
  public VariableRangeSlider(String pLabelText, Variable<T> pLowVar, Variable<T> pHighVar, Variable<T> pMinVar, Variable<T> pMaxVar, T pGranularity, T pTicks)
  {
    this(pLabelText, pLowVar, pHighVar, pMinVar, pMaxVar, new Variable<T>("granularity", pGranularity), pTicks);
   ...
   * @param pMinVar         min value
   * @param pMaxVar         max value
   * @param pGranularityVar granularity
   * @param pTicks          ticks
   */
  public VariableRangeSlider(String pLabelText, Variable<T> pLowVar, Variable<T> pHighVar, Variable<T> pMinVar, Variable<T> pMaxVar, Variable<T> pGranularityVar, T pTicks)
  {
    super();
    mLow = pLowVar;
    mHigh = pHighVar;
    ...
    mRangeSlider = new RangeSlider(mMin.get().doubleValue(), mMax.get().doubleValue(), mLow.get().doubleValue(), mHigh.get().doubleValue());
    ...
    if (pGranularityVar != null && pGranularityVar.get() != null)
      getRangeSlider().setBlockIncrement(pGranularityVar.get().doubleValue());

    pMinVar.addSetListener((o, n) ->
    {
      if (!pMinVar.get().equals(n) && n != null) Platform.runLater(() ->
      {
        updateSliderMinMax(pMinVar, pMaxVar, mTicks);
      });
    });
    ...
    {
      if (!pMaxVar.get().equals(n) && n != null) Platform.runLater(() ->
      {
        updateSliderMinMax(pMinVar, pMaxVar, mTicks);
      });
    });
    ...
    {
      if (!o.equals(n)) setUpdatedLowTextField();
    });

    getHighTextField().textProperty().addListener((obs, o, n) ->
    {
      if (!o.equals(n)) setUpdatedHighTextField();
    });
    ...
    getHighTextField().focusedProperty().addListener((obs, o, n) ->
    {
      if (!n)
      {
        ...
        setVariableValue(mHigh, false, getRangeSlider().getHighValue());
      }
    });
    ...
    if (pMinVar.get() instanceof Double || pMinVar.get() instanceof Float)
    {
      setLowTextFieldDouble(mLow.get());
      setHighTextFieldDouble(mHigh.get());
    }
    if (pMinVar.get() instanceof Integer || pMinVar.get() instanceof Long)
    {
      setLowTextFieldLongValue(mLow.get());
      setHighTextFieldLongValue(mHigh.get());
    }

    getRangeSlider().lowValueChangingProperty().addListener((obs, o, n) ->
    {
      if (isUpdateIfChanging()) return;
      if (o == true && n == false) setVariableValue(mLow, true, getRangeSlider().getLowValue());
    });

    getRangeSlider().highValueChangingProperty().addListener((obs, o, n) ->
    {
      if (isUpdateIfChanging()) return;
      if (o == true && n == false) setVariableValue(mHigh, false, getRangeSlider().getHighValue());
    });
    ...
    mLow.addSetListener((o, n) ->
    {
      ...
      {
        if (n.equals(getRangeSlider().getLowValue()) && n.equals(getLowTextFieldValue())) return;

        if (pMinVar.get() instanceof Double || pMinVar.get() instanceof Float) setLowTextFieldDouble(n);
        else setLowTextFieldLongValue(n);
    ...
    mHigh.addSetListener((o, n) ->
    {
      if (!n.equals(o) && n != null) Platform.runLater(() ->
      {
        if (n.equals(getRangeSlider().getHighValue()) && n.equals(getHighTextFieldValue())) return;

        if (pMinVar.get() instanceof Double || pMinVar.get() instanceof Float) setHighTextFieldDouble(n);
        else setHighTextFieldLongValue(n);
  ...
  {
    if (mMin.get() instanceof Double || mMin.get() instanceof Float) setLowTextFieldDouble(n);
    else if (mMin.get() instanceof Integer || mMin.get() instanceof Long) setLowTextFieldLongValue(n);
  }

  private void setHighTextField(Number n)
  {
    if (mMin.get() instanceof Double || mMin.get() instanceof Float) setHighTextFieldDouble(n);
    else if (mMin.get() instanceof Integer || mMin.get() instanceof Long) setHighTextFieldLongValue(n);
  }

  private void updateSliderMinMax(Variable<T> pMin, Variable<T> pMax, double pTicks)
  {
    double lTicksInterval = mTicks;
    if (lTicksInterval == 0)
    {
      double lRange = abs(pMax.get().doubleValue() - pMin.get().doubleValue());

      if (lRange > 1) lTicksInterval = max(1, ((int) lRange) / 100);
      else lTicksInterval = lRange / 100;
    }

    double lEffectiveSliderMin = lTicksInterval * Math.floor(pMin.get().doubleValue() / lTicksInterval);
    double lEffectiveSliderMax = lTicksInterval * Math.ceil(pMax.get().doubleValue() / lTicksInterval);

    double lRelativeAbsoluteDistance = 2 * (abs(lEffectiveSliderMin) - abs(lEffectiveSliderMax)) / (abs(lEffectiveSliderMin) + abs(lEffectiveSliderMax));

    if (lRelativeAbsoluteDistance < 0.1)
    {
      double lRadius = max(abs(lEffectiveSliderMin), abs(lEffectiveSliderMax));
    ...
    else getRangeSlider().setMin(lEffectiveSliderMin);

    if (Double.isInfinite(mMax.get().doubleValue()) || Double.isNaN(mMax.get().doubleValue()))
      getRangeSlider().setMax(10 * pTicks);
    else getRangeSlider().setMax(lEffectiveSliderMax);
  }

  @SuppressWarnings("unchecked")
  private void setVariableValue(Variable<T> pVariable, boolean pIsLow, Number pNewValue)
  {
    if (!pVariable.get().equals(pNewValue))
    {
      double lCorrectedValueDouble = pIsLow ? correctLowValueDouble(pNewValue.doubleValue()) : correctHighValueDouble(pNewValue.doubleValue());
      long lCorrectedValueLong = pIsLow ? correctLowValueLong(pNewValue.longValue()) : correctHighValueLong(pNewValue.longValue());
      if (mMin.get() instanceof Double) pVariable.set((T) new Double(lCorrectedValueDouble));
      if (mMin.get() instanceof Float) pVariable.set((T) new Float(lCorrectedValueDouble));
      if (mMin.get() instanceof Long) pVariable.set((T) new Long(lCorrectedValueLong));
      if (mMin.get() instanceof Integer) pVariable.set((T) new Integer((int) lCorrectedValueLong));
      if (mMin.get() instanceof Short) pVariable.set((T) new Short((short) lCorrectedValueLong));
      if (mMin.get() instanceof Byte) pVariable.set((T) new Byte((byte) lCorrectedValueLong));
    }
  }
  ...
  private double correctLowValueDouble(double pValue)
  {
    ...
    if (pValue > getRangeSlider().getHighValue()) return getRangeSlider().getHighValue();
  ...
  {
    if (pValue < getRangeSlider().getLowValue()) return getRangeSlider().getLowValue();
  ...
  private long correctLowValueLong(long pValue)
  {
    ...
    if (pValue > getRangeSlider().getHighValue()) return (long) getRangeSlider().getHighValue();
  ...
  {
    if (pValue < getRangeSlider().getLowValue()) return (long) getRangeSlider().getLowValue();
   ...
   *
   * @param pUpdateIfChanging true -> update while dragging
   */
