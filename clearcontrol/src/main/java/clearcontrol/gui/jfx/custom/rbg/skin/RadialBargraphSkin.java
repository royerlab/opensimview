...
public class RadialBargraphSkin extends SkinBase<RadialBargraph> implements Skin<RadialBargraph>
{
  ...
  private void initGraphics()
  {
    ...
    bar.setStroke(null);
    bar.setFill(new RadialGradient(0, 0, PREFERRED_WIDTH * 0.5, PREFERRED_HEIGHT * 0.5, PREFERRED_WIDTH * 0.45, false, CycleMethod.NO_CYCLE, new Stop(0.0, barColor), new Stop(0.76, barColor.deriveColor(-5, 1, 1, 1)), // -5
            // for
            // on
            // the
            // barColorHue)
            new Stop(0.79, barColor), new Stop(0.97, barColor), new Stop(1.0, barColor.deriveColor(-5, 1, 1, 1)))); // -5
    // for
    // on
    // the
    // barColorHue)
    ...
    // Add all nodes
    pane = new Pane();
    pane.getChildren().setAll(background, bar, ticksAndSectionsCanvas, minMeasuredValue, maxMeasuredValue, threshold, knob, title, unit, value, clickRegion);
  ...
  private void touchRotate(final double X, final double Y, final Rotate ROTATE)
  {
    ...
    double newValue = Double.compare(interactiveAngle, 180) <= 0 ? (interactiveAngle + 180.0 + getSkinnable().getStartAngle() - 360) / angleStep : (interactiveAngle - 180.0 + getSkinnable().getStartAngle() - 360) / angleStep;
  ...
  private void resize()
  {
    ...
    if (size > 0)
    {
      ...
      {
        bar.setFill(new RadialGradient(0, 0, centerX, centerY, RADIUS, false, CycleMethod.NO_CYCLE, new Stop(0.0, barColor), new Stop(0.76, barColor.deriveColor(-5, 1, 1, 1)), // -5
                // for
                // on
                // the
                // barColorHue)
