package clearcontrol.calibrator.test;

import clearcontrol.calibrator.CalibrationData;
import clearcontrol.calibrator.LightSheetPositioner;
...
public class CalibrationDataTests
{
   ...
   */
  @Test
  public void saveload() throws JsonGenerationException, JsonMappingException, IOException
  {
    ...
    File lFile = File.createTempFile(CalibrationDataTests.class.getSimpleName(), "saveload");
    ...
    assertEquals(1, lCalibrationDataRead.mPositionerMap.get("test").getTransformMatrix().get(0, 0), 0.001);

    assertEquals(0, lCalibrationDataRead.mPositionerMap.get("test").getTransformMatrix().get(1, 0), 0.001);
