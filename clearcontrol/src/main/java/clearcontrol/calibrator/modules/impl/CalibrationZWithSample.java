 ...
 */
public class CalibrationZWithSample extends CalibrationPerLightSheetBase implements CalibrationModuleInterface, HasStateDescriptionPerLightSheet
{
  ...
  public double calibrateZ(int pLightSheetIndex)
  {
    ...
    do
    {
      ...
      lError = calibrateZ(pLightSheetIndex, lIteration > 0, lSearchAmplitude, pLightSheetIndex == 0);
      info("############################################## Error = " + lError);
  ...
  private double calibrateZ(int pLightSheetIndex, boolean pRestrictedSearch, double pSearchAmplitude, boolean pAdjustDetectionZ)
  {
    ...
    InterpolatedAcquisitionState lInterpolatedAcquisitionState = (InterpolatedAcquisitionState) getLightSheetMicroscope().getAcquisitionStateManager().getCurrentState();

    double lMinDZ = lInterpolatedAcquisitionState.getControlPlaneZ(0); // Double.NEGATIVE_INFINITY;
    double lMaxDZ = lInterpolatedAcquisitionState.getControlPlaneZ(lInterpolatedAcquisitionState.getNumberOfControlPlanes() - 1); // Double.POSITIVE_INFINITY;
    ...
    for (double iz = lMinIZ; iz <= lMaxIZ; iz += lStepIZ)
    {
      ...
      getCalibrationEngine().configureChart(lChartName, lSeriesName, "DZ", "IZ", VisualConsoleInterface.ChartType.Line);
      ...
      if (lPlayQueueAndWait)
      {
        for (int d = 0; d < mNumberOfDetectionArmDevices; d++)
        {
          ...
          ElapsedTime.measureForceOutput("compute metric", () ->
          {
            ...
            } else mMetricArray = ImageAnalysisUtils.computeAverageSquareVariationPerPlane(lStack);/**/
          });
          // info("Begin compute metric");
          ...
          } else
          {
            ...
            getCalibrationEngine().configureChart(lChartName, lSeriesName, "ΔZ", "focus metric", VisualConsoleInterface.ChartType.Line);
