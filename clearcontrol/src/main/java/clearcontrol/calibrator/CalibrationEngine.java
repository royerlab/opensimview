 ...
 * @author royer
 */
public class CalibrationEngine extends TaskDevice implements LoggingFeature, VisualConsoleInterface
{
  ...
  private final Variable<Boolean> mCalibrateZVariable = new Variable<Boolean>("CalibrateZ", true);
  private final Variable<Boolean> mCalibrateZWithSampleVariable = new Variable<Boolean>("CalibrateZWithSample", false);

  private final Variable<Boolean> mCalibrateAVariable = new Variable<Boolean>("CalibrateA", false);
  private final Variable<Boolean> mCalibrateXYVariable = new Variable<Boolean>("CalibrateXY", false);

  private final Variable<Boolean> mCalibratePVariable = new Variable<Boolean>("CalibrateP", false);
  private final Variable<Boolean> mCalibrateWVariable = new Variable<Boolean>("CalibrateW", false);
  private final Variable<Boolean> mCalibrateWPVariable = new Variable<Boolean>("CalibrateWP", false);
  private final Variable<Boolean> mCalibrateHPVariable = new Variable<Boolean>("CalibrateHP", false);

  private final Variable<String> mCalibrationDataName = new Variable<String>("CalibrationName", "system");
  ...
  public CalibrationEngine(LightSheetMicroscope pLightSheetMicroscope)
  {
    ...
    mNumberOfDetectionArmDevices = mLightSheetMicroscope.getDeviceLists().getNumberOfDevices(DetectionArmInterface.class);

    mNumberOfLightSheetDevices = mLightSheetMicroscope.getDeviceLists().getNumberOfDevices(LightSheetInterface.class);
    ...
    {
      mCalibrateLightSheetOnOff[l] = new Variable<Boolean>("CalibrateLightSheet" + l, true);
    }
  ...
  public boolean calibrate()
  {
    ...
    if (getCalibrateZVariable().get() && !calibrateZ()) return false;

    if (isStopRequested()) return false;/**/

    if (getCalibrateZWithSampleVariable().get() && !calibrateZWithSample()) return false;

    if (isStopRequested()) return false;/**/

    if (getCalibrateAVariable().get() && !calibrateA()) return false;

    if (isStopRequested()) return false;/**/

    if (getCalibrateXYVariable().get() && !calibrateXY()) return false;

    if (isStopRequested()) return false;/**/

    if (getCalibratePVariable().get() && !calibrateP()) return false;

    if (isStopRequested()) return false;

    if (getCalibrateWVariable().get() && !calibrateW()) return false;

    if ((getCalibrateAVariable().get() || getCalibrateXYVariable().get()) && getCalibrateZVariable().get() && !calibrateZ())
      return false;

    if (isStopRequested()) return false;/**/

    if (getCalibratePVariable().get() && !calibrateP()) if (isStopRequested()) return false;/**/
  ...
  {
    return ScriptingEngine.isCancelRequestedStatic() || !isRunning() || getStopSignalVariable().get();
  }
  ...
  public void reset()
  {
    ...
    final int lNumberOfDetectionArmDevices = mLightSheetMicroscope.getDeviceLists().getNumberOfDevices(DetectionArmInterface.class);

    for (int i = 0; i < lNumberOfDetectionArmDevices; i++)
    {
      final DetectionArmInterface lDetectionArmDevice = mLightSheetMicroscope.getDeviceLists().getDevice(DetectionArmInterface.class, i);
      lDetectionArmDevice.resetFunctions();

    }

    final int lNumberOfLightSheetDevices = mLightSheetMicroscope.getDeviceLists().getNumberOfDevices(LightSheetInterface.class);

    for (int i = 0; i < lNumberOfLightSheetDevices; i++)
    {
      final LightSheetInterface lLightSheetDevice = mLightSheetMicroscope.getDeviceLists().getDevice(LightSheetInterface.class, i);
   ...
   */
  public void setPositioner(int pLightSheetIndex, int pDetectionArmIndex, LightSheetPositioner pLightSheetPositioner)
  {
    mPositionersMap.put("i" + pLightSheetIndex + "d" + pDetectionArmIndex, pLightSheetPositioner);
