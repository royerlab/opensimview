package clearcontrol.timelapse.timer;
...
public interface TimelapseTimerInterface
{
   ...
   *
   * @param pTimeNeeded   time needed
   * @param pReservedTime reserved time
   * @param pTimeUnit     time unit
   * @return true if there is enough time
   */

  public default boolean enoughTimeFor(long pTimeNeeded, long pReservedTime, TimeUnit pTimeUnit)
  {
    if (pTimeNeeded < 0) return false;

    long lTimeNeededInNS = TimeUnit.NANOSECONDS.convert(pTimeNeeded, pTimeUnit);
    long lReservedTimeInNS = TimeUnit.NANOSECONDS.convert(pReservedTime, pTimeUnit);
