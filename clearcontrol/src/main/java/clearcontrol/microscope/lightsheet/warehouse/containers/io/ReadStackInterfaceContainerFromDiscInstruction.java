...
public class ReadStackInterfaceContainerFromDiscInstruction extends LightSheetMicroscopeInstructionBase implements LoggingFeature, PropertyIOableInstructionInterface
{
  ...
  public ReadStackInterfaceContainerFromDiscInstruction(String[] pDatasetNames, LightSheetMicroscope pLightSheetMicroscope)
  {
    ...
    mRootFolderVariable = new Variable("RootFolder", new File(System.getProperty("user.home") + "/Desktop"));
  }
  ...
  public boolean enqueue(long pTimePoint)
  {
    ...
    if (mDatasetNames.length == 1 && (mDatasetNames[0].contains("fused") || mDatasetNames[0].contains("sequential") || mDatasetNames[0].contains("interleaved") || mDatasetNames[0].contains("opticsprefused") || mDatasetNames[0].contains("default")))
    {
  ...
  {
    return new Variable[]{getRestartFromBeginningWhenReachingEnd(), getRootFolderVariable(), getTimepointOffset(), getTimepointStepSize()};
  }
}
