 ...
 * @author royer
 */
public class CalibrationWP extends CalibrationPerLightSheetBase implements CalibrationModuleInterface
{
  ...
  public double calibrate(int pLightSheetIndex)
  {
    ...
    final double lReferenceIntensity = adjustP(pLightSheetIndex, lDetectionArmIndex, lReferencePower, lReferencePower, lNumberOfSamplesP, lReferenceW, 0, true);
    ...
    {
      final double lPower = adjustP(pLightSheetIndex, lDetectionArmIndex, lMinP, lMaxP, 10, w, lReferenceIntensity, false);
    ...
    String lChartName = String.format(" D=%d, I=%d, W=%g", lDetectionArmIndex, pLightSheetIndex);

    getCalibrationEngine().configureChart(lChartName, "samples", "IW", "intensity", ChartType.Line);

    getCalibrationEngine().configureChart(lChartName, "fit", "IW", "intensity", ChartType.Line);

    List<WeightedObservedPoint> lObservationsList = lObservations.toList();

    for (int j = 0; j < lWList.size(); j++)
    {
      getCalibrationEngine().addPoint(lChartName, "samples", j == 0, lWList.get(j), lPRList.get(j));

      getCalibrationEngine().addPoint(lChartName, "fit", j == 0, lWList.get(j), lObservationsList.get(j).getY());
  ...
  private Double adjustP(int pLightSheetIndex, int pDetectionArmIndex, double pMinP, double pMaxP, int pNumberOfSamples, double pW, double pTargetIntensity, boolean pReturnIntensity)
  {
    try
    {
      ...
      if (lPlayQueueAndWait)
      {
        ...
        String lChartName = String.format("Mode=%s, D=%d, I=%d, W=%g", pReturnIntensity ? "ret_int" : "ret_pow", pDetectionArmIndex, pLightSheetIndex, pW);

        getCalibrationEngine().configureChart(lChartName, "samples", "IP", "avg intensity", ChartType.Line);

        // System.out.format("metric array: \n");
        for (int j = 0; j < lAvgIntensityArray.length; j++)
        {
          getCalibrationEngine().addPoint(lChartName, "samples", j == 0, lPList.get(j), lAvgIntensityArray[j]);
  ...
  public double apply(int pLightSheetIndex, int pDetectionArmIndex)
  {
    ...
    PolynomialFunction lNewWidthPowerFunction = mWPFunctions.get(pLightSheetIndex, pDetectionArmIndex);
