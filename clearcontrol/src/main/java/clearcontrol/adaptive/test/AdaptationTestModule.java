 ...
 * @author royer
 */
public class AdaptationTestModule extends NDIteratorAdaptationModule<TestState> implements AdaptationModuleInterface<TestState>, LoggingFeature
{
   ...
   * @param pDimensions iterator dimensions
   */
  ...
  {
    info("!!!TESTMODULE %s - ACQUIRING DATA... !!: step: %s \n", getName(), Arrays.toString(pStepCoordinates));
    // here we would 'programStep' the data, takes a bit of time... tada...
    ThreadSleep.sleep(100 * pStepCoordinates.length, TimeUnit.MILLISECONDS);
