 ...
 * @author royer
 */
public interface StageDeviceInterface extends NameableInterface, OpenCloseDeviceInterface, WaitingInterface
{
  /**
   ...
   *
   * @param pIndex    DOF's index
   * @param pPosition DOF's new target position
   */
   ...
   */
  public default Boolean waitToBeReady(int pDOFIndex, long pTimeOut, TimeUnit pTimeUnit)
  {
    return waitFor(pTimeOut, pTimeUnit, () -> getReadyVariable(pDOFIndex).get());
  }
   ...
   */
  public default Boolean waitToBeReady(long pTimeOut, TimeUnit pTimeUnit)
  {
    int lNumberOfDOFs = getNumberOfDOFs();
    ...
    {
      for (int i = 0; i < lNumberOfDOFs; i++)
        if (!getReadyVariable(i).get()) return false;
      return true;
    };
   ...
   *
   * @param pIndex    DOF index
   * @param pEpsilon  epsilon (radius)
   * @param pTimeOut  timeout
   * @param pTimeUnit timeout unit
   * @return true of no timeout occurred
   */
  public default Boolean waitToArrive(int pIndex, double pEpsilon, long pTimeOut, TimeUnit pTimeUnit)
  {
   ...
   */
  public default Boolean waitToArrive(double pEpsilon, long pTimeOut, TimeUnit pTimeUnit)
  {
    int lNumberOfDOFs = getNumberOfDOFs();
