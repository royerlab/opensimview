...
import clearcontrol.devices.stages.devices.ecc100.variables.*;
 ...
 * @author royer
 */
public class ECC100StageDevice extends StageDeviceBase implements StageDeviceInterface, StartStopDeviceInterface, WaitingInterface, LoggingFeature
{
  ...
  public boolean open()
  {
    try
    {
      ...
      if (mECC100Controller.open())
      {
        ...
        if (lDeviceIdList.size() == 0) return false;
        ...
        for (final int lDeviceId : lDeviceIdList)
        {
          ...
          {
            final ECC100Axis lAxis = mECC100Controller.getAxis(lDeviceId, axis);

            final String lDeviceConfigString = "device.stage.ecc100." + lDeviceId + "." + axis;
        ...
        for (int dof = 0; dof < lNumberOfDofs; dof++)
        {
          ...
          mEnableVariables.add(new EnableVariable("Enable" + mIndexToNameMap.get(dof), lEcc100Axis));

          mReadyVariables.add(new ReadyVariable("Ready" + mIndexToNameMap.get(dof), lEcc100Axis));

          mHomingVariables.add(new HomingVariable("Homing" + mIndexToNameMap.get(dof), lEcc100Axis));

          mStopVariables.add(new StopVariable("Stop" + mIndexToNameMap.get(dof), lEcc100Axis));

          mResetVariables.add(new ResetVariable("Reset" + mIndexToNameMap.get(dof), lEcc100Axis));

          mTargetPositionVariables.add(new PositionTargetVariable("TargetPosition" + mIndexToNameMap.get(dof), lEcc100Axis));

          mCurrentPositionVariables.add(new PositionCurrentVariable("CurrentPosition" + mIndexToNameMap.get(dof), lEcc100Axis));

          mMinPositionVariables.add(new MinPositionVariable("MinPosition" + mIndexToNameMap.get(dof), lEcc100Axis));

          mMaxPositionVariables.add(new MaxPositionVariable("MaxPosition" + mIndexToNameMap.get(dof), lEcc100Axis));
  ...
  {
    return "ECC100StageDevice [mECC100Controller=" + mECC100Controller + ", getNumberOfDOFs()=" + getNumberOfDOFs() + ", getDeviceName()=" + getName() + "]";
  }
