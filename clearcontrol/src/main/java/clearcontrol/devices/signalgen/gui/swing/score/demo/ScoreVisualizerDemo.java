...
public class ScoreVisualizerDemo
{
  ...
  public void demo() throws InvocationTargetException, InterruptedException
  {
    ...
    SwingUtilities.invokeAndWait(new Runnable()
    {
      ...
      public void run()
      {
        ...
        lTestFrame.setLayout(new MigLayout("insets 0", "[grow,fill]", "[grow,fill]"));
    ...
    while (lTestFrame.isVisible()) Thread.sleep(10);
  }
  ...
  private ScoreInterface getTestScores(double pOmega)
  {
    ...
    for (int i = 0; i < 8; i++)
    {
      final RampContinuousStave lRampContinuousStave = new RampContinuousStave("i=" + i, 0f, (float) min(1, 100 * pOmega), 0f, 0.1f + i / 8f, 0);
    ...
    for (int i = 8; i < 16; i++)
    {
      final SinusStave lSinusStave = new SinusStave("i=" + i, (float) (pOmega * (1 + i)), (float) ((1f + i) / 16f * pOmega), 0.5f);
