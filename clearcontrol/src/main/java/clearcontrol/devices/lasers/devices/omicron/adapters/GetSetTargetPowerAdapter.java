 ...
 * @author royer
 */
public class GetSetTargetPowerAdapter extends OmicronAdapter<Number> implements SerialTextDeviceAdapter<Number>
{
  ...
  public Double parseValue(final byte[] pMessage)
  {
    ...
    final String[] lSplittedMessage = ProtocolOmicron.splitMessage(ProtocolOmicron.cGetPowerLevelReplyPrefix, pMessage);
    final String lSpecPowerString = lSplittedMessage[0];
    final int lCurrentPowerInBinaryUnits = Integer.parseInt(lSpecPowerString, 16);
  ...
  public byte[] getSetValueCommandMessage(final Number pOldPowerInMilliWatt, final Number pNewPowerInMilliWatt)
  {
    ...
    final String lHexPowerString = ProtocolOmicron.toHexadecimalString(lPower, 3);
    // System.out.format("SET: power %s (percent*(4096-1) HEX)
    // \n",lHexPowerString);
    final String lSetTargetPowerCommandString = String.format(ProtocolOmicron.cSetPowerLevelCommand, lHexPowerString);
