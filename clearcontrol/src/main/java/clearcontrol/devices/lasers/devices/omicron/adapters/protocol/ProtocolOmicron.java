...
public class ProtocolOmicron
{
   ...
   * @param pReceivedString string
   * @return double value
   */
  public static double parseDouble(String pPrefix, String pReceivedString)
  {
    int lIndex = pReceivedString.indexOf(pPrefix) + pPrefix.length();
    String lStringWithoutPrefix = pReceivedString.substring(lIndex).trim();
   ...
   *
   * @param pPrefix  prefix
   * @param pMessage message to split
   * @return array of strings
   */
  public static final String[] splitMessage(String pPrefix, final byte[] pMessage)
  {
    String lMessageString = new String(pMessage, StandardCharsets.ISO_8859_1);
   ...
   * @param k numbr of digits for the hexdecimal representation
   * @return hexadec string
   */
  public static String toHexadecimalString(final int n, final int k)
  {
    return String.format("%" + k + "s", Integer.toHexString(n)).replace(' ', '0').toUpperCase();
  }
  ...
  private static final boolean setNoAdHocModeInternal(final Serial pSerial, final int pMaxtries)
  {
    if (pMaxtries <= 0)
    {
      return false;
    ...
    try
    {
      ...
      final String[] lSplitMessage = splitMessage(cGetOperatingModeReplyPrefix, lReadTextMessage);
      final String lOperatingModeAsHexString = lSplitMessage[0];

      int lOperatingModeAsInteger = Integer.parseInt(lOperatingModeAsHexString, 16);

      lOperatingModeAsInteger = lOperatingModeAsInteger & ~cAdGocModeMask;

      final String lNewOperatingModeAsHexString = toHexadecimalString(lOperatingModeAsInteger, 4);
      final String lNewOperatingModeCommand = String.format(cSetOperatingModeCommand, lNewOperatingModeAsHexString);

      purge(pSerial);
      pSerial.write(lNewOperatingModeCommand.getBytes());
      final byte[] lReadTextMessage2 = pSerial.readTextMessage();

      lSuccess = new String(lReadTextMessage2, StandardCharsets.ISO_8859_1).startsWith("!SOM");

      if (lSuccess)
      {
        pSerial.info(ProtocolOmicron.class.getSimpleName() + ": Success setting non-AdHoc mode!");
      } else
      {
    ...
    {
      e.printStackTrace();
      pSerial.warning(ProtocolOmicron.class.getSimpleName() + ": Failed to set non-AdHoc mode, trying again...");
      purge(pSerial);
      lSuccess = setNoAdHocModeInternal(pSerial, pMaxtries - 1);
    }
