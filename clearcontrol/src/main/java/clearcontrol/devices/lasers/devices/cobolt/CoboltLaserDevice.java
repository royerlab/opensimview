 ...
 * @author royer
 */
public class CoboltLaserDevice extends LaserDeviceBase implements LaserDeviceInterface
{
   ...
   *
   * @param pCoboltModelName     Cobolt laser model name
   * @param pMaxPowerInMilliWatt max power in milliwatt
   * @param pDeviceIndex         device index.
   */
  public CoboltLaserDevice(final String pCoboltModelName, final int pMaxPowerInMilliWatt, final int pDeviceIndex)
  {
    this(pCoboltModelName, pMaxPowerInMilliWatt, MachineConfiguration.get().getSerialDevicePort("laser.cobolt", pDeviceIndex, "NULL"));
  }

  /**
   * Instanciates a Cobolt laser device.
   *
   * @param pCoboltModelName     Cobolt model name
   * @param pMaxPowerInMilliWatt max power in milliwatt
   * @param pPortName            port name
   */
  public CoboltLaserDevice(final String pCoboltModelName, final int pMaxPowerInMilliWatt, final String pPortName)
  {
    super("Cobolt" + pCoboltModelName);

    mSerialDevice = new SerialDevice("Cobolt" + pCoboltModelName, pPortName, 115200);

    mCoboltModel = CoboltDeviceEnum.valueOf(pCoboltModelName);
    mMaxPowerInMilliWatt = pMaxPowerInMilliWatt;

    mDeviceIdVariable = new Variable<Integer>("DeviceId", mCoboltModel.ordinal());

    mWavelengthVariable = new Variable<Integer>("WavelengthInNanoMeter", mCoboltModel.getWavelengthInNanoMeter());

    mSpecInMilliWattPowerVariable = new Variable<Number>("SpecPowerInMilliWatt", mMaxPowerInMilliWatt);

    mMaxPowerInMilliWattVariable = new Variable<Number>("MaxPowerInMilliWatt", mMaxPowerInMilliWatt);
    ...
    mTargetPowerInMilliWattVariable = mSerialDevice.addSerialVariable("TargetPowerMilliWatt", lGetSetTargetPowerAdapter);
  ...
  public boolean open()
  {
    ...
    mSerialDevice.getSerial().setLineTerminationCharacter(ProtocolCobolt.cMessageTerminationCharacter);
