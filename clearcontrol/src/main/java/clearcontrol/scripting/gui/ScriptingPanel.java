...
public class ScriptingPanel extends JPanel implements DropTargetListener, FileEventNotifierListener, DocumentListener, KeyListener
{
  ...
  public ScriptingPanel(String pName, ScriptingEngine pScriptingEngine, int pNumberOfRows, int pNumberOfCols)
  {
    this(pName, pScriptingEngine, null, pNumberOfRows, pNumberOfCols);
  }

  public ScriptingPanel(String pName, ScriptingEngine pScriptingEngine, File pDefaultFile, int pNumberOfRows, int pNumberOfCols)
  {
    super();
    mScriptingEngine = pScriptingEngine;

    String lFileName = String.format("/.rtlib/%s.scriptname.txt", pName).replaceAll("[^\\.\\/\\w]+", "_");
    mLastLoadedScriptFile = new File(System.getProperty("user.home"), lFileName);
    mLastLoadedScriptFile.getParentFile().mkdirs();

    new DropTarget(this, this);

    mRSyntaxTextArea = new RSyntaxTextArea(pNumberOfRows, pNumberOfCols);
    ...
    } else mRSyntaxTextArea.setSyntaxEditingStyle(SyntaxConstants.SYNTAX_STYLE_NONE);
    ...
    setLayout(new MigLayout("insets 0", "[84.00px,grow][49px,grow]", "[][][grow]"));

    if (pDefaultFile != null) mCurrentFileTextField = new JTextField(pDefaultFile.getAbsolutePath());
    else mCurrentFileTextField = new JTextField();
    ...
    lExecuteButtonAndConsolePanel.setLayout(new MigLayout("insets 0", "[606px,grow,fill][606px,grow,fill][606px,grow,fill]", "[29px][201px,grow,fill]"));

    final JButton lExecuteButton = new JButton("execute");
    lExecuteButtonAndConsolePanel.add(lExecuteButton, "cell 0 0,alignx left,aligny center");
    ...
    ((AbstractDocument) mConsoleTextArea.getDocument()).setDocumentFilter(new LineLimitedDocumentFilter(mConsoleTextArea, cMaxNumberOfLines));

    final JScrollPane lConsoleTextAreaScrollPane = new JScrollPane(mConsoleTextArea);
    lExecuteButtonAndConsolePanel.add(lConsoleTextAreaScrollPane, "cell 0 1 3 1,alignx center,aligny center");
    ...
    if (mScriptingEngine != null)
    {
      ...
      lExecuteButton.addActionListener((event) ->
      {
        try
        {
          saveToFile(mRSyntaxTextArea.getText(), mCurrentFileTextField.getText());

          if (mScriptingEngine.getScript() == null || !mScriptingEngine.getScript().equals(mRSyntaxTextArea.getText()))
      ...
      lConsoleClearButton.addActionListener((e) ->
      {
        try
        {
          mConsoleTextArea.getDocument().remove(0, mConsoleTextArea.getDocument().getLength());
  ...
  private void saveToFile(String pScriptText, String pFileName) throws IOException
  {
    try
    {
      if (mFileEventNotifier != null) mFileEventNotifier.setIgnore(true);
      if (pFileName == null || pFileName.isEmpty()) return;
      ...
      if (mFileEventNotifier != null) mFileEventNotifier.setIgnore(false);
    } catch (Throwable e)
    {
      e.printStackTrace();
    }
  ...
  public void loadFromFile(String pFileName) throws IOException, InvocationTargetException
  {
    if (pFileName == null || pFileName.isEmpty()) return;
    final File lFile = new File(pFileName);
    loadFromFile(lFile);
  ...
  public void loadFromFile(File pFile) throws IOException, InvocationTargetException
  {
    if (pFile == null || !pFile.exists()) return;
    ...
    try
    {
      if (mFileEventNotifier != null) mFileEventNotifier.close();
  ...
  public void loadText(String pText) throws InvocationTargetException
  {
    ...
    {
      if (mRSyntaxTextArea.getText() == null || !mRSyntaxTextArea.getText().equals(pText))
      {
  ...
  public void setUpdateCaret(boolean pUpdate)
  {
    ...
    caret.setUpdatePolicy(pUpdate ? DefaultCaret.ALWAYS_UPDATE : DefaultCaret.NEVER_UPDATE);
  }
  ...
  @Override
  public void fileEvent(FileEventNotifier pThis, File pFile, FileEventKind pEventKind)
  {
    try
    {
