...
public class GLUniform implements GLInterface
{
  ...
  public void setFloatMatrix(final float[] pProjectionMatrix, final boolean pTranspose)
  {
    mGlProgram.bind();
    mGlProgram.getGL().getGL3().glUniformMatrix4fv(mUniformId, 1, pTranspose, pProjectionMatrix, 0);
  }

  public void setFloatMatrix(final FloatBuffer pProjectionMatrix, final boolean pTranspose)
  {
    mGlProgram.bind();
    mGlProgram.getGL().getGL3().glUniformMatrix4fv(mUniformId, 1, pTranspose, pProjectionMatrix);
  }

  public void setFloatMatrix(final GLMatrix matrix, final boolean pTranspose)
  {
    mGlProgram.bind();
    mGlProgram.getGL().getGL3().glUniformMatrix4fv(mUniformId, 1, pTranspose, FloatBuffer.wrap(matrix.getFloatArray()));
  }
  ...
  {
    return "GLUniform [mGlProgram=" + mGlProgram + ", mUniformId=" + mUniformId + "]";
  }
