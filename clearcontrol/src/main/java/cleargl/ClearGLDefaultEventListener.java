...
public abstract class ClearGLDefaultEventListener implements ClearGLEventListener
{
  ...
  @Override
  public void reshape(final GLAutoDrawable pDrawable, final int pX, final int pY, final int pWidth, final int pHeight)
  {
    setDebugPipeline(pDrawable);
  ...
  {
    if (mAlreadyInDebugMode || !isDebugMode()) return;

    final GL lGL = pDrawable.getGL();
    lGL.getContext().setGL(GLPipelineFactory.create("com.jogamp.opengl.Debug", null, lGL, null));
