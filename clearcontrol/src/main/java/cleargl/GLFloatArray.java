...
public class GLFloatArray
{
  ...
  {
    return ByteBuffer.allocateDirect(pNewCapacity * mElementSize * (Float.SIZE / Byte.SIZE)).order(ByteOrder.nativeOrder()).asFloatBuffer();
  }

  private FloatBuffer allocateN(final int pNewCapacity)
  {
    return ByteBuffer.allocateDirect(pNewCapacity * (Float.SIZE / Byte.SIZE)).order(ByteOrder.nativeOrder()).asFloatBuffer();
  }
  ...
  {
    while (mFloatBuffer.hasRemaining()) mFloatBuffer.put(0);
  }
