...
public class GLVideoRecorder
{
   ...
   * files.
   *
   * @param pRootFolder folder in which video files will be saved.
   */
   ...
   *
   * @param pDisplayRequestRunnable Runnable describing how to request a display update.
   */
  public void startDisplayRequestDeamonThread(final Runnable pDisplayRequestRunnable)
  {
    if (pDisplayRequestRunnable != null)
    {
      ...
      final Thread lDisplayRequestDeamonThread = new Thread(lDisplayRequestRunnable, GLVideoRecorder.class.getSimpleName() + ".DisplayRequestThread");
   ...
   *
   * @param pTargetFrameRate target framerate in FPS.
   */
   ...
   * Sets root folder.
   *
   * @param pFolder root folder file.
   */
  ...
  public void toggleActive()
  {
    if (!mActive)
    {
      if (mFirstTime)
      {
        mRootFolder = FolderChooser.openFolderChooser(null, "Choose root folder to save videos", mRootFolder);
        mFirstTime = false;
      }

      while (getNewVideoFolder()) mVideoCounter++;
      mVideoFolder.mkdirs();
    ...
    } else
    {
      ...
      final JDialog lJDialog = new JDialog((JFrame) null, "Saving video", true);
      ...
      SwingUtilities.invokeLater(new Runnable()
      {
        ...
        public void run()
        {
          ...
          lJDialog.add(BorderLayout.NORTH, new JLabel("Saving images, please wait!"));
   ...
   *
   * @param pGLAutoDrawable JOGL GLAutoDrawable to be used to get pixel data from.
   * @param pAsynchronous   true if call should be asynchronous
   */
  public void screenshot(GLAutoDrawable pGLAutoDrawable, boolean pAsynchronous)
  {
    if (!mActive || tooSoon()) return;
   ...
   *
   * @param pDrawable     JOGL drawable
   * @param pOutputFile   output file
   * @param pAsynchronous
   */
  private void writeDrawableToFile(GLAutoDrawable pDrawable, final File pOutputFile, final boolean pAsynchronous)
  {
    final int lTargetPeriodInMiliSeconds = (int) (1000 / getTargetFrameRate());
    try
    {
      final boolean lIsLocked = mReadPixelsLock.tryLock(lTargetPeriodInMiliSeconds / 2, TimeUnit.MILLISECONDS);
      ...
      if (lIsLocked)
      {
        ...
        if (lByteBuffer == null || lByteBuffer.capacity() != lWidth * lHeight * 3)
        {
          lByteBuffer = ByteBuffer.allocateDirect(lWidth * lHeight * 3).order(ByteOrder.nativeOrder());
        }
        ...
        if (pAsynchronous)
        {
          writeBufferToFile(pOutputFile, lWidth, lHeight, lFinalByteBuffer);
    ...
    {
      if (mReadPixelsLock.isHeldByCurrentThread()) mReadPixelsLock.unlock();
    }
   ...
   * @param pByteBuffer
   */
  private void writeBufferToFile(File pOutputFile, int pWidth, int pHeight, ByteBuffer pByteBuffer)
  {
    try
    {
      ...
      for (int row = 0; row < pHeight; row++)
      {
        ...
        for (int col = 0; col < pWidth; col++)
        {
          ...
          lPixelInts[i++] = 0xFF000000 | ((iR & 0x000000FF) << 16) | ((iG & 0x000000FF) << 8) | (iB & 0x000000FF);
        }
      ...
      if (lBufferedImage == null || lBufferedImage.getWidth() != pWidth || lBufferedImage.getHeight() != pHeight)
      {
        lBufferedImage = new BufferedImage(pWidth, pHeight, BufferedImage.TYPE_INT_ARGB);
