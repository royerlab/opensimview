...
public class DemoHalcyonMain extends Application
{
  ...
  public void start(Stage pPrimaryStage) throws Exception
  {
    ...
    TreePanel lTreePanel = new TreePanel("Config", "Test Microscopy", getClass().getResourceAsStream(lRootIconPath), lNodeTypeList);
    ...
    final HalcyonFrame lHalcyonFrame = new HalcyonFrame("Demo", lAppIconPath, 800, 600);

    lHalcyonFrame.setTreePanel(lTreePanel);

    final HalcyonNode lLaser1 = HalcyonNode.wrap("Laser-1", DemoHalcyonNodeType.ONE, new VBox());

    final HalcyonNode lLaser2 = HalcyonNode.wrap("Laser-2", DemoHalcyonNodeType.ONE, new VBox());

    final HalcyonNode lCamera = HalcyonNode.wrap("Camera-1", DemoHalcyonNodeType.TWO, new VBox());

    final HalcyonNode lStage1 = HalcyonNode.wrap("Stage-1", DemoHalcyonNodeType.THREE, new VBox());
