...
public class HalcyonGroupNode extends HalcyonNodeBase implements HalcyonNodeInterface
{
  ...
  public HalcyonGroupNode(String name, HalcyonNodeType type, Grouping grouping, List<HalcyonNodeInterface> nodes)
  {
    super(name, type);
  ...
  {
    if (mPane != null) mPane.getChildren().removeIf(c -> c.equals(node.getPanel()));
    else if (mTabPane != null) mTabPane.getTabs().removeIf(c -> c.getContent().equals(node.getPanel()));
