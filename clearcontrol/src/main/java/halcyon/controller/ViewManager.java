...
public class ViewManager
{
   ...
   */
  public ViewManager(DockPane pDockPane, TreePanel pTreePanel, HalcyonNodeRepository nodes, ObservableCollection<DockNode> pConsoles, ObservableCollection<DockNode> pToolbars, Menu pViewMenu, String pAppIconPath)
  {
  ...
  {
    if (mControlNodeMap.get(pMenu).getCount() == 0) item.dock(mDockPane, pDefaultPosition, mTreePanel);
    else item.dock(mDockPane, DockPos.CENTER, mControlNodeMap.get(pMenu).get(0));

    mControlNodeMap.get(pMenu).add(item);
  }

  private void dockNodes(String pMenu, DockPos pPosition, ObservableCollection<DockNode> pControlNodes)
  {
    ...
    if (pControlNodes.getCount() > 0) pControlNodes.get(0).focus();
  }

  private void addViewMenuItem(String pMenuGroupName, DockNode pControlNode)
  {
    mViewMenu.getItems().stream().filter(c -> c.getText().equals(pMenuGroupName)).findFirst().ifPresent(c ->
    {
      CheckMenuItem lMenuItem = new CheckMenuItem(pControlNode.getTitle());

      lMenuItem.setSelected(!pControlNode.isClosed());
      pControlNode.closedProperty().addListener(new ChangeListener<Boolean>()
      {
        @Override
      ...
      lMenuItem.setOnAction(new EventHandler<ActionEvent>()
      {
        ...
        public void handle(ActionEvent event)
        {
          if (pControlNode.isClosed())
          {
            ...
            if (pMenuGroupName.equals("Console")) lDefaultPos = DockPos.RIGHT;
            else if (pMenuGroupName.equals("Toolbar")) lDefaultPos = DockPos.TOP;
  ...
  public void makeIndependentWindow(HalcyonNodeInterface node)
  {
    ...
    if (!mExternalNodeMap.containsKey(node))
    {
      ...
      if (mAppIconPath != null) lStage.getIcons().add(new Image(getClass().getResourceAsStream(mAppIconPath)));
      ...
      lStage.setOnCloseRequest(new EventHandler<WindowEvent>()
      {
        ...
        public void handle(WindowEvent event)
        {
          if (!lStage.isIconified() && !node.getName().equals(""))
          {
            ...
            lHolder.addProperty("Size", new Double[]{lStage.getWidth(), lStage.getHeight()});
            lHolder.addProperty("Position", new Double[]{lStage.getX(), lStage.getY()});
  ...
  public void storeOtherNodePreference(String filePath)
  {
    ...
    {
      ContentHolder halcyonNode = new ContentHolder(n.getName(), ContentHolder.Type.FloatingNode);
      ...
      halcyonNode.addProperty("Stage", mExternalNodeMap.get(n).getTitle());

      if (n instanceof HalcyonGroupNode)
      {
        halcyonNode.addProperty("Grouping", ((HalcyonGroupNode) n).getGrouping());
        halcyonNode.addProperty("Nodes", ((HalcyonGroupNode) n).getNodeNames());
      }

      halcyonNode.addProperty("Title", n.getName());
      halcyonNode.addProperty("Size", new Double[]{mExternalNodeMap.get(n).getWidth(), mExternalNodeMap.get(n).getHeight()});
      halcyonNode.addProperty("Position", new Double[]{mExternalNodeMap.get(n).getX(), mExternalNodeMap.get(n).getY()});
  ...
  protected void restoreEntry(HashMap<String, ContentHolder> map, String key)
  {
    ...
    if (node.getProperties().containsKey("Stage"))
    {
      ...
      if (node.getProperties().containsKey("Grouping"))
      {
        ...
        n = new HalcyonGroupNode("", DemoHalcyonNodeType.ONE, lGrouping, nodeList);
      } else
      {
      ...
      if (null == position) position = new Double[]{0d, 0d};
