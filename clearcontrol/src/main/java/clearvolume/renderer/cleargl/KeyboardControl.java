...
class KeyboardControl extends KeyAdapter implements KeyListener
{
   ...
   *
   * @param pMouseControl
   * @param pJoglVolumeRenderer renderer
   */
  KeyboardControl(final ClearVolumeRendererInterface pClearVolumeRenderer, MouseControl pMouseControl)
  {
  ...
  public void keyPressed(final KeyEvent pE)
  {
    ...
    final float lTranslationSpeed = lIsShiftPressed ? 0.1f : (lIsMetaPressed ? 0.001f : 0.01f);
    final float lRotationSpeed = (float) (2 * PI * (lIsShiftPressed ? 0.025f : (lIsMetaPressed ? 0.0005f : 0.005f)));
    ...
    switch (pE.getKeyCode())
    {
      ...
      case KeyEvent.VK_DOWN:
        if (mToggleRotationTranslation)
        {
          ...
          if (lAutoRotateController.isActive()) lAutoRotateController.addRotationSpeedX(-lAutoRotationSpeed);
          else
            mClearVolumeRenderer.setQuaternion(mClearVolumeRenderer.getQuaternion().invert().rotateByAngleX(-lRotationSpeed).invert());

        } else mClearVolumeRenderer.addTranslationY(-lTranslationSpeed);
      ...
      case KeyEvent.VK_LEFT:
        if (mToggleRotationTranslation)
        {
          if (lAutoRotateController.isActive()) lAutoRotateController.addRotationSpeedY(-lAutoRotationSpeed);
          else
            mClearVolumeRenderer.setQuaternion(mClearVolumeRenderer.getQuaternion().invert().rotateByAngleY(+lRotationSpeed).invert());

        } else mClearVolumeRenderer.addTranslationX(-lTranslationSpeed);
        mClearVolumeRenderer.notifyChangeOfVolumeRenderingParameters();
        break;
      ...
      case KeyEvent.VK_PAGE_DOWN:
        if (mToggleRotationTranslation)
        {
          if (lAutoRotateController.isActive()) lAutoRotateController.addRotationSpeedZ(-lAutoRotationSpeed);
          else
            mClearVolumeRenderer.setQuaternion(mClearVolumeRenderer.getQuaternion().invert().rotateByAngleZ(+lRotationSpeed).invert());

        } else mClearVolumeRenderer.addTranslationZ(-lTranslationSpeed / mClearVolumeRenderer.getFOV());
      ...
      case KeyEvent.VK_ESCAPE:
        if (mClearVolumeRenderer.isFullScreen()) mClearVolumeRenderer.toggleFullScreen();
        break;
      ...
      case KeyEvent.VK_O:
        ...
        else mClearVolumeRenderer.setFOV(ClearVolumeRendererBase.cDefaultFOV);
        break;
    ...
    if (pE.getKeyCode() >= KeyEvent.VK_0 && pE.getKeyCode() <= KeyEvent.VK_9)
    {
      ...
      if (lRenderLayerIndex == 0) lRenderLayerIndex = 10;
      else lRenderLayerIndex--;

      if (lRenderLayerIndex < mClearVolumeRenderer.getNumberOfRenderLayers())
      {
        if (lIsShiftPressed)
          mClearVolumeRenderer.setLayerVisible(lRenderLayerIndex, !mClearVolumeRenderer.isLayerVisible(lRenderLayerIndex));
        else mClearVolumeRenderer.setCurrentRenderLayer(lRenderLayerIndex);
      }
    }
  ...
  private void processSingleKeyToggableEvents(KeyEvent pE, final Collection<?> lOverlays)
  {
    ...
    if (lHasAnyOverlayBeenToggled) mClearVolumeRenderer.notifyChangeOfVolumeRenderingParameters();
  }
