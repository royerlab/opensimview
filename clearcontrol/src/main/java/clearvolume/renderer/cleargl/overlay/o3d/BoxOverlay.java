 ...
 */
public class BoxOverlay extends OverlayBase implements Overlay3D, SingleKeyToggable
{
  ...
  public BoxOverlay(final int pNumberOfGridlines, final float width, final boolean pAlignToClipBox,
  ...
  public void init(GL pGL, DisplayRequestInterface pDisplayRequestInterface)
  {
    ...
    try
    {
      mBoxGLProgram = GLProgram.buildProgram(pGL, BoxOverlay.class, "shaders/box_vert.glsl", "shaders/box_frag.glsl");

      mClearGeometryObject = new ClearGeometryObject(mBoxGLProgram, 3, GL.GL_TRIANGLES);

      mBoxGLProgram.getUniform("NumberOfGridLines").setInt(mNumberOfGridLines);
      ...
      lIndexIntArray.add(0, 1, 2, 0, 2, 3, 4, 5, 6, 4, 6, 7, 8, 9, 10, 8, 10, 11, 12, 13, 14, 12, 14, 15, 16, 17, 18, 16, 18, 19, 20, 21, 22, 20, 22, 23);
  ...
  {
    setVertices(new float[]{-1.f, 1.f, -1.f, 1.f, -1.f, 1.f});
  ...
  public void render3D(ClearGLVolumeRenderer pClearGLVolumeRenderer, GL pGL, int pWidth, int pHeight, GLMatrix pProjectionMatrix, GLMatrix pModelViewMatrix)
  {
    ...
    if (isDisplayed())
    {
      ...
      if (mAlignToClipBox)
      {
        ...
        if (!Arrays.equals(mClipBox, lClipBox)) updateClipBox(lClipBox);
      }
