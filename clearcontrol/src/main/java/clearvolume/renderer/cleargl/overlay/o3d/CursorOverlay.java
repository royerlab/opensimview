 ...
 */
public class CursorOverlay extends OverlayBase implements Overlay3D, SingleKeyToggable, EyeRayListener
{
  ...
  private float[] mColor = new float[]{0.8f, 0.8f, 1f, 1f};
  ...
  public void init(GL pGL, DisplayRequestInterface pDisplayRequestInterface)
  {
    try
    {
      mBoxGLProgram = GLProgram.buildProgram(pGL, CursorOverlay.class, "shaders/cursor_vert.glsl", "shaders/cursor_frag.glsl");

      mPlaneX = new ClearGeometryObject(mBoxGLProgram, 3, GL.GL_TRIANGLES);
      mPlaneX.setDynamic(true);/**/

      mPlaneY = new ClearGeometryObject(mBoxGLProgram, 3, GL.GL_TRIANGLES);
      mPlaneY.setDynamic(true);

      mPlaneZ = new ClearGeometryObject(mBoxGLProgram, 3, GL.GL_TRIANGLES);
      ...
      final GLFloatArray lTexCoordFloatArrayPlane = new GLFloatArray(4, 2);
   ...
   */
  @Override
  public void render3D(ClearGLVolumeRenderer pClearGLVolumeRenderer, GL pGL, int pWidth, int pHeight, GLMatrix pProjectionMatrix, GLMatrix pModelViewMatrix)
  {
    if (isDisplayed())
    {
      ...
      mBoxGLProgram.getUniform("linethick").setFloat(1.f / getLineThickness());
      mBoxGLProgram.getUniform("linelength").setFloat(1.f / getLineLength());
      mBoxGLProgram.getUniform("lineperiod").setFloat(getLinePeriod());
      mBoxGLProgram.getUniform("boxlinesalpha").setFloat(getBoxLinesAlpha());
      ...
      final float[] lProject = project(new float[]{2 * x - 1, 2 * y - 1, 2 * z - 1, 1}, pModelViewMatrix, pProjectionMatrix);
  ...
  public boolean notifyEyeRay(ClearGLVolumeRenderer pRenderer, MouseEvent pMouseEvent, EyeRay pEyeRay)
  {
    ...
    if (!(lCorrectMouseButton && lCorrectMouseEvent && lCorrectModifier)) return false;

    final float[] lX = new float[]{x, y, z};
    ...
    final double lDistance = sqrt((mx - px) * (mx - px) + (my - py) * (my - py));
  ...
  public static final float[] project(float[] pVector, GLMatrix pModelViewMatrix, GLMatrix pProjectionMatrix)
  {
