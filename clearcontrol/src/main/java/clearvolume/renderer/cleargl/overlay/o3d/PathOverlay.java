...
{
  protected static final FloatBuffer cBoxColor = FloatBuffer.wrap(new float[]{1.f, 1.f, 1.f, 1.f});
  ...
  public void init(GL pGL, DisplayRequestInterface pDisplayRequestInterface)
  {
    ...
    try
    {
      mBoxGLProgram = GLProgram.buildProgram(pGL, PathOverlay.class, new String[]{"shaders/path_vert.glsl", "shaders/path_geom.glsl", "shaders/path_frag.glsl"});

      mPath = new ClearGeometryObject(mBoxGLProgram, 3, GL.GL_LINE_STRIP);
  ...
  public void render3D(ClearGLVolumeRenderer pClearGLVolumeRenderer, GL pGL, int pWidth, int pHeight, GLMatrix pProjectionMatrix, GLMatrix pModelViewMatrix)
  {
    if (isDisplayed())
    {
      ...
      mPath.getProgram().getUniform("vertexCount").setInt((mPathPoints.getFloatBuffer().capacity() / 3));
