...
public class OldBoxOverlay extends OverlayBase implements Overlay3D
{
  ...
  private static final FloatBuffer cBoxColor = FloatBuffer.wrap(new float[]{1.f, 1.f, 1.f, 0.5f});
  ...
  public void init(GL pGL, DisplayRequestInterface pDisplayRequestInterface)
  {
    ...
    try
    {
      mBoxGLProgram = GLProgram.buildProgram(pGL, OldBoxOverlay.class, "shaders/oldbox_vert.glsl", "shaders/oldbox_frag.glsl");
  ...
  public void render3D(ClearGLVolumeRenderer pClearGLVolumeRenderer, GL pGL, int pWidth, int pHeight, GLMatrix pProjectionMatrix, GLMatrix pModelViewMatrix)
  {
    if (isDisplayed())
    {
      ...
      mOverlayModelViewMatrixUniform.setFloatMatrix(pModelViewMatrix.getFloatArray(), false);

      mOverlayProjectionMatrixUniform.setFloatMatrix(pProjectionMatrix.getFloatArray(), false);
