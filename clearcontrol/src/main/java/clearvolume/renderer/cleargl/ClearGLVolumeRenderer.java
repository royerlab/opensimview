        ...
        ClearVolumeRendererBase implements ClearGLEventListener
{
  ...
  private static final GLMatrix cOverlay2dProjectionMatrix = GLMatrix.getOrthoProjectionMatrix(-1, 1, -1, 1, 0, 1000);/**/
  ;
  ...
  private volatile int mViewportX, mViewportY, mViewportWidth, mViewportHeight;
  ...
  private volatile int mMaxRenderWidth, mMaxRenderHeight, mRenderWidth, mRenderHeight;
  private volatile boolean mUpdateTextureWidthHeight = true;

  private volatile boolean mRequestDisplay = true;

  private final float mLightVector[] = new float[]{-1.f, 1.f, 1.f};
   ...
   */
  public ClearGLVolumeRenderer(final String pWindowName, final int pWindowWidth, final int pWindowHeight)
  {
    this(pWindowName, pWindowWidth, pWindowHeight, NativeTypeEnum.UnsignedByte);
  }
   ...
   */
  public ClearGLVolumeRenderer(final String pWindowName, final int pWindowWidth, final int pWindowHeight, final NativeTypeEnum pNativeTypeEnum)
  {
    this(pWindowName, pWindowWidth, pWindowHeight, pNativeTypeEnum, 768, 768);
  }
   ...
   * @param pWindowHeight    window height
   * @param pNativeTypeEnum  native type
   * @param pMaxRenderWidth  max render width
   * @param pMaxRenderHeight max render height
   */
  public ClearGLVolumeRenderer(final String pWindowName, final int pWindowWidth, final int pWindowHeight, final NativeTypeEnum pNativeTypeEnum, final int pMaxRenderWidth, final int pMaxRenderHeight)
  {
    this(pWindowName, pWindowWidth, pWindowHeight, pNativeTypeEnum, pMaxRenderWidth, pMaxRenderHeight, 1);
  }
   ...
   * @param pMaxRenderWidth  max render width
   * @param pMaxRenderHeight max render height
   * @param pUseInCanvas     if true, this Renderer will not be displayed in a window of it's
   *                         own, but must be embedded in a GUI as Canvas.
   */
  public ClearGLVolumeRenderer(final String pWindowName, final int pWindowWidth, final int pWindowHeight, final NativeTypeEnum pNativeTypeEnum, final int pMaxRenderWidth, final int pMaxRenderHeight, final boolean pUseInCanvas)
  {
    this(pWindowName, pWindowWidth, pWindowHeight, pNativeTypeEnum, pMaxRenderWidth, pMaxRenderHeight, 1, pUseInCanvas);
  }
   ...
   * @param pWindowHeight         window height
   * @param pNativeTypeEnum       native type
   * @param pMaxTextureWidth      max render width
   * @param pMaxTextureHeight     max render height
   * @param pNumberOfRenderLayers number of render layers
   */
  public ClearGLVolumeRenderer(final String pWindowName, final int pWindowWidth, final int pWindowHeight, final NativeTypeEnum pNativeTypeEnum, final int pMaxTextureWidth, final int pMaxTextureHeight, final int pNumberOfRenderLayers)
  {
    this(pWindowName, pWindowWidth, pWindowHeight, pNativeTypeEnum, pMaxTextureWidth, pMaxTextureHeight, 1, false);
  }
  ...
  public ClearGLVolumeRenderer(final String pWindowName, final int pWindowWidth, final int pWindowHeight, final NativeTypeEnum pNativeTypeEnum, final int pMaxRenderWidth, final int pMaxRenderHeight, final int pNumberOfRenderLayers, final boolean pUseInCanvas)
  {
    ...
    final KeyboardControl lKeyboardControl = new KeyboardControl(this, lMouseControl);
    ...
    setVideoRecorder(new BasicVideoRecorder(new File(SystemUtils.USER_HOME, "Videos/ClearVolume")));
  ...
  private void displayInternal(final GLAutoDrawable pDrawable)
  {
    ...
    if (lTryLock) try
    {
      ...
      renderVolume(lModelViewMatrix.clone().invert().transpose().getFloatArray(), lProjectionMatrix.clone().invert().transpose().getFloatArray());
      ...
      renderOverlays3D(lGL, lAspectRatioCorrectedProjectionMatrix, lModelViewMatrix);
      ...
      if (lLastRenderPass && getVideoRecorder() != null)
        getVideoRecorder().screenshot(pDrawable, !getAutoRotateController().isRotating());
  ...
  private void ensureTextureAllocated()
  {
    if (mUpdateTextureWidthHeight)
    {
      ...
      try
      {
        for (int i = 0; i < getNumberOfRenderLayers(); i++)
        {
          if (mLayerTextures[i] != null) mLayerTextures[i].close();

          mLayerTextures[i] = new GLTexture(mGLProgram, GLTypeEnum.UnsignedByte, 4, mRenderWidth, mRenderHeight, 1, true, 2);
  ...
  private GLMatrix getAspectRatioCorrectedProjectionMatrix()
  {
    ...
    lProjectionMatrix.setPerspectiveProjectionMatrix(getFOV(), 1, .1f, 1000);
  ...
  private GLMatrix getModelViewMatrix()
  {
    ...
    lModelViewMatrix.translate((clipBox[0] + clipBox[1]) / 2.0f + getTranslationX(), (clipBox[2] + clipBox[3]) / 2.0f + getTranslationY(), (clipBox[4] + clipBox[5]) / 2.0f + getTranslationZ());

    lModelViewMatrix.mult(getQuaternion());

    lModelViewMatrix.translate(-(clipBox[0] + clipBox[1]) / 2.0f, -(clipBox[2] + clipBox[3]) / 2.0f, -(clipBox[4] + clipBox[5]) / 2.0f);

    lModelViewMatrix.scale((float) (lScaleX / lMaxScale), (float) (lScaleY / lMaxScale), (float) (lScaleZ / lMaxScale));
  ...
  public void init(final GLAutoDrawable drawable)
  {
    ...
    if (initVolumeRenderer())
    {
      ...
      try
      {
        ...
        mGLProgram = GLProgram.buildProgram(lGL, lVertexShaderSource, lFragmentShaderSource);
  ...
  public boolean isShowing()
  {
    try
    {
      if (mNewtCanvasAWT != null) return mNewtCanvasAWT.isVisible();

      if (mClearGLWindow != null) return mClearGLWindow.isVisible();
    } catch (final NullPointerException e)
    {
      return false;
    }
   ...
   *
   * @param pRenderer   renderer that calls listeners
   * @param pMouseEvent associated mouse event.
   * @return true if event captured
   */
  public boolean notifyEyeRayListeners(ClearGLVolumeRenderer pRenderer, MouseEvent pMouseEvent)
  {
    if (mEyeRayListenerList.isEmpty()) return false;

    final int lX = pMouseEvent.getX();
    final int lY = pMouseEvent.getY();

    final GLMatrix lInverseModelViewMatrix = getModelViewMatrix().clone().invert();
    final GLMatrix lInverseProjectionMatrix = getClearGLWindow().getProjectionMatrix().clone().invert();
    ...
    final EyeRay lEyeRay = ScreenToEyeRay.convert(getViewportWidth(), getViewportHeight(), lX, lY, lInverseModelViewMatrix, lAspectRatioCorrectedProjectionMatrix);

    boolean lPreventOtherDisplayChanges = false;

    for (final EyeRayListener lEyeRayListener : mEyeRayListenerList)
    {
      lPreventOtherDisplayChanges |= lEyeRayListener.notifyEyeRay(pRenderer, pMouseEvent, lEyeRay);
      if (lPreventOtherDisplayChanges) break;
    }
    ...
    try
    {
      ...
      for (final Overlay lOverlay : mOverlayMap.values())
        if (lOverlay instanceof Overlay2D)
        {
          ...
          try
          {
            lOverlay2D.render2D(this, lGL, getWindowWidth(), getWindowHeight(), pProjectionMatrix);
          } catch (final Throwable e)
          {
            e.printStackTrace();
          }
  ...
  private void renderOverlays3D(final GL lGL, final GLMatrix pProjectionMatrix, final GLMatrix pModelViewMatrix)
  {
    try
    {
      for (final Overlay lOverlay : mOverlayMap.values())
        if (lOverlay instanceof Overlay3D)
        {
          ...
          try
          {
            lOverlay3D.render3D(this, lGL, getWindowWidth(), getWindowHeight(), pProjectionMatrix, pModelViewMatrix);
          } catch (final Throwable e)
          {
            e.printStackTrace();
          }
  ...
  /**
   * @param pModelViewMatrix  Model-mViewMatrix matrix as float array
   * @param pProjectionMatrix Projection matrix as float array
   * @param pPhase
   * @param pClearBuffer
   ...
   */
  @Override
  public void reshape(final GLAutoDrawable pDrawable, final int x, final int y, int pWidth, int pHeight)
  {
    try
    {
      ...
      if (pHeight < 16) pHeight = 16;

      if (pWidth < 16) pWidth = 16;
      ...
      else mQuadProjectionMatrix.setOrthoProjectionMatrix(-lAspectRatio, lAspectRatio, -1, 1, 0, 1000);/**/

      // FIXME: first layer decides... this is temporary hack, should be
      // resolvd
      // by using the scene graph
      final int lMaxVolumeDimension = (int) max(getVolumeSizeX(0), max(getVolumeSizeY(0), getVolumeSizeZ(0)));

      final int lMaxTextureWidth = min(mMaxRenderWidth, 2 * lMaxVolumeDimension);
      final int lMaxTextureHeight = min(mMaxRenderHeight, 2 * lMaxVolumeDimension);

      final int lCandidateTextureWidth = ((min(lMaxTextureWidth, getViewportWidth()) / 128) * 128);
      final int lCandidateTextureHeight = ((min(lMaxTextureHeight, getViewportHeight()) / 128) * 128);

      if (lCandidateTextureWidth == 0 || lCandidateTextureHeight == 0) return;
      ...
      if (lRatioWidth > 0 && lRatioWidth < 1) lRatioWidth = 1f / lRatioWidth;

      if (lRatioHeight > 0 && lRatioHeight < 1) lRatioHeight = 1f / lRatioHeight;

      if (lRatioAspect > 0 && lRatioAspect < 1) lRatioAspect = 1f / lRatioAspect;
  ...
  {
    if (getClearGLWindow() != null) getClearGLWindow().setPerspectiveProjectionMatrix(getFOV(), 1, .1f, 1000);
  }
  ...
  {
    if (mNewtCanvasAWT == null && mClearGLWindow != null) mClearGLWindow.setVisible(pIsVisible);
  }
  ...
  public void toggleFullScreen()
  {
    try
    {
      if (mClearGLWindow.isFullscreen())
      {
        ...
        if (mLastWindowWidth > 0 && mLastWindowHeight > 0) mClearGLWindow.setSize(mLastWindowWidth, mLastWindowHeight);
      } else
      {
  ...
  {
    if (getVideoRecorder() != null) getVideoRecorder().toggleActive();
  }
