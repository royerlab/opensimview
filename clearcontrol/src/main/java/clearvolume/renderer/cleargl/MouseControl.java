...
class MouseControl extends MouseAdapter implements MouseListener
{
  ...
  MouseControl(final ClearGLVolumeRenderer pClearVolumeRenderer)
  {
    ...
    mArcBall.setBounds(mRenderer.getViewportWidth(), mRenderer.getViewportHeight());
  }
  ...
  {
    if (mRenderer.notifyEyeRayListeners(mRenderer, pMouseEvent)) return;
  ...
  public void mouseDragged(final MouseEvent pMouseEvent)
  {
    ...
    if (!handleAlphaBlending(pMouseEvent))
    {
  ...
  protected void handleMoveLight(final MouseEvent pMouseEvent)
  {
    ...
    if (!mMoveLightMode && mRenderer.getRenderAlgorithm(mRenderer.getCurrentRenderLayerIndex()) == RenderAlgorithm.IsoSurface && !pMouseEvent.isMetaDown() && !pMouseEvent.isShiftDown() && !pMouseEvent.isControlDown() && pMouseEvent.isAltDown() && !pMouseEvent.isAltGraphDown() && !pMouseEvent.isButtonDown(1))
    {
  ...
  {
    if (!pMouseEvent.isMetaDown() && !pMouseEvent.isShiftDown() && !pMouseEvent.isAltDown() && !pMouseEvent.isControlDown() && pMouseEvent.isButtonDown(1))
    {
  ...
  public void mousePressed(MouseEvent pMouseEvent)
  {
    ...
    if (!pMouseEvent.isMetaDown() && !pMouseEvent.isShiftDown() && !pMouseEvent.isControlDown() && pMouseEvent.isButtonDown(1))
    {
      final float lMouseX = pMouseEvent.getX();
      final float lMouseY = pMouseEvent.getY();
      mArcBall.setBounds(mRenderer.getViewportWidth(), mRenderer.getViewportHeight());
  ...
  private void handleTranslation(final MouseEvent pMouseEvent)
  {
    ...
    if (!pMouseEvent.isMetaDown() && !pMouseEvent.isControlDown() && (pMouseEvent.isButtonDown(3)))
    {
  ...
  public void handleGammaMinMax(final MouseEvent pMouseEvent)
  {
    if (!pMouseEvent.isMetaDown() && !pMouseEvent.isShiftDown() && pMouseEvent.isControlDown() && pMouseEvent.isButtonDown(1))
    {
      ...
      mRenderer.setTransferFunctionRange(Math.abs(Math.pow(nx, 3)), Math.abs(Math.pow(ny, 3)));
   ...
   * @param pMouseEvent mouse event
   */
  public boolean handleAlphaBlending(final MouseEvent pMouseEvent)
  {
    if (mRenderer.getRenderAlgorithm(mRenderer.getCurrentRenderLayerIndex()) == RenderAlgorithm.MaxProjection && !pMouseEvent.isMetaDown() && !pMouseEvent.isShiftDown() && pMouseEvent.isControlDown() && pMouseEvent.isAltDown() && !pMouseEvent.isAltGraphDown() && pMouseEvent.isButtonDown(1))
    {
      final double lWidth = mRenderer.getViewportWidth();
      final double nx = (pMouseEvent.getX()) / lWidth;
      ...
      mRenderer.setAlphaBlending(mRenderer.getCurrentRenderLayerIndex(), lAlpha);
