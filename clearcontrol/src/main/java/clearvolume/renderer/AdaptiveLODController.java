...
public class AdaptiveLODController
{
  ...
  private volatile long mFirstPassTimingStartTime = Long.MIN_VALUE, mFirstPassTimingStopTime;
  ...
  {
    mNewNumberOfPasses = min(cMaxNumberOfPasses, max(1, pNumberOfPasses));
  ...
  public float getPhase()
  {
    ...
    final float lPhase = computePhase(getNumberOfPasses(), mCurrentGenerator, mPassIndex);
  ...
  public void afterRendering()
  {
    if (mPassIndex == 0 && mFirstPassTimingStartTime != Long.MIN_VALUE)
    {
      ...
      mFilteredElapsedTimeInMs = alpha * mFilteredElapsedTimeInMs + (1 - alpha) * lElapsedTimeInMs;

      format("elapsed= %.3f, filtered=%f \n", lElapsedTimeInMs, mFilteredElapsedTimeInMs);

      final double lQuality = 1 - ((getNumberOfPasses() - 1.0) / cMaxNumberOfPasses);
      final double lSpeed = 1 - (mFilteredElapsedTimeInMs - mMinTimeForFirstPassInMilliseconds) / (mMaxTimeForFirstPassInMilliseconds - mMinTimeForFirstPassInMilliseconds);
  ...
  private boolean proceedWithMultiPass()
  {
    ...
    printlnverbose(this.getClass().getSimpleName() + ".proceedWithMultiPass -> continues with pass #" + mPassIndex);
    mPassIndex++;
  ...
  private static float computePhase(int pNumberOfPasses, int pGenerator, int pPassIndex)
  {
  ...
  {
    if (System.nanoTime() > mLastUserInputTime + cMarginTime) return false;
    return true;
  }
