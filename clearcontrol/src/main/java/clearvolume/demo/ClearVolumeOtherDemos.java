...
public class ClearVolumeOtherDemos
{
  ...
  public void demoRendererInJFrame() throws InterruptedException, IOException
  {
    ...
    final ClearVolumeRendererInterface lClearVolumeRenderer = ClearVolumeRendererFactory.newBestRenderer("ClearVolumeTest", 512, 512, NativeTypeEnum.UnsignedByte, 512, 512, 1, true);
  ...
  @Test
  public void demoDitheringAndResolutionOpenCL() throws InterruptedException, IOException
  {

    final ClearVolumeRendererInterface lClearVolumeRenderer = ClearVolumeRendererFactory.newBestRenderer("ClearVolumeTest", 512, 512, NativeTypeEnum.UnsignedByte, 512, 512, 1, false);
    ...
    for (int z = 0; z < lResolutionZ; z++)
      for (int y = 0; y < lResolutionY; y++)
        for (int x = 0; x < lResolutionX; x++)
        {
          ...
          final int lDistance = (x - lCenter) * (x - lCenter) + (y - lCenter) * (y - lCenter) + (z - lCenter) * (z - lCenter);

          lVolumeDataArray[lIndex] = (byte) (255 * Math.exp(-.001 * abs(lDistance - lCenter * lCenter / 5)));
  ...
  {
    final ClearVolumeRendererInterface lClearVolumeRenderer = new OpenCLVolumeRenderer("ClearVolumeTest", 512, 512, NativeTypeEnum.UnsignedByte, 512, 512, 1, false);
    lClearVolumeRenderer.setTransferFunction(TransferFunctions.getDefault());
    lClearVolumeRenderer.setVisible(true);
  ...
  @Test
  public void demoAspectRatioPreset() throws InterruptedException, IOException
  {
    final ClearVolumeRendererInterface lClearVolumeRenderer = new OpenCLVolumeRenderer("ClearVolumeTest", 512, 512, NativeTypeEnum.UnsignedByte, 512, 512, 1, false);
    lClearVolumeRenderer.setTransferFunction(TransferFunctions.getDefault());
    lClearVolumeRenderer.setVisible(true);
    ...
    lClearVolumeRenderer.setVoxelSize(0, lResolutionX * 5.0, lResolutionY * 4.0, lResolutionZ * 3.0);
    lClearVolumeRenderer.requestDisplay();
  ...
  @Test
  public void demoBoundingBox() throws InterruptedException, IOException
  {
    final ClearVolumeRendererInterface lClearVolumeRenderer = ClearVolumeRendererFactory.newBestRenderer("ClearVolumeTest", 512, 512, NativeTypeEnum.UnsignedByte, 512, 512, 1, false);
    lClearVolumeRenderer.setTransferFunction(TransferFunctions.getDefault());
    lClearVolumeRenderer.setVisible(true);
    ...
    for (int z = 0; z < lResolutionZ; z++)
      for (int y = 0; y < lResolutionY; y++)
        ...
        {
          final int lIndex = x + lResolutionX * y + lResolutionX * lResolutionY * z;
          lVolumeDataArray[lIndex] = (byte) (255 * Math.exp(-.0001 * (5 * (x - x0) * (x - x0) + (y - y0) * (y - y0) + (z - z0) * (z - z0))));
    ...
    while (lClearVolumeRenderer.isShowing())
    {
      ...
      lClearVolumeRenderer.setClipBox(new float[]{-x, x, -y, y, -z, z});
  ...
  @Test
  public void demoWithGeneratedDatasetWithEgg3D() throws InterruptedException, IOException
  {

    final ClearVolumeRendererInterface lClearVolumeRenderer = ClearVolumeRendererFactory.newBestRenderer8Bit("ClearVolumeTest", 512, 512, false);
    lClearVolumeRenderer.setTransferFunction(TransferFunctions.getDefault());
    lClearVolumeRenderer.setVisible(true);
    ...
    try
    {
      lEgg3DController = new ExternalRotationController(ExternalRotationController.cDefaultEgg3DTCPport, lClearVolumeRenderer);
    ...
    lClearVolumeRenderer.setVolumeDataBuffer(0, lWrappedArray, lResolutionX, lResolutionY, lResolutionZ);
    ...
    if (lEgg3DController != null) lEgg3DController.close();

  }

  @Test
  public void demoCaptureVolumeData() throws InterruptedException, IOException
  {
    ...
    lClearVolumeRenderer.addVolumeCaptureListener(new VolumeCaptureListener()
    {
      ...
      @Override
      public void capturedVolume(final ByteBuffer pCaptureBuffers, final NativeTypeEnum pNativeTypeEnum, final long pVolumeWidth, final long pVolumeHeight, final long pVolumeDepth, final double pVoxelWidth, final double pVoxelHeight, final double pVoxelDepth)
      {
        System.out.format("Captured volume type=%s (%d, %d, %d) (%g, %g, %g) \n", pNativeTypeEnum, pVolumeWidth, pVolumeHeight, pVolumeDepth, pVoxelWidth, pVoxelHeight, pVoxelDepth);
  ...
  @Test
  public void demoEyeRayListener() throws InterruptedException, IOException
  {
    ...
    lClearVolumeRenderer.addEyeRayListener(new EyeRayListener()
    {
      ...
      @Override
      public boolean notifyEyeRay(ClearGLVolumeRenderer pRenderer, MouseEvent pMouseEvent, EyeRay pEyeRay)
      {
        if (pMouseEvent.getButton() != 2 && pMouseEvent.getEventType() != MouseEvent.EVENT_MOUSE_CLICKED) return false;
        ...
        for (float i = 0; i < 100000 * lStepSize; i += lStepSize)
        {
          ...
          if (ix < 0 || ix >= lResolutionX || iy < 0 || iy >= lResolutionY || iz < 0 || iz >= lResolutionZ)
            if (lOnceIn) break;
            else continue;

          lOnceIn = true;
          final int lIndex = ix + lResolutionX * iy + lResolutionX * lResolutionY * iz;
  ...
  @Test
  public void demoWithAlphaBlending() throws InterruptedException, IOException
  {
    ...
    for (int z = 0; z < lResolutionZ; z++)
      for (int y = 0; y < lResolutionY; y++)
        for (int x = 0; x < lResolutionX; x++)
        {
          ...
          double r1 = Math.sqrt((x - lResolutionX / 2 - dx) * (x - lResolutionX / 2 - dx) + (y - lResolutionY / 2) * (y - lResolutionY / 2) + (z - lResolutionZ / 2) * (z - lResolutionZ / 2));
          double r2 = Math.sqrt((x - lResolutionX / 2 + dx) * (x - lResolutionX / 2 + dx) + (y - lResolutionY / 2) * (y - lResolutionY / 2) + (z - lResolutionZ / 2) * (z - lResolutionZ / 2));
          ...
          double res = (0 + 200 * Math.exp(-100 * (r1 - r0) * (r1 - r0)) + 200 * Math.exp(-100 * (r2 - r0) * (r2 - r0)));
  ...
  @Test
  public void demoClipping() throws InterruptedException, IOException
  {
