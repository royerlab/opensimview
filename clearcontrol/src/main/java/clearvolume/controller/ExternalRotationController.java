 ...
 */
public class ExternalRotationController extends QuaternionRotationControllerBase implements RotationControllerInterface, Closeable, Runnable
{
   ...
   *
   * @param pPortNumber     port number
   * @param pDisplayRequest display request
   * @throws UnknownHostException thrown when host unknown
   * @throws IOException          thrown when IO error
   */
  public ExternalRotationController(final int pPortNumber, DisplayRequestInterface pDisplayRequest) throws UnknownHostException, IOException
  {
    super();
  ...
  public void connectAsynchronouslyOrWait()
  {
    ...
    final Thread lConnectionThread = new Thread(lConnectionRunnable, "ClearVolume-ExternalControllerConnectionThread");
  ...
  public boolean connect()
  {
    try
    {
      ...
      mReceptionThread = new Thread(this, "ClearVolume-ExternalController");
  ...
  private void parseMessage(String pReadLine)
  {
    ...
    final String lSubString = pReadLine.substring(1, pReadLine.length() - 2);
  ...
  {
    return mClientSocket != null && mClientSocket.isConnected() && !mClientSocket.isClosed() && super.isActive();
  }
