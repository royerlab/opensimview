...
public class StackGenerator implements AutoCloseable
{
  ...
  public StackGenerator(LightSheetMicroscopeSimulator pSimulator)
  {
    ...
    try
    {
      ...
      mDrosophilaFluorescencePhantom =
                                     new DrosophilaHistoneFluorescence(mContext,
                                                                       mDrosophila,
                                                                       lPhantomWidth,
                                                                       lPhantomHeight,
                                                                       lPhantomDepth);
      ...
      mDrosophilaScatteringPhantom =
                                   new DrosophilaScatteringPhantom(mContext,
                                                                   mDrosophila,
                                                                   mDrosophilaFluorescencePhantom,
                                                                   lPhantomWidth / 2,
                                                                   lPhantomHeight / 2,
      ...
      mSimulator.setPhantomParameter(PhantomParameter.Fluorescence,
                                     mDrosophilaFluorescencePhantom.getImage());
      mSimulator.setPhantomParameter(PhantomParameter.Scattering,
  ...
  public void setCenteredROI(int pWidth, int pHeight)
  {
    for (int lCameraIndex =
                          0; lCameraIndex < mSimulator.getNumberOfDetectionArms(); lCameraIndex++)
    {
      mSimulator.setNumberParameter(CameraParameter.ROIWidth,
                                    lCameraIndex,
                                    pWidth);
      mSimulator.setNumberParameter(CameraParameter.ROIHeight,
                                    lCameraIndex,
                                    pHeight);
    }
  }
   ...
   * 
   * @param pLightSheetIntensity
   *          lightsheet intensity
   */
   ...
   *          number of planes
   */
  public void generateStack(final int pCameraIndex,
                            ...
                            final int pNumberOfPlanes)
  {
    ...
    int lImageWidth = mSimulator
                                .getNumberParameter(CameraParameter.ROIWidth,
                                                    pCameraIndex)
                                .intValue();
    int lImageHeight = mSimulator
                                 .getNumberParameter(CameraParameter.ROIHeight,
                                                     pCameraIndex)
                                 .intValue();

    if (mStack == null || mStack.getWidth() != lImageWidth
        || mStack.getHeight() != lImageHeight
        || mStack.getDepth() != pNumberOfPlanes)
      mStack =
             mContext.createSingleChannelImage(ImageChannelDataType.UnsignedInt16,
                                               lImageWidth,
                                               lImageHeight,
                                               pNumberOfPlanes);
    ...
    {
      mSimulator.setNumberParameter(IlluminationParameter.Height,
                                    i,
                                    mLightSheetHeight);
      mSimulator.setNumberParameter(IlluminationParameter.Intensity,
                                    i,
                                    0.f);
    ...
    mSimulator.setNumberParameter(IlluminationParameter.Intensity,
                                  pLightsheetIndex,
                                  mLightSheetIntensity);

    float dz = (pHighZ - pLowZ) / pNumberOfPlanes;

    for (int lPlaneIndex =
                         0; lPlaneIndex < pNumberOfPlanes; lPlaneIndex++)
    {
      ...
      ClearCLImage lCameraImage =
                                mSimulator.getCameraImage(pCameraIndex);

      lCameraImage.copyTo(mStack,
                          Region3.originZero(),
                          Region3.origin(0, 0, lPlaneIndex),
                          Region3.region(lImageWidth, lImageHeight),
                          true);
