...
public class ClearVolumeNetworkTests
{
  ...
  public void testConsole() throws IOException, InterruptedException
  {
    ...
    final VolumeSinkInterface lVolumeSink =
                                          new VolumeSinkAdapter(cNumberOfAvailableVolumes)
                                          {
                                            @Override
                                            public void sendVolume(final Volume pVolume)
                                            {
                                              System.out.println("Received volume:"
                                                                 + pVolume);
                                            }
                                          };
  ...
  public void testLive() throws IOException, InterruptedException
  {
    ...
    final ClearVolumeRendererSink lClearVolumeRendererSink =
                                                           new ClearVolumeRendererSink(lClearVolumeRenderer,
                                                    ...
                                                    InterruptedException
  {
    ...
    final VolumeManager lVolumeManager =
                                       new VolumeManager(cNumberOfAvailableVolumes);
    final ClearVolumeTCPServerSink lClearVolumeTCPServerSink =
                                                             new ClearVolumeTCPServerSink(cVolumeQueueLength);
    lClearVolumeTCPServerSink.setRelaySink(new NullVolumeSink(lVolumeManager));

    final SocketAddress lServerSocketAddress =
                                             new InetSocketAddress(ClearVolumeSerialization.cStandardTCPPort
                                                                   + lPortRandomizer);
    assertTrue(lClearVolumeTCPServerSink.open(lServerSocketAddress));
    assertTrue(lClearVolumeTCPServerSink.start());

    final ClearVolumeTCPClient lClearVolumeTCPClient =
                                                     new ClearVolumeTCPClient(lVolumeSink);

    final SocketAddress lClientSocketAddress =
    ...
    final Volume lVolume =
                         lClearVolumeTCPServerSink.getManager()
    ...
    for (int i = 0; i < pNumberOfVolumes; i++)
    {
      ...
      for (int z = 0; z < cWidth; z++)
        for (int y = 0; y < cHeight; y++)
          ...
          {
            byte lCharValue = (byte) ((byte) x * i ^ (byte) y
                                      ^ (byte) z);
            if (lCharValue < 12)
              lCharValue = 0;
