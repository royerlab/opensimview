...
public class ClearVolumeSerialization
{
                                           ...
                                           ByteBuffer pByteBuffer)
  {
    ...
    final int lNeededBufferLength =
                                  ToIntExact.toIntExact(3
                                                        * cLongSizeInBytes
                                                        + lHeaderLength
                                                        + lDataLength);
    if (pByteBuffer == null
        || pByteBuffer.capacity() != lNeededBufferLength)
    {
                                        ...
                                        StringBuilder pStringBuilder)
  {
    ...
    final LinkedHashMap<String, String> lHeaderMap =
                               ...
                               Volume pVolume)
  {
    ...
    final Map<String, String> lHeaderMap =
                                         KeyValueMaps.readMapFromBuffer(pByteBuffer,
                                                                        pHeaderLength,
                                                                        null);
    final long lIndex = parseLong(lHeaderMap.get("index"), 0);
    final double lTime = parseDouble(lHeaderMap.get("time"), 0);
    final int lVolumeChannelID =
                               parseInt(lHeaderMap.get("channel"), 0);
    final String lVolumeChannelName =
                                    parseString(lHeaderMap.get("channelname"),
                                                "noname");

    final float[] lColor =
                         lHeaderMap.get("color") == null ? null
                                                         : parseFloatArray(lHeaderMap.get("color"),
                                                                           new float[]
                                                                           { 1.f, 1.f, 1.f, 1.f });

    final float[] lViewMatrix =
                              parseFloatArray(lHeaderMap.get("viewmatrix"),
                                              new float[]
    ...
    final String lRealUnitName =
                               parseString(lHeaderMap.get("realunit"),
                                           "1");
    final double lVoxelWidth =
                             parseDouble(lHeaderMap.get("voxelwidth"),
                                         1.);
    final double lVoxelHeight =
                              parseDouble(lHeaderMap.get("voxelheight"),
                                          1.);
    final double lVoxelDepth =
                             parseDouble(lHeaderMap.get("voxeldepth"),
                                         1.);
  ...
  private static ThreadLocal<ByteBuffer> sScratchBufferThreadLocal =
                                                              ...
                                                              final int lHeaderLength)
  {
    if (pScratchBuffer == null
        || pScratchBuffer.capacity() < lHeaderLength)
    {
                                    ...
                                    ByteBuffer pScratchBuffer) throws IOException
  {
    ...
    pScratchBuffer.rewind();
    final int lHeaderLength =
                            ToIntExact.toIntExact(pScratchBuffer.getLong());
  ...
  {
    pByteBuffer.rewind();
    final int lWholeLength =
                           ToIntExact.toIntExact(pByteBuffer.getLong());
    final int lHeaderLength =
                            ToIntExact.toIntExact(pByteBuffer.getLong());
                             ...
                             Volume pVolume)
  {
    ...
    if (pVolume.getDataBuffer() == null
        || pVolume.getDataBuffer().capacity() != pDataLength)
    {
      final ByteBuffer lByteBuffer =
                                   ByteBuffer.allocateDirect(ToIntExact.toIntExact(pDataLength));
