...
public class ClearVolumeTCPServerSink extends RelaySinkAdapter
                                      implements
                                      Closeable,
                                      RelaySinkInterface
{
  ...
  public ClearVolumeTCPServerSink(int pBufferMaxCapacity)
  {
    ...
    mSourceToSinkBufferedAdapter =
                                 new SourceToSinkBufferedAdapter(getManager(),
                                                                 pBufferMaxCapacity);
  }
  ...
  {
    lRunnable =
              new ClearVolumeTCPServerSinkRunnable(this,
                                                   mServerSocketChannel,
                                                   mSourceToSinkBufferedAdapter);
    mRunnableThread = new Thread(lRunnable,
                                 ClearVolumeTCPServerSinkRunnable.class.getSimpleName()
                                            + "Thread");
  ...
  public void sendVolume(Volume pVolume)
  {
    if (pVolume != null)
    {
      ...
      {
        lNewLastSeenVolume =
                           mManager.requestAndWaitForVolumeLike(1,
      ...
      final boolean lSucceededInSending =
                                        mSourceToSinkBufferedAdapter.sendVolumeWithFeedback(pVolume);
