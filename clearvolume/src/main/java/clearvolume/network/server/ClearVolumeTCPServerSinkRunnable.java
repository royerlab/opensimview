...
public class ClearVolumeTCPServerSinkRunnable implements Runnable
{
  ...
  public void run()
  {
    try
    {
      while (!mStopSignal)
      {
        final SocketChannel lSocketChannel =
                                           mServerSocketChannel.accept();
        ...
        try
        {
          ...
          while (lSocketChannel.isOpen()
                 && lSocketChannel.isConnected() && !mStopSignal)
          {
            final Volume lVolumeToSend =
                                       mVolumeSource.requestVolumeAndWait(10,
