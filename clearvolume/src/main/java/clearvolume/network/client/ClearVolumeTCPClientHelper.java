...
public abstract class ClearVolumeTCPClientHelper
{
                          ...
                          final Image appicon)
  {
    ...
    try
    {
      ...
      final NativeTypeEnum lNativeType =
                                       pBytesPerVoxel == 1 ? NativeTypeEnum.UnsignedByte
                                                           : NativeTypeEnum.UnsignedShort;

      try (
          ClearVolumeRendererSink lClearVolumeRendererSink =
                                                           new ClearVolumeRendererSink(lWindowTitle,
                                                                                       pWindowSize,
                                                                                       pWindowSize,
                                                                                       lNativeType,
                                                                                       pNumberOfLayers,
                                                                                       ...
                                                                                       cMaxAvailableVolumes);)
      {
        ...
        RelaySinkInterface lSinkAfterAsynchronousVolumeSinkAdapter =
        ...
        if (pChannelFilter)
        {
          ...
          lSinkAfterAsynchronousVolumeSinkAdapter =
                                                  lChannelFilterSink;
        }
        ...
        if (pTimeShift)
        {
          ...
          lTimeShiftingSinkJFrame =
                                  new TimeShiftingSinkJFrame(lTimeShiftingSink);
        ...
        final AsynchronousVolumeSinkAdapter lAsynchronousVolumeSinkAdapter =
                                                                           new AsynchronousVolumeSinkAdapter(lSinkAfterAsynchronousVolumeSinkAdapter,
                                                                                                             cMaxQueueLength,
                                                                                                             cMaxMillisecondsToWait,
                                                                                                             TimeUnit.MILLISECONDS);

        final ClearVolumeTCPClient lClearVolumeTCPClient =
                                                         new ClearVolumeTCPClient(lAsynchronousVolumeSinkAdapter);

        final SocketAddress lClientSocketAddress =
                                                 new InetSocketAddress(pServerAddress,
                                                                       pPortNumber);
        if (!lClearVolumeTCPClient.open(lClientSocketAddress))
        {
          throw new RuntimeException("Could not open connection to "
                                     + pServerAddress);
        }
    ...
    try
    {
      if (ExceptionUtils.getFullStackTrace(pThrowable)
        ...
        JOptionPane.showMessageDialog(null,
               ...
               && pThrowable.getLocalizedMessage() != null
