...
public class AdaptiveLODController
{
  ...
  private volatile double mFilteredElapsedTimeInMs =
                                                   mMaxTimeForFirstPassInMilliseconds;
  ...
  public void setNumberOfPasses(final int pNumberOfPasses)
  {
    ...
    mNewGenerator =
                  ModularSequence.findKWithBestGapScoreCached(pNumberOfPasses);
  }
  ...
  {
    printlnverbose(this.getClass().getSimpleName()
  ...
  public boolean beforeRendering()
  {
    ...
    {
      printlnverbose(this.getClass().getSimpleName()
                     + ".beforeRendering -> multi-pass is active");
      if (isUserInteractionInProgress())
      {
        // multipass rendering needs to restart from scratch:
        printlnverbose(this.getClass().getSimpleName()
                       + ".beforeRendering -> multi-pass needs to be restarted");
        resetMultiPassRendering();
  ...
  public void afterRendering()
  {
        ...
        && mFirstPassTimingStartTime != Long.MIN_VALUE)
    {
      ...
      final double lElapsedTimeInMs = ((mFirstPassTimingStopTime
                                        - mFirstPassTimingStartTime)
                                       * 1e-6);

      final double alpha = 0.95;

      mFilteredElapsedTimeInMs = alpha * mFilteredElapsedTimeInMs
                                 + (1 - alpha) * lElapsedTimeInMs;

      format("elapsed= %.3f, filtered=%f \n",
             lElapsedTimeInMs,
             mFilteredElapsedTimeInMs);

      final double lQuality = 1 - ((getNumberOfPasses() - 1.0)
                                   / cMaxNumberOfPasses);
      final double lSpeed =
                          1 - (mFilteredElapsedTimeInMs
                               - mMinTimeForFirstPassInMilliseconds)
                              / (mMaxTimeForFirstPassInMilliseconds
                                 - mMinTimeForFirstPassInMilliseconds);
  ...
  private boolean proceedWithMultiPass()
  {
    ...
    printlnverbose(this.getClass().getSimpleName()
                   + ".proceedWithMultiPass -> continues with pass #"
                   + mPassIndex);
    mPassIndex++;
    if (mPassIndex < getNumberOfPasses())
    {
      ...
      printlnverbose(this.getClass().getSimpleName()
                     + ".proceedWithMultiPass -> more passes to do");
    ...
    else
    {
      // we are done:
      println(this.getClass().getSimpleName()
              + ".proceedWithMultiPass -> all passes done! finished!");
  ...
  private static float computePhase(int pNumberOfPasses,
                                    ...
                                    int pPassIndex)
  {
    final float lPhase = (((float) ((pPassIndex * pGenerator)
                                    % pNumberOfPasses)))
                         / pNumberOfPasses;
    return lPhase;
  }
