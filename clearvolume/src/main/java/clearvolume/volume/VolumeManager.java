...
public class VolumeManager implements ClearVolumeCloseable
{
  ...
  public VolumeManager(int pMaxAvailableVolumes)
  {
    ...
    mAvailableVolumesQueue =
                           new ArrayBlockingQueue<SoftReference<Volume>>(mMaxAvailableVolumes);
  }
  ...
  public Volume requestAndWaitForVolumeLike(int pTimeOut,
                                            ...
                                            Volume pVolume)
  {
    if (pVolume.getDataBuffer().remaining() == 2
                                               * pVolume.getBytesPerVoxel()
                                               * pVolume.getNumberOfVoxels()
        && pVolume.getNativeType() == NativeTypeEnum.UnsignedByte)
    {
                                        ...
                                        long... pDimensions)
  {
    do
    {
      try
      {
        ...
        lPolledVolumeReference =
                               mAvailableVolumesQueue.poll(pTimeOut,
                                                           pTimeUnit);
