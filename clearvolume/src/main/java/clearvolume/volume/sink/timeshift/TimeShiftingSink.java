                              ...
                              ClearVolumeCloseable
{
  ...
  private static final ExecutorService mSeekingExecutor =
                                                        Executors.newSingleThreadExecutor();
  private final SwitchableSoftReferenceManager<Volume> mSwitchableSoftReferenceManager;

  private final Object mLock = new Object();
  private HashMap<Integer, TreeMap<Long, SwitchableSoftReference<Volume>>> mChannelToVolumeListsMap =
                                                                                                    new HashMap<>();
  ...
  {
    super();
    mSwitchableSoftReferenceManager =
                                    new SwitchableSoftReferenceManager<>();
    mSoftMemoryHorizonInTimePointIndices =
                                         Math.min(pSoftMemoryHoryzonInTimePointIndices,
                                                  pHardMemoryHoryzonInTimePointIndices);
    mHardMemoryHorizonInTimePointIndices =
                                         Math.max(pHardMemoryHoryzonInTimePointIndices,
                                                  pSoftMemoryHoryzonInTimePointIndices);
    mCleanUpPeriodInTimePoints =
                               (long) (mSoftMemoryHorizonInTimePointIndices
                                       * cCleanUpRatio);
  }
  ...
  public void sendVolume(Volume pVolume)
  {
    synchronized (mLock)
    {
      ...
      TreeMap<Long, SwitchableSoftReference<Volume>> lTimePointIndexToVolumeMapReference =
                                                                                         mChannelToVolumeListsMap.get(lVolumeChannelID);
      ...
      mHighestTimePointIndexSeen =
                                 Math.max(mHighestTimePointIndexSeen,
  ...
  private Volume getVolumeToSend(int pVolumeChannelID)
  {
    synchronized (mLock)
    {
      final TreeMap<Long, SwitchableSoftReference<Volume>> lTimePointIndexToVolumeMap =
                                                                                      mChannelToVolumeListsMap.get(pVolumeChannelID);

      if (lTimePointIndexToVolumeMap.isEmpty())
        return null;

      Entry<Long, SwitchableSoftReference<Volume>> lIndexVolumeEntry =
                                                                     lTimePointIndexToVolumeMap.floorEntry(mHighestTimePointIndexSeen
                                                                                                           + mTimeShift);
      if (lIndexVolumeEntry == null)
        lIndexVolumeEntry =
                          lTimePointIndexToVolumeMap.ceilingEntry(mHighestTimePointIndexSeen
                                                                  + mTimeShift);
  ...
  private void cleanUpOldVolumes(long pTimePointIndex, int pChannelID)
  {
    synchronized (mLock)
    {
      ...
      final TreeMap<Long, SwitchableSoftReference<Volume>> lTimePointIndexToVolumeMap =
                                                                                      mChannelToVolumeListsMap.get(pChannelID);
      if (lTimePointIndexToVolumeMap.isEmpty())
      {
        mChannelToVolumeListsMap.remove(pChannelID);
        return;
      }
      final Long lLastTimePoint =
                                lTimePointIndexToVolumeMap.lastKey();

      Long lTimePoint = lLastTimePoint;
      while (lTimePoint != null
             && lTimePoint > lLastTimePoint
                             - mSoftMemoryHorizonInTimePointIndices)
      {
      ...
      while (lTimePoint != null
             && lTimePoint > lLastTimePoint
                             - mHardMemoryHorizonInTimePointIndices)
      {
        final SwitchableSoftReference<Volume> lSwitchableSoftReference =
                                                                       lTimePointIndexToVolumeMap.get(lTimePoint);
  ...
  public void close()
  {
    synchronized (mLock)
    {
      ...
      {
        final TreeMap<Long, SwitchableSoftReference<Volume>> lTimeLineTreeMap =
                                                                              lTimeLineForChannelEntry.getValue();

        for (final Map.Entry<Long, SwitchableSoftReference<Volume>> lTimePointEntry : lTimeLineTreeMap.entrySet())
        {
          final SwitchableSoftReference<Volume> lVolumeSoftReference =
                                                                     lTimePointEntry.getValue();
