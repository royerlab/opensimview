...
public class SwitchableSoftReferenceManager<T>
{
  ...
  public SwitchableSoftReferenceManager()
  {
      ...
      public void run()
      {
        while (true)
        {
          try
          {
            while (true)
            {
              Reference<? extends T> lReference =
                                                mReferenceQueue.poll();
              if (lReference == null)
                break;

              @SuppressWarnings("unchecked")
              SwitchableSoftReference<T> lSwitchableSoftReference =
                                                                  (SwitchableSoftReference<T>) lReference;

              Runnable lCleanUpRunnable =
                                        lSwitchableSoftReference.getCleanUpRunnable();
    ...
    Thread lCleanUpThread =
                          new Thread(lRunnable,
