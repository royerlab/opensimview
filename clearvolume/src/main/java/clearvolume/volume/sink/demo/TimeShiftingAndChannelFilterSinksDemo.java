...
public class TimeShiftingAndChannelFilterSinksDemo
{
  ...
  public void demo() throws InterruptedException, IOException
  {
    final ClearVolumeRendererInterface lClearVolumeRenderer =
                                                            ClearVolumeRendererFactory.newBestRenderer("TimeShift Demo",
                                                                                                       512,
                                                                                                       512,
    ...
    final TimeShiftingSink lTimeShiftingSink =
                                             new TimeShiftingSink(50,
                                                                  100);
    final ChannelFilterSink lChannelFilterSink =
                                               new ChannelFilterSink();

    final ClearVolumeTCPServerSink lClearVolumeTCPServer =
                                                         new ClearVolumeTCPServerSink(cNumberOfAvailableVolumes);
    ...
    for (int i = 0; i < lMaxVolumesSent; i++)
    {
      ...
      final int lDepth = (int) ((cDepth - 10)
                                * (1. * i / lMaxVolumesSent));
      ...
      for (int z = 0; z < 10 + lDepth; z++)
        for (int y = 0; y < cHeight / (1 + lChannel); y++)
          for (int x = 0; x < cWidth / (1 + lChannel); x++)
          {
            ...
            final byte lByteValue = (byte) (((byte) x ^ (byte) y
                                             ^ (byte) z));
