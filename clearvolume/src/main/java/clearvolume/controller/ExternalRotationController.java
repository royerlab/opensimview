...
public class ExternalRotationController extends
                                        QuaternionRotationControllerBase
                                        implements
                                        ...
                                        Runnable
{
  ...
  public void connectAsynchronouslyOrWait()
  {
    ...
    final Thread lConnectionThread =
                                   new Thread(lConnectionRunnable,
  ...
  public boolean connect()
  {
    try
    {
      ...
      mBufferedInputStreamFromServer =
  ...
  public void run()
  {
    final BufferedReader lBufferedInputStreamFromServer =
                                                        mBufferedInputStreamFromServer;
    while (mBufferedInputStreamFromServer != null
           && mBufferedInputStreamFromServer == lBufferedInputStreamFromServer)
    {
      String lReadLine;
      try
      {
  ...
  private void parseMessage(String pReadLine)
  {
    ...
    final float lQuaternionW =
                             Float.parseFloat(lSplittedSubString[0]);
    final float lQuaternionX =
                             Float.parseFloat(lSplittedSubString[1]);
    final float lQuaternionY =
                             Float.parseFloat(lSplittedSubString[2]);
    final float lQuaternionZ =
                             Float.parseFloat(lSplittedSubString[3]);

    // TODO: find some use for these:
    final float lAccelerationX =
                               Float.parseFloat(lSplittedSubString[4]);
    final float lAccelerationY =
                               Float.parseFloat(lSplittedSubString[5]);
    final float lAccelerationZ =
                               Float.parseFloat(lSplittedSubString[6]);
