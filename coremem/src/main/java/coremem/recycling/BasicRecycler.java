 ...
 * @param <P> Request type (P stands for request Parameter)
 * @author royer
 */
public class BasicRecycler<R extends RecyclableInterface<R, P>, P extends RecyclerRequestInterface> extends FreeableBase implements RecyclerInterface<R, P>, Freeable
{
   ...
   * available objects.
   *
   * @param pRecyclableFactory      factory capable of instanciating recyclables
   * @param pMaximumNumberOfObjects maximum number of objects (live + availables)
   */
  public BasicRecycler(final RecyclableFactoryInterface<R, P> pRecyclableFactory, final int pMaximumNumberOfObjects)
  {
    this(pRecyclableFactory, pMaximumNumberOfObjects / 2, pMaximumNumberOfObjects / 2, true);
  }
   ...
   * @param pAutoFree                        true -> free released/cleared objects.
   */
  public BasicRecycler(final RecyclableFactoryInterface<R, P> pRecyclableFactory, final int pMaximumNumberOfLiveObjects, final int pMaximumNumberOfAvailableObjects, final boolean pAutoFree)
  {
   ...
   * must be provided.
   *
   * @param pAvailableQueueWaitTime time
   * @param pTimeUnit               time unit
   */
  public void setAvailableQueueWaitingTime(long pAvailableQueueWaitTime, TimeUnit pTimeUnit)
  {
  ...
  @Override
  public long ensurePreallocated(final long pNumberofPrealocatedRecyclablesNeeded, final P pRecyclerRequest)
  {
    complainIfFreed();
    final long lNumberOfAvailableObjects = getNumberOfAvailableObjects();
    final long lNumberOfObjectsToAllocate = Math.max(0, pNumberofPrealocatedRecyclablesNeeded - lNumberOfAvailableObjects);
    long i = 1;
    try
    {
      for (; i <= lNumberOfObjectsToAllocate; i++)
      {
        ...
        if (lNewInstance == null) return i - 1;
   ...
   */
  @Override
  public R getOrWait(final long pWaitTime, final TimeUnit pTimeUnit, final P pRecyclerRequest)
  {
  ...
  @Override
  public R get(final boolean pWaitForLiveObjectToComeBack, final long pWaitTime, final TimeUnit pTimeUnit, final P pRecyclerRequest)
  {
    complainIfFreed();
    ...
    if (lRecyclable != null)
    {
      ...
      if (lRecyclable.isCompatible(pRecyclerRequest))
      {
        ...
        return addToLiveObjectQueue(pWaitForLiveObjectToComeBack, pWaitTime, pTimeUnit, lRecyclable);
      } else
      {
        // Got unlucky, first we trash this recyclable:
        if (mAutoFree && !lRecyclable.isFree()) lRecyclable.free();
    ...
    } else
    {
      ...
      try
      {
        ...
        if (pWaitForLiveObjectToComeBack) waitForFreeSpaceInLiveQueue(pWaitTime, pTimeUnit);
   ...
   * @param pTimeUnit
   */
  private void waitForFreeSpaceInLiveQueue(long pWaitTime, TimeUnit pTimeUnit)
  {
   ...
   * @return
   */
  private R addToLiveObjectQueue(final boolean pWait, final long pWaitTime, final TimeUnit pTimeUnit, R lRecyclable)
  {
    ...
    if (pWait)
    {
      ...
      {
        return addToLiveObjectQueue(pWait, pWaitTime, pTimeUnit, lRecyclable);
      }
    } else
    {
  ...
  private R retrieveFromAvailableObjectsQueue() throws InterruptedException
  {
    ...
    R lObject = mAvailableObjectsQueue.poll(mAvailableQueueWaitTime, mAvailableQueueTimeUnit);
    notifyListeners();
    return lObject;
  }
  ...
  private void addToAvailableObjectsQueue(R lRecyclable)
  {
    ...
    if (!mAvailableObjectsQueue.contains(lRecyclable))
      if (!mAvailableObjectsQueue.offer(lRecyclable)) if (mAutoFree && !lRecyclable.isFree()) lRecyclable.free();
  ...
  public long computeLiveMemorySizeInBytes()
  {
    ...
    for (R lRecyclable : mLiveObjectsQueue)
      if (!lRecyclable.isFree()) lMemorySizeInBytes += lRecyclable.getSizeInBytes();
  ...
  {
    R lRecyclable;
    while ((lRecyclable = mLiveObjectsQueue.poll()) != null) if (mAutoFree && !lRecyclable.isFree()) lRecyclable.free();
    notifyListeners();
  }
  ...
  {
    if (mRecyclerListeners.isEmpty()) return;
    ...
    for (RecyclerListenerInterface lRecyclerListener : mRecyclerListeners)
      lRecyclerListener.update(lNumberOfLiveObjects, lNumberOfAvailableObjects, lNumberOfFailedRequests);
  }
