...
public class NIOBuffersInterop
{
  ...
  {
    if (!pBuffer.isDirect()) throw new UnsupportedWrappingException("Cannot wrap a non-native NIO Buffer");

    final long lBufferAddress = getAddress(pBuffer);

    final OffHeapMemory lOffHeapMemory = new OffHeapMemory(pBuffer, lBufferAddress, pSizeInBytes);
    return lOffHeapMemory;
  }
  ...
  {
    return getByteBuffersForContiguousMemory(pContiguousMemory, 0, pContiguousMemory.getSizeInBytes());
  }
  ...
  )
  {
    ...
    int lEstimatedNumberOfChuncks = (int) (1 + (lLeftToAssign / lLargestByteBufferSizeInBytes));
    ArrayList<ByteBuffer> lListOfBuffers = new ArrayList<ByteBuffer>(lEstimatedNumberOfChuncks);

    while (lLeftToAssign > 0)
    {
      long lChunkSize = min(lLeftToAssign, lLargestByteBufferSizeInBytes);
      ByteBuffer lDirectByteBuffer = JNI.newDirectByteBuffer(lAddress, lChunkSize);
