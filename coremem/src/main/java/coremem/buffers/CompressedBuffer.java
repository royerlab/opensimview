...
public class CompressedBuffer extends ContiguousBuffer
{
  ...
  public void writeCompressedMemory(ContiguousMemoryInterface pContiguousMemory)
  {
    ...
    if (lBufferSize>cMaxBufferSize)
    {
      ...
      writeCompressedMemorySingleChunk(pContiguousMemory);
  ...
  long lMaxCompressedSize = pContiguousMemory.getSizeInBytes() + JBlosc.OVERHEAD;
    ...
    long lCompressedBufferLength = IBloscDll.blosc_compress_ctx(
              ...
              new NativeLong(pContiguousMemory.getSizeInBytes()),
              JNAInterop.getJNAPointer(pContiguousMemory),
              JNAInterop.getJNAPointer(getRemainingContiguousMemory()),
              new NativeLong(Math.min(remainingBytes(), lMaxCompressedSize)),
              mCodecName,
              new NativeLong(0),
              mNumThreads
              );
