 ...
 * @author royer
 */
public class LightSheetFastFusionEngine extends FastFusionEngine implements
                                                                 ...
                                                                 LoggingFeature
{
  ...
  private volatile boolean
      mRegistration =
      MachineConfiguration.get().getBooleanProperty("fastfuse.register", true);

  private volatile boolean
      mBackgroundSubtraction =
      MachineConfiguration.get()
                          .getBooleanProperty("fastfuse.backgroundsubtraction", false);

  private volatile boolean
      mDownscale =
      MachineConfiguration.get().getBooleanProperty("fastfuse.downscale", true);

  private volatile double
      mMemRatio =
      MachineConfiguration.get().getDoubleProperty("fastfuse.memratio", 0.8);
                                    ...
                                    int pNumberOfDetectionArms)
  {
    ...
    long
        lMaxMemoryInBytes =
        (long) (mMemRatio * pContext.getDevice().getGlobalMemorySizeInBytes());
                                                     ...
                                                     float[] lKernelSigmas)
  {
    if (isRegistration())
    {
      ...
      mRegistrationTask.getParameters()
                       .setZeroTransformMatrix(AffineMatrix.scaling(-1, 1, 1));
                                                   ...
                                                   float[] lKernelSigmas)
  {
    if (isRegistration())
    {
      ...
      mRegistrationTask =
          new RegistrationTask("C0L0blur", "C1L0blur", "C0L0d", "C1L0d", "C1L0reg");
      mRegistrationTask.getParameters()
                       .setZeroTransformMatrix(AffineMatrix.scaling(-1, 1, 1));
  ...
  @Override public void reset(boolean pCloseImages)
  {
    ...
    try
    {
      ...
      */
          ...
          && lStackMetaData.getVoxelDimZ() != null)
      {
        float
            lZAspectRatio =
            (float) (lStackMetaData.getVoxelDimZ() / lStackMetaData.getVoxelDimX());
  ...
  @Override public void notifyListenersOfNewScoreForComputedTheta(double pScore)
  {
  ...
  @Override public void notifyListenersOfNewScoreForUsedTheta(double pScore)
  {
    mVisualConsoleInterface.addPoint("Registration error",
                                     "error (actual)",
                                     false,
  ...
  @Override public void newComputedTheta(double[] pTheta)
  {
    double Tx = pTheta[0];
    double Ty = pTheta[1];
    double Tz = pTheta[2];
  ...
  @Override public void newUsedTheta(double[] pTheta)
  {
    double Tx = pTheta[0];
    double Ty = pTheta[1];
    double Tz = pTheta[2];
