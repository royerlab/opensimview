 ...
 * @author royer
 */
public class LightSheetFastFusionProcessor extends ClearCLStackProcessorBase implements
                                                                             ...
                                                                             LoggingFeature
{
  ...
  ConcurrentLinkedQueue<Triple<Integer, StackMetaData, ClearCLImage>>
      mFusedStackQueue =
      new ConcurrentLinkedQueue<>();

  private volatile StackInterface mFusedStack;

  private final Variable<Integer>
      mNumberOfRestartsVariable =
      new Variable<Integer>("NumberOfRestarts", 5);

  private final Variable<Integer>
      mMaxNumberOfEvaluationsVariable =
      new Variable<Integer>("MaxNumberOfEvaluations", 200);

  private final BoundedVariable<Double>
      mTranslationSearchRadiusVariable =
      new BoundedVariable<Double>("TranslationSearchRadius", 15.0);
  private final BoundedVariable<Double>
      mRotationSearchRadiusVariable =
      new BoundedVariable<Double>("RotationSearchRadius", 3.0);

  private final BoundedVariable<Double>
      mSmoothingConstantVariable =
      new BoundedVariable<Double>("SmoothingConstant", 0.05);

  private final Variable<Boolean>
      mTransformLockSwitchVariable =
      new Variable<Boolean>("TransformLockSwitch", true);

  private final Variable<Integer>
      mTransformLockThresholdVariable =
      new Variable<Integer>("TransformLockThreshold", 20);

  private final Variable<Boolean>
      mBackgroundSubtractionSwitchVariable =
                                             ...
                                             RecyclerInterface<StackInterface, StackRequest> pStackRecycler)
  {
    ...
    if (mEngine.isDownscale())
    {
          ...
          && pStack.getMetaData().getVoxelDimY() != null)
      {
        ...
        System.out.println("pStack.getMetaData().getVoxelDimX()" + pStack.getMetaData()
                                                                         .getVoxelDimX());
    ...
    if (mEngine.getRegistrationTask() != null)
    {
      try
      {
        ...
        if (getTransformLockSwitchVariable().get().booleanValue()
            && pStack.getMetaData().getValue(MetaDataOrdinals.TimePoint)
               > getTransformLockThresholdVariable().get().intValue())
        {
        ...
        mEngine.getRegistrationTask()
               .getParameters()
               .setNumberOfRestarts(getNumberOfRestartsVariable().get().intValue());
        ...
        mEngine.getRegistrationTask()
               .setSmoothingConstant(getSmoothingConstantVariable().get().doubleValue());
    ...
    if (pStack.getMetaData().hasEntry(MetaDataFusion.RequestPerCameraFusion))
    {
      ...
      Triple<Integer, StackMetaData, ClearCLImage>
          lImageFromQueue =
   ...
   * @return
   */
  @Deprecated protected StackInterface copyFusedStack(RecyclerInterface<StackInterface, StackRequest> pStackRecycler,
                                                      ...
                                                      String pChannel)
  {
    ...
    if (pChannel != null)
      mFusedStack.getMetaData().addEntry(MetaDataChannel.Channel, pChannel);
  ...
  private boolean isPassThrough(StackInterface pStack)
  {
    AcquisitionType
        lAcquisitionType =
        pStack.getMetaData().getValue(MetaDataAcquisitionType.AcquisitionType);

    if (lAcquisitionType == AcquisitionType.Interactive)
      return true;

    if (pStack.getMetaData().hasEntry(MetaDataFusion.RequestFullFusion))
      return false;
   ...
   */
  @Deprecated public LightSheetFastFusionEngine getEngine()
  {
    return mEngine;
  }
