 ...
 * @author royer
 */
public class LightSheetSimulationStackProvider extends StackCameraSimulationProviderBase implements
                                                                                         ...
                                                                                         LoggingFeature
{
  ...
  private ConcurrentHashMap<Integer, LightSheetQueue>
      mLightSheetStateQueuesMap =
                                           ...
                                           int pCameraIndex)
  {
    ...
    mDetectionArmDevice =
        mLightSheetMicroscope.getDevice(DetectionArmInterface.class, mCameraIndex);
  ...
  @Override protected void fillStackData(StackCameraSimulationQueue pQueue,
                                         ...
                                         StackInterface pStack)
  {
    synchronized (mLightSheetMicroscopeSimulator)
    {
      ...
      LightSheetMicroscopeQueue
          ...
          mLightSheetMicroscope.getPlayedQueueVariable().get();

      boolean
          lIsSharedLightSheetControl =
      ...
      for (int zi = 0, i = 0; zi < lQueueLength; zi++)
      {
        ...
        if (pKeepPlaneList.get(zi))
        {
          ...
          ContiguousMemoryInterface
              lImagePlane =
              lContiguousMemory.subRegion(lOffset, lCameraImage.getSizeInBytes());
  ...
  private void collectOpticalSwitchStateQueues(LightSheetMicroscopeQueue pLightSheetMicroscopeQueue)
  {
    mOpticalSwitchStateQueue =
        (LightSheetOpticalSwitchQueue) pLightSheetMicroscopeQueue.getDeviceQueue(
            mOpticalSwitch);
                                             ...
                                             int pLightSheetIndex)
  {
    ...
    mLightSheetStateQueuesMap.put(pLightSheetIndex,
                                  (LightSheetQueue) pLightSheetMicroscopeQueue.getDeviceQueue(
                                      lLightSheetDevice));
  }
  ...
  private void passDetectionParameters(int zi)
  {
    ...
    float
        z =
        mDetectionStateQueue.getQueuedValue(mDetectionArmDevice.getZFunction().get(),
                                          ...
                                          int zi)
  {
    ...
    float
        x =
        lSelectedLightSheetQueue.getQueuedValue(lLightSheet.getXFunction().get(),
    ...
    float
        y =
        lSelectedLightSheetQueue.getQueuedValue(lLightSheet.getYFunction().get(),
    ...
    float
        z =
        lSelectedLightSheetQueue.getQueuedValue(lLightSheet.getZFunction().get(),
    ...
    float
        alpha =
        lLightSheetQueue.getQueuedValue(lLightSheet.getAlphaFunction().get(),
    ...
    float
        beta =
        lLightSheetQueue.getQueuedValue(lLightSheet.getBetaFunction().get(),
    ...
    float
        height =
        lSelectedLightSheetQueue.getQueuedValue(lLightSheet.getHeightFunction().get(),
    ...
    float lLightSheetPower =
        ...
        lLightSheetQueue.getQueuedValue(lLightSheet.getPowerFunction().get(),
                                        lLightSheetQueue.getPowerVariable(),
                                        zi).floatValue();

    boolean
        lLightSheetSwitchedOn =
        mOpticalSwitchStateQueue.getQueuedBooleanValue(mOpticalSwitchStateQueue.getSwitchVariable(
            pLightSheetIndex), zi);

    LaserDeviceInterface
        lLaserDevice =
        mLightSheetMicroscope.getDevice(LaserDeviceInterface.class, 0);

    float
        lLaserPower =
        lLaserDevice.getCurrentPowerInMilliWattVariable().get().floatValue();

    float
        lEffectiveIlluminationIntensity =
        (lLightSheetSwitchedOn ? 1 : 0) * lLightSheetPower * lLaserPower;
