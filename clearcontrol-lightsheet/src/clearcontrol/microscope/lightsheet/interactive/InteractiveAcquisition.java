                                                                   ...
                                                                   LoggingFeature
{
  ...
  private final AcquisitionStateManager<InterpolatedAcquisitionState>
      mAcquisitionStateManager;

  private volatile InteractiveAcquisitionModes
  ...
  private final Variable<Boolean> mSyncDetectionArmsVariable, mSyncLightSheetsVariable,
  ...
  private ChangeListener<AcquisitionStateInterface<LightSheetMicroscopeInterface, LightSheetMicroscopeQueue>>
      mAcquisitionStateChangeListener;
   ...
   */
  @SuppressWarnings("unchecked") public InteractiveAcquisition(String pDeviceName,
                                                               LightSheetMicroscope pLightSheetMicroscope)
  {
    ...
    mExposureVariableInSeconds =
        new BoundedVariable<Double>(pDeviceName + "Exposure",
                                    0.0,
                                    0.0,
                                    Double.POSITIVE_INFINITY,
                                    0.0);

    mTriggerOnChangeVariable =
        new Variable<Boolean>(pDeviceName + "TriggerOnChange", false);

    mUseCurrentAcquisitionStateVariable =
        new Variable<Boolean>(pDeviceName + "UseCurrentAcquisitionState", false);

    Variable<Number>
        lMinVariable =
        mLightSheetMicroscope.getDevice(DetectionArmInterface.class, 0)
                             .getZVariable()
                             .getMinVariable();
    Variable<Number>
        lMaxVariable =
    ...
    mSyncLightSheetsAndDetectionArmsVariable =
        new Variable<Boolean>("Sync lightsheets and detection arms", false);
    ...
    for (int d = 0; d < lNumberOfDetectionArms; d++)
      if (d != 0)
      {
        ...
        if (pSync)
        {
          ...
          lDetectionArm.getZVariable().doNotSyncWith(lFirstDetectionArm.getZVariable());
      }
    ...
    for (int l = 0; l < lNumberOfLightSheets; l++)
      if (l != 0)
      {
        ...
        if (pSync)
        {
          ...
          lLightSheet.getAlphaInDegreesVariable()
                     .set(lFirstLightSheet.getAlphaInDegreesVariable().get());

          lLightSheet.getBetaInDegreesVariable()
                     .syncWith(lFirstLightSheet.getBetaInDegreesVariable());
          lLightSheet.getBetaInDegreesVariable()
                     .set(lFirstLightSheet.getBetaInDegreesVariable().get());
          ...
          for (int la = 0; la < lNumberofLaserLines; la++)
          {
            ...
            lLightSheet.getLaserOnOffArrayVariable(la)
                       .set(lFirstLightSheet.getLaserOnOffArrayVariable(la).get());
          }
    ...
    for (int d = 0; d < lNumberOfDetectionArms; d++)
      for (int l = 0; l < lNumberOfLightSheets; l++)
      {
        ...
        if (pSync)
        {
          ...
          lDetectionArm.getZVariable().doNotSyncWith(lLightSheet.getZVariable());
      }
  ...
  @Override public boolean loop()
  {
    try
    {
      ...
      for (int c = 0; c < getNumberOfCameras(); c++)
      {
        StackMetaData
            lMetaData =
            mQueue.getCameraDeviceQueue(c).getMetaDataVariable().get();
