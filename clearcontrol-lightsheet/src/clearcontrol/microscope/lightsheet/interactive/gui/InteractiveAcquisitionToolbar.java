...
public class InteractiveAcquisitionToolbar extends CustomGridPane
{
  ...
  public InteractiveAcquisitionToolbar(InteractiveAcquisition pInteractiveAcquisition)
  {
...

    {
      ...
      lStop.setOnAction((e) -> {
        InteractiveAcquisitionModes
            lCurrentAcquisitionMode =
...

    {
      ...
      lTypicalIntervalComboBox.valueProperty().addListener((c, o, n) -> {
        if (n != null)
          pInteractiveAcquisition.getLoopPeriodVariable().setAsync(n);
      });

      lIntervalTextField.getTextField().textProperty().addListener((c, o, n) -> {
        if (o != null && !o.equals(n))
          lTypicalIntervalComboBox.getSelectionModel().clearSelection();
      });
...

    {
      ...
      lTypicalExposuresComboBox.valueProperty().addListener((c, o, n) -> {
        if (n != null)
          pInteractiveAcquisition.getExposureVariable().setAsync(n);
      });

      lExposureTextField.getTextField().textProperty().addListener((c, o, n) -> {
        if (o != null && !o.equals(n))
          lTypicalExposuresComboBox.getSelectionModel().clearSelection();
      });
...

    {
      ...
      final int
          ...
          pInteractiveAcquisition.getLightSheetMicroscope()
                                 .getDevice(StackCameraDeviceInterface.class, 0)
                                 .getMaxWidthVariable()
                                 .get()
                                 .intValue();
      final int
      ...
      CameraResolutionGrid
          lGridPane =
          new CameraResolutionGrid(lButtonHandler, lMaxCameraWidth, lMaxCameraHeight);
...

    {
      LightSheetOpticalSwitch
          lDevice =
          pInteractiveAcquisition.getLightSheetMicroscope()
                                 .getDevice(LightSheetOpticalSwitch.class, 0);
      ...
      SwitchingDevicePanel
          lLightSheetOpticalSwitchPanel =
          new SwitchingDevicePanel(lDevice, lNames);
