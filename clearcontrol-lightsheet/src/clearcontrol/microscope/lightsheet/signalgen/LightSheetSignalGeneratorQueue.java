...

{
  ...
  private Variable<Integer>
      mSelectedLightSheetIndexVariable =
      new Variable<Integer>("SelectedLightSheetIndex", 0);

  private Measure mBeforeExposureMeasure, mExposureMeasure, mFinalMeasure;

  private final ConcurrentHashMap<DetectionArm, DetectionArmStaves>
      mDetectionArmToStavesMap =
      new ConcurrentHashMap<>();

  private final ArrayList<LightSheet> mLightSheetList = new ArrayList<LightSheet>();

  private final ConcurrentHashMap<LightSheet, LightSheetStaves>
      mLightSheetToStavesMap =
      new ConcurrentHashMap<>();
  private final ConcurrentHashMap<LightSheetOpticalSwitch, LightSheetOpticalSwitchStaves>
      mOpticalSwitchToStavesMap =
      new ConcurrentHashMap<>();

  private final Variable<Double>
      mTransitionDurationInSecondsVariable =
      new Variable<Double>("mTransitionDurationInSeconds", 0d);
  ...
  public LightSheetSignalGeneratorQueue(LightSheetSignalGeneratorDevice pLightSheetSignalGeneratorDevice)
  {
    ...
    mDelegatedQueue =
        pLightSheetSignalGeneratorDevice.getDelegatedSignalGenerator().requestQueue();
  ...
  public LightSheetSignalGeneratorQueue(LightSheetSignalGeneratorQueue pTemplateQueue)
  {
    ...
    mDelegatedQueue =
        getLightSheetSignalGeneratorDevice().getDelegatedSignalGenerator().requestQueue();
  ...
  {
    LightSheetOpticalSwitchStaves
        lLightSheetOpticalSwitchStaves =
        new LightSheetOpticalSwitchStaves(pLightSheetOpticalSwitchQueue, 0);
  ...
  private void update()
  {
    synchronized (this)
    {
      ...
      for (Entry<LightSheetOpticalSwitch, LightSheetOpticalSwitchStaves> lEntry : mOpticalSwitchToStavesMap
          .entrySet())
      {
