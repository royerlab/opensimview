 ...
 * @author royer
 */
@SuppressWarnings("javadoc") public class LightSheetStaves implements LoggingFeature
{
  private final static MachineConfiguration
      cCurrentMachineConfiguration =
      MachineConfiguration.get();

  private LightSheetQueue mLightSheetQueue;

  private final BoundedVariable<Double>
      mLineExposureInMicrosecondsVariable =
      new BoundedVariable<Double>("LineExposureInMicroseconds", 10.0);
  ...
  private ConstantStave mBeforeExposureXStave, mExposureXStave, mBeforeExposureBStave,
      mExposureBStave, mBeforeExposureWStave, mExposureWStave, mBeforeExposureLAStave,
      mExposureLAStave;
  ...
  private int mStaveXIndex, mStaveYIndex, mStaveZIndex, mStaveBIndex, mStaveWIndex,
      mStaveLAIndex, mStaveTIndex;
  ...
  public LightSheetStaves(LightSheetQueue pLightSheetQueue)
  {
    ...
    mStaveXIndex =
        cCurrentMachineConfiguration.getIntegerProperty(lPrefix + ".x.index", 2);

    mStaveYIndex =
        cCurrentMachineConfiguration.getIntegerProperty(lPrefix + ".y.index", 3);

    mStaveZIndex =
        cCurrentMachineConfiguration.getIntegerProperty(lPrefix + ".z.index", 4);

    mStaveBIndex =
        cCurrentMachineConfiguration.getIntegerProperty(lPrefix + ".b.index", 5);

    mStaveWIndex =
        cCurrentMachineConfiguration.getIntegerProperty(lPrefix + ".w.index", 6);

    mStaveLAIndex =
        cCurrentMachineConfiguration.getIntegerProperty(lPrefix + ".la.index", 7);

    mStaveTIndex =
        cCurrentMachineConfiguration.getIntegerProperty(lPrefix + ".t.index", 8 + 7);
  ...
  public void ensureStavesAddedToBeforeExposureMeasure(Measure pBeforeExposureMeasure)
  {
    ...
    mBeforeExposureXStave =
        pBeforeExposureMeasure.ensureSetStave(mStaveXIndex, mBeforeExposureXStave);

    mBeforeExposureYStave =
        pBeforeExposureMeasure.ensureSetStave(mStaveYIndex, mBeforeExposureYStave);

    mBeforeExposureZStave =
        pBeforeExposureMeasure.ensureSetStave(mStaveZIndex, mBeforeExposureZStave);

    mBeforeExposureBStave =
        pBeforeExposureMeasure.ensureSetStave(mStaveBIndex, mBeforeExposureBStave);

    mBeforeExposureWStave =
        pBeforeExposureMeasure.ensureSetStave(mStaveWIndex, mBeforeExposureWStave);

    mBeforeExposureLAStave =
        pBeforeExposureMeasure.ensureSetStave(mStaveLAIndex, mBeforeExposureLAStave);

    mBeforeExposureTStave =
        pBeforeExposureMeasure.ensureSetStave(mStaveTIndex, mBeforeExposureTStave);
  ...
  public void ensureStavesAddedToExposureMeasure(Measure pExposureMeasure)
  {
    ...
    for (int i = 0; i < mLightSheetQueue.getNumberOfLaserDigitalControls(); i++)
      setLaserDigitalTriggerStave(pExposureMeasure,
                                      i,
                                      mNonSIIluminationLaserTriggerStaveArray[i]);/**/
                     ...
                     Measure pFinalMeasure)
  {
    synchronized (this)
    {
      ...
      final double
          lFinalMeasureTimeInMicroseconds =
      ...
      final double
          lLineExposureTimeInMicroseconds =
          lReadoutTimeInMicroseconds + lExposureMeasureTimeInMicroseconds;
      ...
      UnivariateAffineFunction
          lHeightFunction =
          getLightSheet().getHeightFunction().get();
      ...
      final double
          lGalvoZOffset =
          galvoRotateZ(lGalvoYOffsetBeforeRotation, lGalvoZOffsetBeforeRotation);
      ...
      double
          ...
          getLightSheet().getWidthFunction()
                         .get()
                         .value(mLightSheetQueue.getWidthVariable().get().doubleValue());
      ...
      double
          ...
          (lOverscan - 1) / (2 * lOverscan) * lExposureMeasureTimeInMicroseconds;
      final double
      ...
      for (int i = 0; i < mLightSheetQueue.getNumberOfLaserDigitalControls(); i++)
      {
        final Variable<Boolean>
            lLaserBooleanVariable =
            mLightSheetQueue.getLaserOnOffArrayVariable(i);

        if (mLightSheetQueue.getSIPatternOnOffVariable(i).get())
        {

          final StructuredIlluminationPatternInterface
              lStructuredIlluminationPatternInterface =
              mLightSheetQueue.getSIPatternVariable(i).get();
          final StaveInterface
              lSIIlluminationLaserTriggerStave =
              lStructuredIlluminationPatternInterface.getStave(lMarginTimeRelativeUnits);
        ...
        else
        {
          ...
          mNonSIIluminationLaserTriggerStaveArray[i].setStop((float) (1.0f
                                                              - lMarginTimeRelativeUnits));
      ...
      double
          ...
          getLightSheet().getPowerFunction()
                         .get()
                         .value(mLightSheetQueue.getPowerVariable().get().doubleValue());

      if (mLightSheetQueue.getAdaptPowerToWidthHeightVariable().get())
      {
        double
            lWidthPowerFactor =
            getLightSheet().getWidthPowerFunction().get().value(lWidthValue);
  ...
  private <O extends StaveInterface> void setLaserDigitalTriggerStave(Measure pExposureMeasure,
                                                                   int pLaserLineIndex,
                                                                   O pStave)
  {

    final int
        lLaserDigitalLineIndex =
        MachineConfiguration.get()
                            .getIntegerProperty("device.lsm.lightsheet." + getLightSheet()
                                                    .getName() + ".ld" + pLaserLineIndex + ".index",
                                                8 + pLaserLineIndex);
  ...
  private double galvoRotateY(double pY, double pZ)
  {
    ...
    double
        lAlphaDegrees =
        mLightSheetQueue.getAlphaInDegreesVariable().get().doubleValue();
