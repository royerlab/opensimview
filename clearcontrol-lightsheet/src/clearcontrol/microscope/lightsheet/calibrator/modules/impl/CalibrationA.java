 ...
 * @author royer
 */
public class CalibrationA extends CalibrationPerLightSheetBase implements
                                                               ...
                                                               HasStateDescriptionPerLightSheet
{
  ...
  private BoundedVariable<Double>
      mAngleOptimisationRangeWidthVariable =
      new BoundedVariable<Double>("Angle range in degrees", 20.0, 0.0, 40.0, 1.0);
  ...
  private BoundedVariable<Integer>
      mNumberOfRepeatsVariable =
      new BoundedVariable<Integer>("Number of repeats", 4, 1, Integer.MAX_VALUE);

  private BoundedVariable<Integer>
      mNumberOfAnglesVariable =
      new BoundedVariable<Integer>("Number of angles", 32, 1, Integer.MAX_VALUE);
  ...
  private BoundedVariable<Double>
      mYRangeVariable =
      new BoundedVariable<Double>("Y range for testing", 1.0, 0.0, 1.0, 0.01);
  private BoundedVariable<Double>
      mZRangeVariable =
      new BoundedVariable<Double>("Z range for testing", 1.0, 0.0, 1.0, 0.01);
  ...
  private double calibrate(int pLightSheetIndex, int pNumberOfDetectionArmDevices)
  {
    ...
    for (int r = 0; r < lNumberOfRepeats; r++)
    {
      ...
      final double[]
          anglesM =
          focusA(pLightSheetIndex,
                 lMinA,
                 lMaxA,
                 (lMaxA - lMinA) / (lNumberOfAngles - 1),
                 -y,
                 z);
      ...
      if (lValid)
      {
        ...
        System.out.format(
            "Angle are not valid, we continue with next set of y values... \n");
    ...
    System.out.format(
        "Average alpha angle for all detection arms (assumes that the cameras are well aligned): %s \n",
        angle);

    UnivariateAffineFunction
        lUnivariateAffineFunction =
        new UnivariateAffineFunction(1, angle);
    ...
    try
    {
      ...
      if (lPlayQueueAndWait)
      {
        for (int i = 0; i < lNumberOfDetectionArmDevices; i++)
        {
          ...
          final Double
              lArgMax =
              mArgMaxFinder.argmax(lAList.toArray(), lAvgIntensityArray);
          ...
          if (lArgMax != null)
          {
            ...
            double
                lAmplitudeRatio =
                (lAvgIntensityList.max() - lAvgIntensityList.min())
  ...
  private double apply(int pLightSheetIndex)
  {
    ...
    Variable<UnivariateAffineFunction>
        lFunctionVariable =
    ...
    double
        lError =
        abs(lUnivariateAffineFunction.getSlope() - 1)
  ...
  @Override public String getStateDescription(int pLightSheetIndex)
  {
    ...
    UnivariateAffineFunction
        lUnivariateAffineFunction =
        lLightSheetDevice.getAlphaFunction().get();
