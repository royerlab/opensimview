 ...
 * @author royer
 */
public class CalibrationHP extends CalibrationPerLightSheetBase implements
                                                                CalibrationModuleInterface
{
  ...
  BoundedVariable<Integer>
      mNumberOfHSamplesVariable =
      new BoundedVariable<Integer>("Number of height samples", 6, 0, Integer.MAX_VALUE);
  ...
  public double calibrate(int pLightSheetIndex)
  {
    ...
    String
        lChartName =
        String.format(" D=%d, I=%d", lDetectionArmIndex, pLightSheetIndex);
                         ...
                         boolean pReturnIntensity)
  {
    try
    {
      ...
      if (lPlayQueueAndWait)
      {
        ...
        final double[]
            lRobustmaxIntensityArray =
            ImageAnalysisUtils.computePercentileIntensityPerPlane(lStack, 99);

        smooth(lRobustmaxIntensityArray, 1);

        String
            lChartName =
            String.format("Mode=%s, D=%d, I=%d, H=%g",
  ...
  private double apply(int pLightSheetIndex, int pDetectionArmIndex)
  {
    ...
    PolynomialFunction
        ...
        mHPFunctions.get(pLightSheetIndex, pDetectionArmIndex);

    Variable<PolynomialFunction>
        lCurrentHeightFunctionVariable =
