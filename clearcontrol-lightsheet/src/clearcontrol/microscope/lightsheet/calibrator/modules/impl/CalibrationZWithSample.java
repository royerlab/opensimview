...
/**
 * This is a fork of CalibrationZ goal of the test is to determine if calibration Z with
 * sample makes sense / works. If yes: TODO: Remove code duplication and clean up all
 * calibraiton modules
 *
 * @author royer Author: Robert Haase (http://haesleinhuepf.net) at MPI CBG
 * (http://mpi-cbg.de) December 2017
 */
public class CalibrationZWithSample extends CalibrationPerLightSheetBase implements
                                                                         ...
                                                                         HasStateDescriptionPerLightSheet
{
  ...
  private BoundedVariable<Double>
      mMaxDeltaZ =
      new BoundedVariable<Double>("Maximum DeltaZ", 30.0, 0.0, Double.MAX_VALUE, 0.1);
  private BoundedVariable<Double>
  ...
  private BoundedVariable<Double>
      mStoppingConditionErrorThreshold =
                            ...
                            boolean pAdjustDetectionZ)
  {
    ...
    info(
        "Starting to calibrate Z for lightsheet %d, with %d D samples, %d I samples, and a search amplitude of %g ",
        pLightSheetIndex,
        lNumberOfDSamples,
        lNumberOfISamples,
        pSearchAmplitude);
    ...
    double
        lMinDZ =
        lInterpolatedAcquisitionState.getControlPlaneZ(0); // Double.NEGATIVE_INFINITY;
    double
        lMaxDZ =
    ...
    for (double iz = lMinIZ; iz <= lMaxIZ; iz += lStepIZ)
    {
      ...
      final double[]
          dz =
          focusZ(pLightSheetIndex, lNumberOfDSamples, lMinDZ, lMaxDZ, lPerturbedIZ);
    ...
    for (int d = 0; d < mNumberOfDetectionArmDevices; d++)
    {
      ...
      BoundedVariable<Number>
          ...
          getLightSheetMicroscope().getDeviceLists()
                                   .getDevice(DetectionArmInterface.class, d)
                                   .getZVariable();
                          ...
                          double pIZ)
  {
    ...
    try
    {
      ...
      info(
          "Focussing for lightsheet %d at %g, with %d D samples, with Dz values within [%g,%g] \n",
          pLightSheetIndex,
          pIZ,
          pNumberOfDSamples,
          lMinDZ,
  ...
  public double apply(int pLightSheetIndex, boolean pAdjustDetectionZ)
  {
    ...
    if (abs(lSlope) > 0.00001 && !Double.isNaN(lSlope) && !Double.isNaN(lOffset))
    {
      lLightSheetDevice.getZFunction()
                       .get()
                       .composeWith(new UnivariateAffineFunction(lSlope, lOffset));
    ...
    else
    {
      if (abs(lSlope) <= 0.00001)
        ...
        warning("invalid slope or offset: (y= %g x + %g)", lSlope, lOffset);
  ...
  protected void adjustYFunctionScale(final LightSheetInterface lLightSheetDevice)
  {
    ...
    Double
        lXYRatio =
        lMachineConfiguration.getDoubleProperty("device.lsm.lighsheet.yzratio", 1.0);
  ...
  protected void applyDetectionZ(int pLightSheetIndex)
  {
    ...
    final DetectionArmInterface
        lDetectionArmDevice0 =
        getLightSheetMicroscope().getDeviceLists()
                                 .getDevice(DetectionArmInterface.class, 0);
    final DetectionArmInterface
        lDetectionArmDevice1 =
        getLightSheetMicroscope().getDeviceLists()
                                 .getDevice(DetectionArmInterface.class, 1);
  ...
  @Override public String getStateDescription(int pLightSheetIndex)
  {
    ...
    UnivariateAffineFunction
        ...
        lLightSheetDevice.getZFunction().get();
