 ...
 * @author royer
 */
public class CalibrationW extends CalibrationPerLightSheetBase implements
                                                               CalibrationModuleInterface,
                                                               HasStateDescriptionPerLightSheet
{
  private BoundedVariable<Integer>
      mNumberOfSamplesVariable =
      new BoundedVariable<Integer>("Number of samples", 32, 1, Integer.MAX_VALUE);
  ...
  private boolean calibrate(int pDetectionArmIndex)
  {
    ...
    for (int lLightSheetIndex = 0; lLightSheetIndex
                                   < lNumberOfLightSheets; lLightSheetIndex++)
    {
      double[]
          lAverageIntensities =
          calibrate(lLightSheetIndex, pDetectionArmIndex, mNumberOfSamplesVariable.get());
                             ...
                             int pNumberOfSamples)
  {
    ...
    try
    {
      ...
      @SuppressWarnings("unused") UnivariateAffineFunction
          lWFunction =
          lLightSheetDevice.getWidthFunction().get();
      ...
      String
          lChartName =
          String.format("D=%d, I=%d", pDetectionArmIndex, pLightSheetIndex);
  ...
  public double apply()
  {
    ...
    for (double lIntensity = lIntensityMin; lIntensity <= lIntensityMax;
         lIntensity += lStep)
    {
      for (int lLightSheetIndex = 0; lLightSheetIndex
                                     < lNumberOfLightSheets; lLightSheetIndex++)
                                   ...
                                   < lNumberOfLightSheets; lLightSheetIndex++)
    {
      double
          lMedianOffset =
          StatUtils.percentile(lOffsetsLists[lLightSheetIndex].toArray(), 50);
                                   ...
                                   < lNumberOfLightSheets; lLightSheetIndex++)
    {
      LightSheetInterface
          lLightSheetDevice =
          getLightSheetMicroscope().getDeviceLists()
                                   .getDevice(LightSheet.class, lLightSheetIndex);
