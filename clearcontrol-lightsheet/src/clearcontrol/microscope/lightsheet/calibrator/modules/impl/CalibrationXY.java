 ...
 * @author royer
 */
public class CalibrationXY extends CalibrationPerLightSheetBase implements
                                                                ...
                                                                HasStateDescriptionPerLightSheet
{
  ...
  private MultiKeyMap<Integer, Vector2D> mOriginFromX, mUnitVectorFromX, mOriginFromY,
      mUnitVectorFromY;
  ...
  BoundedVariable<Integer>
      mNumberOfPointsVariable =
      new BoundedVariable<Integer>("Number of points", 3, 0, Integer.MAX_VALUE);
                            ...
                            int pNumberOfPoints)
  {
    boolean
        result =
        calibrate(pLightSheetIndex, pDetectionArmIndex, pNumberOfPoints, true)
        && calibrate(pLightSheetIndex, pDetectionArmIndex, pNumberOfPoints, false);
    if (!result)
    {
      setConfigurationState(pLightSheetIndex, ConfigurationState.FAILED);
    }
                            ...
                            boolean pDoAxisX)
  {
    ...
    try
    {
      ...
      for (double f = 0.5 * lMaxAbsY; f <= 0.7 * lMaxAbsY;
           f += (0.2 * lMaxAbsY / (pNumberOfPoints - 1)))
      {
                                                       ...
                                                       TimeoutException
  {
    ...
    Vector2D
        lNormalizedPoint =
        new Vector2D(2 * lPoint.getX() / lWidth, 2 * lPoint.getY() / lHeight);
  ...
  @Override public String getStateDescription(int pLightSheetIndex)
  {
    ...
    UnivariateAffineFunction
        lUnivariateAffineFunctionX =
        lLightSheetDevice.getXFunction().get();
    UnivariateAffineFunction
        lUnivariateAffineFunctionY =
        lLightSheetDevice.getYFunction().get();
