...
public class LightSheetTimelapseToolbar extends TimelapseToolbar implements LoggingFeature
{
  ...
  private File
      mProgramTemplateDirectory =
      MachineConfiguration.get().getFolder("ProgramTemplates");
  ...
  public LightSheetTimelapseToolbar(LightSheetTimelapse pLightSheetTimelapse)
  {
...

    {
      ...
      lRow = 10;
      {
        ComboBox lExistingScheduleTemplates;
        {
          ...
          lLoadScheduleTemplateBytton.setOnAction((e) -> {
            try
            {
              ...
              new ProgramReader(lSchedulerList,
...

        {
          ...
          Variable<String>
              lFileNameVariable =
              new Variable<String>("filename", "Program");
          ...
          lSaveProgramButton.setOnAction((e) -> {
            try
            {
              new ProgramWriter(mLightSheetTimelapse.getCurrentProgram(),
      ...
      String[]
          lFilters =
          { "Acquisition:", "Adaptation:", "Adaptive optics:", "Filter wheel:", "Fusion:",
      ...
      lRow = 7;
      {
        ...
        TreeItem<String>
            rootItem =
            buildInstructionTree(pLightSheetTimelapse, lFilters, "", lIcons);
          ...
          @Override public void handle(MouseEvent mouseEvent)
          {
            if (mouseEvent.getClickCount() == 2)
            {
              ...
              if (item.getParent() != null
                  && item.getParent().getValue().compareTo("Instructions") != 0)
              {
                int
                    lSelectedIndexInMainList =
                ...
                mCurrentProgramScheduleListView.setItems(FXCollections.observableArrayList(
                    lSchedulerList));
                if (mCurrentProgramScheduleListView.getSelectionModel().getSelectedIndex()
                    > -1)
                {
                  mCurrentProgramScheduleListView.getSelectionModel()
...

    {
      ...
      AdaptiveEngine
          lAdaptiveEngine =
          (AdaptiveEngine) lMicroscopeInterface.getDevice(AdaptiveEngine.class, 0);
      ...
      if (lAdaptiveEngine != null)
      {
        ...
        TitledPane
            lTitledPane =
            new TitledPane("Adaptation state", lConfigurationStatePanel);
        lTitledPane.setAnimated(false);
    ...
    if (mCurrentProgramScheduleListView.getSelectionModel().getSelectedIndex() > -1)
    {
      InstructionInterface
          ...
          lSchedulerList.get(mCurrentProgramScheduleListView.getSelectionModel()
                                                            .getSelectedIndex());
      System.out.println("Selected: " + lSchedulerList.get(mCurrentProgramScheduleListView
                                                               .getSelectionModel()
                                                               .getSelectedIndex()));
      try
      {
        ...
        String
            ...
            lInstructionClass.getPackage().getName()
            + ".gui."
  ...
  @NotNull private TreeItem<String> buildInstructionTree(LightSheetTimelapse pLightSheetTimelapse,
                                                         String[] lFilters,
                                                         ...
                                                         Node[] lIcons)
  {
    TreeItem<String>
        rootItem =
        new TreeItem<String>("Instructions", MicroscopeNodeType.Other.getIcon());
    rootItem.setExpanded(true);
    for (int i = 0; i < lFilters.length; i++)
    {
      ArrayList<InstructionInterface>
          lAvailableSchedulersList =
          pLightSheetTimelapse.getListOfAvailableSchedulers(lFilters[i], pSearchFilter);
      if (lAvailableSchedulersList.size() > 0)
      {

        TreeItem<String>
            item =
            new TreeItem<String>(lFilters[i].replace(":", ""), lIcons[i]);
