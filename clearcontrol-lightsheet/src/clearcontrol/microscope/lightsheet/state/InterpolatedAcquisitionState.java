 ...
 * @author royer
 */
public class InterpolatedAcquisitionState extends AcquisitionStateBase<LightSheetMicroscopeInterface, LightSheetMicroscopeQueue> implements
...

{
  ...
  private final BoundedVariable<Number>
      mZStepVariable =
      new BoundedVariable<Number>("ZStep", 1, 0, 1000);

  private final Variable<Number>
      mNumberOfZPlanesVariable =
      new Variable<Number>("ZPlanes", 1);
   ...
   */
  @SuppressWarnings("unchecked") public InterpolatedAcquisitionState(String pName,
                                                                     LightSheetMicroscopeInterface pLightSheetMicroscope)
  {
        ...
        mZLowVariable =
            new BoundedVariable<Number>("LowZ",
                                        lDetectionArm.getZVariable()
                                                     .getMin()
                                                     .doubleValue(),
                                        lDetectionArm.getZVariable().getMin(),
                                        lDetectionArm.getZVariable().getMax());
        mZHighVariable =
...

    {
      mInterpolationTables =
          new InterpolationTables(mNumberOfDetectionArms, mNumberOfLightSheets);
    ...
    {
      mLightSheetOnOff[i] =
          new Variable<Boolean>(String.format("LightSheet%dOnOff", i), true);
    ...
    if (lMainXYZRStage != null)
    {
      ...
      getStageXVariable().setMinMax(lMainXYZRStage.getMinPositionVariable(0).get(),
                                    lMainXYZRStage.getMaxPositionVariable(0).get());

      getStageYVariable().setMinMax(lMainXYZRStage.getMinPositionVariable(1).get(),
                                    lMainXYZRStage.getMaxPositionVariable(1).get());

      getStageZVariable().setMinMax(lMainXYZRStage.getMinPositionVariable(2).get(),
                                    lMainXYZRStage.getMaxPositionVariable(2).get());
    }
    else
    {
  ...
  {
    double
        lPixelSizeInMicrons = getMicroscope().getDetectionArm(0).getPixelSizeInMicrometerVariable().get().doubleValue();
  ...
  @Override public InterpolatedAcquisitionState duplicate(String pName)
  {
                                 ...
                                 ControlPlaneLayout pControlPlaneLayout)
  {
    ...
    for (int cpi = 0; cpi < pNumberOfControlPlanes; cpi++)
    {
      ...
      double
          lAbsoluteZ =
          pControlPlaneLowZ + lNormalizedZ * (pControlPlaneHighZ - pControlPlaneLowZ);
  ...
  public void copyCurrentMicroscopeSettings()
  {
    ...
    for (int cpi = 0; cpi < lNumberOfControlPlanes; cpi++)
    {
      for (int l = 0; l < mNumberOfLightSheets; l++)
      {
        double
            lHeight =
            getMicroscope().getLightSheet(l).getHeightVariable().get().doubleValue();
        double
            lPower =
            getMicroscope().getLightSheet(l).getPowerVariable().get().doubleValue();
  ...
  @Override public void prepareAcquisition(long pTimeOut, TimeUnit pTimeUnit)
  {
  ...
  @Override public LightSheetMicroscopeQueue getQueue(int pCameraIndexMin,
                                                      int pCameraIndexMax,
                                                      ...
                                                      int pExtendedDepthOfFieldSliceCount)
  {
    ...
    int
        lEDFRelativeMaxSliceDistance =
        pExtendedDepthOfFieldSliceCount + lEDFRelativeMinSliceDistance;

    for (int lIndex = 0; lIndex < lStackDepthInPlanes; lIndex++)
    {
      for (int lRelativeIndex = lIndex + lEDFRelativeMinSliceDistance; lRelativeIndex
                                                                       <= lIndex
  ...
  @Override public LightSheetMicroscopeQueue getQueue()
  {
                                                              ...
                                                              int pLaserLineIndexMax)
  {
    ...
    for (int la = pLaserLineIndexMin; la < pLaserLineIndexMax; la++)
    {
      pQueue.setILO(pLightSheetIndex,
                    la,
                    mLightSheetOnOff[pLightSheetIndex].get() && mLaserOnOff[la].get());
    }
  }
   ...
   */
  @Override public void applyStateAtControlPlane(LightSheetMicroscopeQueue pQueue,
  ...
  public double getZRamp(int pPlaneIndex)
  {
    final double
        lZ =
        mZLowVariable.get().doubleValue() + pPlaneIndex * getStackZStepVariable().get()
                                                                                 .doubleValue();
    return lZ;
  }
  ...
  public double get(LightSheetDOF pDOF, int pPlaneIndex, int pDeviceIndex)
  {
    ...
    final double
        lInterpolatedValue =
        mInterpolationTables.getInterpolated(pDOF, pDeviceIndex, lRamp);
   ...
   */
  @Override public InterpolationTables getInterpolationTables()
  {
  ...
  @Override public Variable<Boolean> getCameraOnOffVariable(int pLightSheetIndex)
  {
    return mCameraOnOff[pLightSheetIndex];
  }

  @Override public Variable<Boolean> getLightSheetOnOffVariable(int pLightSheetIndex)
  {
    return mLightSheetOnOff[pLightSheetIndex];
  }

  @Override public Variable<Boolean> getLaserOnOffVariable(int pLaserLineIndex)
  {
