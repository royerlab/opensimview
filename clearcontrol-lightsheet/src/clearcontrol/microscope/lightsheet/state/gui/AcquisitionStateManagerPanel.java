...
public class AcquisitionStateManagerPanel extends AcquisitionStateManagerPanelBase<InterpolatedAcquisitionState>
{
  ...
  private ObservableList<InterpolatedAcquisitionState>
  ...
  public AcquisitionStateManagerPanel(AcquisitionStateManager<InterpolatedAcquisitionState> pAcquisitionStateManager)
  {
    ...
    SingleCheckCellManager<InterpolatedAcquisitionState>
        lSingleCheckCellManager =
        new SingleCheckCellManager<InterpolatedAcquisitionState>()
        {
          @Override public void checkOnlyCell(SingleCheckCell<InterpolatedAcquisitionState> pSelectedCell)
          {
    ...
    mStateListView =
        new SingleCheckListView<>(lSingleCheckCellManager, mObservableStateList);

    mStateListView.getSelectionModel()
                  .getSelectedItems()
                  .addListener(new ListChangeListener<AcquisitionStateInterface<?, ?>>()
                  {
                    @Override public void onChanged(ListChangeListener.Change<? extends AcquisitionStateInterface<?, ?>> change)
                    {
                      AcquisitionStateInterface<?, ?>
                          lAcquisitionStateInterface =
    ...
    contextMenu.getItems()
               .addAll(lNewItem, lDuplicateItem, lDeleteItem, lDeleteOthersItem);
    ...
    lNewItem.setOnAction((e) -> {
      ...
      InterpolatedAcquisitionState
          ...
          new InterpolatedAcquisitionState("new", lMicroscope);
    ...
    lDuplicateItem.setOnAction((e) -> {
      ...
      InterpolatedAcquisitionState
          lSelectedItem =
          mStateListView.getSelectionModel().getSelectedItem();
      ...
      if (lSelectedItem instanceof InterpolatedAcquisitionState)
      {
        ...
        InterpolatedAcquisitionState
            lInterpolatedAcquisitionState =
            new InterpolatedAcquisitionState(lNewName, lOriginalState);
        pAcquisitionStateManager.addState(lInterpolatedAcquisitionState);
  ...
  @Override protected void updateStateList(List<InterpolatedAcquisitionState> pStateList)
  {
    Runnable lRunnable = () -> {
