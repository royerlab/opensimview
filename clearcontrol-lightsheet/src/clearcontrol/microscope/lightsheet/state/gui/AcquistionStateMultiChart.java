...
public class AcquistionStateMultiChart extends MultiChart
{
  ...
  private ConcurrentHashMap<String, ObservableList<Data<Number, Number>>>
      mNameToDataMap =
      ...
      for (int d = 0; d < lNumberOfDetectionArms; d++)
      {
        ...
        for (int zi = 0; zi < lDepth; zi++)
        {
          MultiChart.addData(lData,
                             ...
                             pAcquisitionState.get(LightSheetDOF.DZ, zi, d));
        }
      }
        ...
        if (lLightSheetDOF != LightSheetDOF.DZ)
        {
          for (int i = 0; i < lNumberOfIlluminationArms; i++)
          {
            ObservableList<Data<Number, Number>>
                lData =
                mNameToDataMap.get(lLightSheetDOF.toString() + i);
            lData.clear();
            for (int zi = 0; zi < lDepth; zi++)
            {
              MultiChart.addData(lData,
                                 pAcquisitionState.getZRamp(zi),
                                 pAcquisitionState.get(lLightSheetDOF, zi, i));
            }
          }
        }
