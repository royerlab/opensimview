 ...
 * 2018
 */
public class SequentialAcquisitionInstruction extends AbstractAcquistionInstruction implements
                                                                                    ...
                                                                                    LoggingFeature
{
  ...
  @Override public boolean enqueue(long pTimePoint)
  {
      ...
      if (isLightSheetOn(l))
      {
        ...
        for (int c = 0; c < lNumberOfDetectionArms; c++)
        {
          if (isCameraOn(c))
          {
            ...
            StackMetaData
                lMetaData =
                lQueueForView.getCameraDeviceQueue(c).getMetaDataVariable().get();
        ...
        for (int d = 0; d < getLightSheetMicroscope().getNumberOfDetectionArms(); d++)
        {
          if (isCameraOn(d))
          {
            StackInterface
                lStack =
                getLightSheetMicroscope().getCameraStackVariable(d).get();
    ...
    getLightSheetMicroscope().getDataWarehouse()
                             .put("sequential_raw_" + pTimePoint, lContainer);

    return true;
  }

  protected LightSheetMicroscopeQueue getQueueForSingleLightSheet(
      LightSheetAcquisitionStateInterface<?> pCurrentState,
      int pLightSheetIndex)
  {
    ...
    int
        ...
        mCurrentState.getNumberOfZPlanesVariable().get().intValue();
    ...
    double
        ...
        mCurrentState.getExposureInSecondsVariable().get().doubleValue();
  ...
  @Override public SequentialAcquisitionInstruction copy()
  {
