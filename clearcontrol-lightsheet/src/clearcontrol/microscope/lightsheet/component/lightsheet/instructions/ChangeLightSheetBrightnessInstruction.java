 ...
 */
public class ChangeLightSheetBrightnessInstruction extends LightSheetMicroscopeInstructionBase implements
                                                                                          PropertyIOableInstructionInterface
{
  ...
  private final BoundedVariable<Double>
          mLightSheetBrightness =
      new BoundedVariable<Double>("Light sheet brightness",
                                  0.0,
                                  0.0,
                                  1.0,
                                  0.005);

  public ChangeLightSheetBrightnessInstruction(LightSheetMicroscope pLightSheetMicroscope,
                                               int pLightSheetIndex,
                                               double pLightSheetBrightness)
  {
    ...
    for (int cpi = 0; cpi < lState.getNumberOfControlPlanes(); cpi++)
    {
      int lLightSheetIndex = mLightSheetIndex.get();
        lState.getInterpolationTables()
              .set(LightSheetDOF.IP, cpi, lLightSheetIndex, mLightSheetBrightness.get());
    }
    return true;
  }

  @Override public ChangeLightSheetBrightnessInstruction copy()
  {
    return new ChangeLightSheetBrightnessInstruction(getLightSheetMicroscope(),
                                                mLightSheetIndex.get(),
                                                mLightSheetBrightness.get());
  }
