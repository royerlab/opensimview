 ...
 * @author royer
 */
public class LightSheetQueue extends VariableQueueBase implements QueueInterface,
                                                                  LightSheetParameterInterface
{
  ...
  private final BoundedVariable<Number> mEffectiveExposureInSecondsVariable,
      mFinalisationTimeInSecondsVariable, mReadoutTimeInMicrosecondsPerLineVariable,
      mOverScanVariable, mXVariable, mYVariable, mZVariable, mAlphaInDegreesVariable,
      mBetaInDegreesVariable, mWidthVariable, mHeightVariable, mPowerVariable;
  ...
  private final Variable<StructuredIlluminationPatternInterface>[]
   ...
   */
  @SuppressWarnings("unchecked") public LightSheetQueue(LightSheet pLightSheet)
  {
    ...
    mEffectiveExposureInSecondsVariable =
        new BoundedVariable<Number>(lNamePrefix + "-EffectiveExposureInSeconds", 0.005);

    mFinalisationTimeInSecondsVariable =
        new BoundedVariable<Number>(lNamePrefix + "-FinalizationTimeInSeconds", 0.000005);

    mImageHeightVariable =
        new BoundedVariable<Long>(lNamePrefix + "-ImageHeight", 2 * 1024L);
    mReadoutTimeInMicrosecondsPerLineVariable =
        new BoundedVariable<Number>(lNamePrefix + "-ReadoutTimeInMicrosecondsPerLine",
                                    9.74);
    ...
    mAlphaInDegreesVariable =
        new BoundedVariable<Number>(lNamePrefix + "-LightSheetAlphaInDegrees", 0.0);
    mBetaInDegreesVariable =
        new BoundedVariable<Number>(lNamePrefix + "-LightSheetBetaInDegrees", 0.0);
    mWidthVariable =
        new BoundedVariable<Number>(lNamePrefix + "-LightSheetRange",
                                    0.45);
    ...
    mAdaptPowerToWidthHeightVariable =
        new Variable<Boolean>(lNamePrefix + "-AdaptLightSheetPowerToWidthHeight", false);
    ...
    for (int i = 0; i < mNumberOfLaserDigitalControls; i++)
    {
      ...
      mSIPatternVariableArray[i] =
          new Variable<StructuredIlluminationPatternInterface>(
              "StructuredIlluminationPattern",
      ...
      mSIPatternOnOffVariableArray[i] =
          new Variable<Boolean>(lLaserName + "SIPatternOnOff", false);
  ...
  public LightSheetQueue(LightSheetQueue pLightSheetQueueTemplate)
  {
    ...
    for (int i = 0; i < pLightSheetQueueTemplate.getNumberOfLaserDigitalControls(); i++)
    {
      ...
      mLaserOnOffVariableArray[i].set(pLightSheetQueueTemplate.getLaserOnOffArrayVariable(
          i).get());
      mSIPatternOnOffVariableArray[i].set(pLightSheetQueueTemplate.getSIPatternOnOffVariable(
          i).get());
    }
