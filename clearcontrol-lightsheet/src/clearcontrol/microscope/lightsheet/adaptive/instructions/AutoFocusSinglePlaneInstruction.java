 ...
 */
public class AutoFocusSinglePlaneInstruction extends LightSheetMicroscopeInstructionBase implements
                                                                                         LoggingFeature,
                                                                                         PropertyIOableInstructionInterface
{
  ...
  private BoundedVariable<Integer>
      mControlPlaneIndex =
      new BoundedVariable<Integer>("Control plane index", 5, 0, Integer.MAX_VALUE);
  private BoundedVariable<Integer>
      mDetectionArmIndex =
      new BoundedVariable<Integer>("Detection arm index", 0, 0, Integer.MAX_VALUE);
  ...
  @Override public boolean enqueue(long pTimePoint)
  {
    ...
    InstructionInterface[]
        lSchedulers =
        new InstructionInterface[] { lAlphaScheduler, lFocusScheduler, };
                                   ...
                                   < getLightSheetMicroscope().getNumberOfLightSheets(); lLightSheetIndex++)
    {
      for (int cpi = 0; cpi < lAcquisitionState.getNumberOfControlPlanes(); cpi++)
      {
        ...
        for (LightSheetDOF lDOF : new LightSheetDOF[] { LightSheetDOF.IZ,
                                                        LightSheetDOF.IA })
        {
          it.set(lDOF,
                 cpi,
                 lLightSheetIndex,
                 it.get(lDOF, mControlPlaneIndex.get(), lLightSheetIndex));
        }
      }
    }
  ...
  @Override public AutoFocusSinglePlaneInstruction copy()
  {
    AutoFocusSinglePlaneInstruction
        copied =
