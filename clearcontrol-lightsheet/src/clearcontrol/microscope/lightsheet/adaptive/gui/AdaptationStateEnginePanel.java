...
public class AdaptationStateEnginePanel extends CustomGridPane
{
  ...
  private Node buildDetectionArmCurrentStatePane(final int pDetectionArmIndex)
  {
    ...
    */
    Timeline timeline = new Timeline(new KeyFrame(Duration.millis(500), (ae) -> {
      lZLabel.setText("Z: " + formatNumber(mLightSheetMicroscope.getDetectionArm(
          pDetectionArmIndex).getZVariable().get().doubleValue()));
    ...
    TitledPane
        lTitledPane =
        new TitledPane("Detection arm " + pDetectionArmIndex + " state", lGridPane);
  ...
  private Node buildLightSheetCurrentStatePane(final int pLightSheetIndex)
  {
    ...
    TitledPane
        lTitledPane =
        new TitledPane("Light sheet " + pLightSheetIndex + " state", lGridPane);
    lTitledPane.setAnimated(false);
    lTitledPane.setExpanded(true);
    return lTitledPane;
  }
  ...
  private Tab buildCalibrationTab()
  {
    ...
    TitledPane
        lTitledPane =
        new TitledPane("Calibration state", lConfigurationStatePanel);
    lTitledPane.setAnimated(false);
    lTitledPane.setExpanded(true);
                                    ...
                                    GridPane pGridPane)
  {
    ...
    lCustomGridPane.add(buildVariableLabel("", pLightSheetInterface.getXFunction()),
                        0,
                        lRow++);
    lCustomGridPane.add(new Label("Y function"), 0, lRow++);
    lCustomGridPane.add(buildVariableLabel("", pLightSheetInterface.getYFunction()),
                        0,
                        lRow++);
    lCustomGridPane.add(new Label("Z function"), 0, lRow++);
    lCustomGridPane.add(buildVariableLabel("", pLightSheetInterface.getZFunction()),
                        0,
                        lRow++);
    lCustomGridPane.add(new Label("Alpha function"), 0, lRow++);
    lCustomGridPane.add(buildVariableLabel("", pLightSheetInterface.getAlphaFunction()),
                        0,
                        lRow++);
    lCustomGridPane.add(new Label("Beta function"), 0, lRow++);
    lCustomGridPane.add(buildVariableLabel("", pLightSheetInterface.getBetaFunction()),
                        0,
                        lRow++);
    lCustomGridPane.add(new Label("Width function"), 0, lRow++);
    lCustomGridPane.add(buildVariableLabel("", pLightSheetInterface.getWidthFunction()),
                        0,
                        lRow++);
    lCustomGridPane.add(new Label("Height function"), 0, lRow++);
    lCustomGridPane.add(buildVariableLabel("", pLightSheetInterface.getHeightFunction()),
                        0,
                        lRow++);
    lCustomGridPane.add(new Label("Power function"), 0, lRow++);
    lCustomGridPane.add(buildVariableLabel("", pLightSheetInterface.getPowerFunction()),
                        0,
                        lRow++);

    TitledPane
        lTitledPane =
        new TitledPane("Light sheet "
                       + pLightSheetInterface.getName()
                       + " calibration state", lCustomGridPane);
    lTitledPane.setAnimated(false);
    lTitledPane.setExpanded(true);
    // GridPane.setColumnSpan(lTitledPane, 3);
  ...
  private VariableLabel buildVariableLabel(String name, Variable variable)
  {
    ...
    {
      @Override public void setEvent(Object pCurrentValue, Object pNewValue)
      {
        Platform.runLater(new Runnable()
        {
          @Override public void run()
  ...
  private TitledPane buildModuleStatePanel(HasControlPlaneState pHasControlPlaneState)
  {
    ...
    TitledPane
        lTitledPane =
        new TitledPane("Adaptation " + lName + " state", lControlPlaneStatePanel);
    lTitledPane.setAnimated(false);
    lTitledPane.setExpanded(true);
