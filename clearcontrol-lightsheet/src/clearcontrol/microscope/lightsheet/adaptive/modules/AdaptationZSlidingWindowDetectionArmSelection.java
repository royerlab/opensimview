                                                                                    ...
                                                                                    AdaptationModuleInterface<InterpolatedAcquisitionState>
{
  ...
  private final BoundedVariable<Integer>
      mSlidingWindowWidthVariable =
      new BoundedVariable<Integer>("SlidingWindowWidth", 1, 0, Integer.MAX_VALUE);

  private final Variable<Boolean>
      mFirstAndLastControlPlaneZero =
      new Variable<Boolean>("pFirstAndLastControlPlaneZero", true);
  ...
  @Override protected void updateStateInternal(InterpolatedAcquisitionState pStateToUpdate,
                                               ...
                                               boolean pFlipCorrectionSign)
  {
    ...
    for (int cpi = 0; cpi < lNumberOfControlPlanes; cpi++)
    {
      ...
      info("Best detection arms for control plane " + cpi + ": " + Arrays.toString(
          selectedDetectionArms[cpi]));
    }
    ...
    for (int cpi = 0; cpi < lNumberOfControlPlanes; cpi++)
    {
      int
          lSlidingWindowEnd =
          Math.min(cpi + (int) (lSlidingWindowWidth * 0.5), lNumberOfControlPlanes - 1);
      ...
      {
        lSlidingWindowEnd =
            Math.min(lSlidingWindowStart + lSlidingWindowWidth - 1,
    ...
    for (int cpi = 0; cpi < lNumberOfControlPlanes; cpi++)
    {
      ...
      for (int l = 0; l < lNumberOfLightSheets; l++)
      {
        ...
        if (mFirstAndLastControlPlaneZero.get() && (cpi == 0
                                                    || cpi == lNumberOfControlPlanes - 1))
        {
