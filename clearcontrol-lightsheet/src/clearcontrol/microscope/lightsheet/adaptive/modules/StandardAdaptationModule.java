 ...
 * @author royer
 */
public abstract class StandardAdaptationModule extends NDIteratorAdaptationModule<InterpolatedAcquisitionState> implements
...

{
  ...
  private final Variable<Integer>
          mLaserLineVariable =
          new Variable<Integer>("LaserLine", 0);

  private final Variable<Integer>
      mNumberOfSamplesVariable =
      new Variable<Integer>("NumberOfSamples");

  private final Variable<Double>
        mProbabilityThresholdVariable =
        new Variable<Double>("ProbabilityThreshold");
  private final Variable<Double>
          mMaxCorrection =
          new Variable<Double>("MaxCorrection", 1.0);

  private final Variable<Double>
      mImageMetricThresholdVariable =
  ...
  private HashMap<Triple<Integer, Integer, Integer>, Result>
      mResultsMap =
      new HashMap<>();
  ...
  @Override public void reset()
  {
    ...
    int
        ...
        lAcquisitionState.getInterpolationTables().getNumberOfControlPlanes();
                                       ...
                                       final TDoubleArrayList pDOFValueList)
  {
    ...
    try
    {
      ...
      {
        mControlPlaneStates =
            new ConfigurationState[lLightsheetMicroscope.getNumberOfLightSheets()][pStackAcquisition
        ...
        for (int lLightSheetIndex = 0; lLightSheetIndex
                                       < lLightsheetMicroscope.getNumberOfLightSheets(); lLightSheetIndex++)
        {
          for (int lControlPlaneIndex = 0; lControlPlaneIndex
                                           < pStackAcquisition.getNumberOfControlPlanes(); lControlPlaneIndex++)
          {
      ...
      {
        final StackInterface
            lStackInterface =
            lLightsheetMicroscope.getCameraStackVariable(d).get();
      ...
      Runnable lRunnable = () -> {
        ...
        try
        {
          ...
          for (int pDetectionArmIndex = 0; pDetectionArmIndex
                                           < lNumberOfDetectionArmDevices; pDetectionArmIndex++)

          {
            ...
            Double
                lArgmax =
                lSmartArgMaxFinder.argmax(pDOFValueList.toArray(), lMetricArray);
            ...
            setResult(pControlPlaneIndex,
                      pLightSheetIndex,
                      pDetectionArmIndex,
                      Result.of(lArgmax, lMetricMax, lFitProbability));
  ...
  protected void setResult(int pControlPlaneIndex,
                           ...
                           Result pResult)
  {
    mResultsMap.put(Triple.of(pControlPlaneIndex, pLightSheetIndex, pDetectionArmIndex),
                    pResult);
  }
                                     ...
                                     boolean pFlipCorrectionSign)
  {
    ...
    int
        ...
        getAdaptiveEngine().getAcquisitionStateVariable().get().getNumberOfLightSheets();
                                                                     ...
                                                                     int l)
  {
    double
        lValue =
        pRelativeCorrection ?
        pStateToUpdate.getInterpolationTables().get(mLightSheetDOF, cpi, l) :
        0;
    ...
    if (cpi == 0)
    {
      ...
      double
          lValueAfter =
          pStateToUpdate.getInterpolationTables().get(mLightSheetDOF, cpi + 1, l);
  ...
  ArrayList<ConfigurationStateChangeListener>
      mConfigurationStateChangeListeners =
      new ArrayList<>();
  ...
  ArrayList<ControlPlaneStateListener>
      mControlPlaneStateListenerList =
