...
public class Mirao52LibraryDemo
{
  ...
  public void testOpenClose() throws InterruptedException
  {
    ...
    for (int i = 0; i < 100; i++)
    {
      ...
      Pointer<Double> lCommand = Pointer.pointerToDoubles(lCommandDoubleArray);
      assertTrue(Mirao52eLibrary.mroApplyCommand(lCommand, (byte) Mirao52eLibrary.MRO_TRUE, null) == Mirao52eLibrary.MRO_TRUE);
      Thread.sleep(100);
    }
    ...
    if (lMroOpen == Mirao52eLibrary.MRO_FALSE)
    {
      System.err.println("Could not close status=" + status.getByte());
      fail();
    }
  }
