...
public class Mirao52eDeformableMirror implements AutoCloseable
{
  ...
  public boolean open()
  {
    ...
    synchronized (mLock)
    {
      ...
      {
        System.err.println(Mirao52eDeformableMirror.class.getSimpleName() + ": Could not open status=" + lStatusByte);
        return false;
      }
      mIsOpen = true;

      if (mIsOpen) Runtime.getRuntime().addShutdownHook(new Thread()
      {
        @Override
        public void run()
        {
  ...
  public void close() throws IOException
  {
    ...
    synchronized (mLock)
    {
      ...
      {
        System.err.println(Mirao52eDeformableMirror.class.getSimpleName() + ": Could not close status=" + lStatusByte);
        fail();
      }
    }
  }
   ...
   * as a 2D image.
   *
   * @param pFullMatrixMirrorShapeVector flat vector of 8x8 double values as double[]
  ...
  public boolean sendFullMatrixMirrorShapeVector(double[] pFullMatrixMirrorShapeVector)
  {
    ...
    Mirao52eSpecifications.convertLayout(pFullMatrixMirrorShapeVector, lRawMirrorShapeDoubleArray);
   ...
   *
   * @param pRawMirrorShapeVector raw mirror shape
   * @return true if succeeded.
   */
   ...
   * as a 2D image.
   *
   * @param pFullMatrixMirrorShapeVectorDoubleBuffer flat vector of 8x8 double values as Pointer<Double>
   * @return true if succeeded.
   */
  public boolean sendFullMatrixMirrorShapeVector(Pointer<Double> pFullMatrixMirrorShapeVectorDoubleBuffer)
  {
    checkVectorDimensions(pFullMatrixMirrorShapeVectorDoubleBuffer, Mirao52eSpecifications.cMirao52FullMatrixMirrorShapeVectorLength);
   ...
   * every actuator on the mirror.
   *
   * @param pRawMirrorShapeVectorDoubleBuffer raw mirror shape as buffer of doubles
   * @return true if succeeded.
   */
  public boolean sendRawMirrorShapeVector(Pointer<Double> pRawMirrorShapeVectorDoubleBuffer)
  {
    synchronized (mLock)
    {
      try
      {
        checkVectorDimensions(pRawMirrorShapeVectorDoubleBuffer, Mirao52eSpecifications.cMirao52NumberOfActuators);

        final byte lMroApplyCommand = Mirao52eLibrary.mroApplyCommand(pRawMirrorShapeVectorDoubleBuffer, (byte) (mOutputTrigger ? Mirao52eLibrary.MRO_TRUE : Mirao52eLibrary.MRO_FALSE), null);

        final boolean lSuccess = lMroApplyCommand == Mirao52eLibrary.MRO_TRUE;
        return lSuccess;
      } catch (final Throwable e)
      {
        throw new Mirao52Exception("Excepion while sending mirror shape: '" + e.getLocalizedMessage() + "'", e);
      }
    }
  }
  ...
  {
    checkVectorDimensions(pSquareMirrorShapeVectorDoubleBuffer, Mirao52eSpecifications.cMirao52FullMatrixMirrorShapeVectorLength);
    ...
    Mirao52eSpecifications.convertLayout(pSquareMirrorShapeVectorDoubleBuffer, mRawMirrorShapeVector);
  ...
  private double[] addFlatCalibration(double[] pRawMatrixMirrorShapeVector)
  {
    ...
    if (mFlatCalibrationRawMatrix != null) for (int i = 0; i < Mirao52eSpecifications.cMirao52NumberOfActuators; i++)
