...
public class DirettoreDemo
{
  ...
  @Test
  public void testSinus() throws UnknownHostException, SocketException, InterruptedException
  {
    ...
    final IntBuffer lDeltaTimeBuffer = ByteBuffer.allocateDirect(lNumberOfMatrices * 4).order(ByteOrder.nativeOrder()).asIntBuffer();

    final ByteBuffer lSyncControlByteBuffer = ByteBuffer.allocateDirect(lNumberOfMatrices * 4).order(ByteOrder.nativeOrder());

    final IntBuffer lSyncControlShortBuffer = lSyncControlByteBuffer.asIntBuffer();

    final IntBuffer lNumberOfTimePointsBuffer = ByteBuffer.allocateDirect(lNumberOfMatrices * 4).order(ByteOrder.nativeOrder()).asIntBuffer();

    final ShortBuffer lMatrixBuffer = ByteBuffer.allocateDirect(lNumberOfMatrices * lNumberOfChannels * lNumberOfTimePoints * 2).order(ByteOrder.nativeOrder()).asShortBuffer();
    ...
    for (int m = 0; m < lNumberOfMatrices; m++)
    {
      ...
      {
        final double lValue = Math.sin((2 * Math.PI / lNumberOfTimePoints) * t * lFrequency);
    ...
    for (int i = 0; i < 50000; i++)
    {
      ...
      lDirettore.play(lDeltaTimeBuffer, lNumberOfTimePointsBuffer, lSyncControlShortBuffer, lNumberOfMatrices, lMatrixBuffer);
