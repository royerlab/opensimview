...
public class Direttore implements AutoCloseable
{
  ...
  public boolean open()
  {
    synchronized (mLockObject)
    {
      ...
      DirettoreLibrary.direttoreOpen(mTriggerFIFODepth, mMatrixFIFODepth, mFPGAReference, mErrorOut);

      final boolean lIsOpened = !(mError = reportError(mErrorOut));

      if (lIsOpened) Runtime.getRuntime().addShutdownHook(new Thread()
      {
        @Override
        public void run()
        {
  ...
  public boolean play(double pDeltaTimeInMicroSeconds, int pNumberOfTimePointsToPlay, int pSyncChannel, int pSyncMode, final int pNumberOfMatrices, ShortBuffer pMatricesBuffer)
  {
    synchronized (mLockObject)
    {
      final IntBuffer lDeltaTimeBuffer = ByteBuffer.allocateDirect(pNumberOfMatrices * 4).order(ByteOrder.nativeOrder()).asIntBuffer();

      final IntBuffer lNbTimePointsBuffer = ByteBuffer.allocateDirect(pNumberOfMatrices * 4).order(ByteOrder.nativeOrder()).asIntBuffer();

      final ByteBuffer lSyncControlByteBuffer = ByteBuffer.allocateDirect(pNumberOfMatrices * 4).order(ByteOrder.nativeOrder());
      ...
      return play(lDeltaTimeBuffer, lNbTimePointsBuffer, lSyncControlShortBuffer, pNumberOfMatrices, pMatricesBuffer);
    }
  }

  public boolean play(IntBuffer pDeltaTimeInTicks, IntBuffer pNumberOfTimePointsToPlay, IntBuffer pSyncControl, final int pNumberOfMatrices, ShortBuffer pMatricesBuffer)
  {
    synchronized (mLockObject)
    {
      if (mError) return false;
      ...
      play(lPointerToDeltaTimeInTicksBuffer, lPointerToNumberOfTimePointsToPlay, lPointerToSyncControlBuffer, pNumberOfMatrices, lPointerToMatricesBuffer);

      if (lPointerToDeltaTimeInTicksBuffer != null) lPointerToDeltaTimeInTicksBuffer.release();
      if (lPointerToNumberOfTimePointsToPlay != null) lPointerToNumberOfTimePointsToPlay.release();
      if (lPointerToSyncControlBuffer != null) lPointerToSyncControlBuffer.release();
      if (lPointerToMatricesBuffer != null) lPointerToMatricesBuffer.release();

      return true;/**/
    }
  }

  public boolean play(Pointer<Integer> pDeltaTimeInTicks, Pointer<Integer> pNumberOfTimePointsToPlay, Pointer<Integer> pSyncControl, final int pNumberOfMatrices, Pointer<Short> pMatricesBuffer)
  {
    synchronized (mLockObject)
    {
      if (mError) return false;
  ...
  private boolean reportError(Pointer<TD1> pErrorOut)
  {
    ...
    if (lError)
    {
      try
      {
        final LStrHandleStruct lLStrHandleStruct = lTd1.source().get().get();
