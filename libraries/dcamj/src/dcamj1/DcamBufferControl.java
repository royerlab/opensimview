...
public class DcamBufferControl extends DcamBase
{
  ...
  public DcamBufferControl(final DcamDevice pDcamDevice, final DcamAcquisition pDcamAcquisition)
  {
  ...
  {
    if (mAttachedDcamFrame == pDcamFrame) return true;
    ...
    {
      @SuppressWarnings("unchecked") Pointer<Byte> lPointerToIndividualBuffer = pDcamFrame.getPointerForSinglePlane(i);
  ...
  private boolean attachBuffersInternal(final long lNumberOfBuffers)
  {
    ...
    final IntValuedEnum<DCAMERR> lError = DcamapiLibrary.dcambufAttach(mDcamDevice.getHDCAMPointer(), pointerTo(mDCAMBUF_ATTACH));
    final boolean lSuccess = addErrorToListAndCheckHasSucceeded(lError);
    return lSuccess;
  }
  ...
  {
    final long lImageSizeInBytes = (long) mDcamDevice.getProperties().getPropertyValue(DCAMIDPROP.DCAM_IDPROP_BUFFER_FRAMEBYTES);
