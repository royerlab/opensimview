...
class DcamAquisitionRunnable implements Runnable
{
  ...
  public DcamAquisitionRunnable(DcamAcquisition pDcamAcquisition, final long pNumberOfFramesToCapture, final boolean pContinuousAcquisition, final boolean pStackAcquisition)
  {
  ...
  public void run()
  {
    ...
    try
    {
      if (mDcamAcquisition.mDebug) System.out.println("DcamJ(Runnable): Starting acquisition:");

      mTrueIfStarted = true;
      mDcamAcquisition.mAcquiredFrameIndex = 0;

      if (mStackAcquisition && mContinuousAcquisition) while (mStopContinousIfFalse)
      {
        runOnce();
      }
      else
    ...
    {
      if (!mStackAcquisition && mContinuousAcquisition) mDcamAcquisition.mDcamDevice.stop();
      mTrueIfStopped = true;
      if (mDcamAcquisition.mDebug) System.out.println("DcamJ(Runnable): stopping acquisition:");
  ...
  private void runOnce()
  {
    ...
    {
      if (mDcamAcquisition.mDebug) System.out.format("DcamJ(Runnable): Starting continuous acquisition \n");
    ...
    while (mStopIfFalse)
    {
      ...
      else if (mStackAcquisition) lDcamcapEventToWaitFor = DCAMWAIT_EVENT.DCAMCAP_EVENT_FRAMEREADYORSTOPPED;
      else lDcamcapEventToWaitFor = DCAMWAIT_EVENT.DCAMCAP_EVENT_FRAMEREADY;

      if (mDcamAcquisition.mDebug) System.out.print("DcamJ(Runnable): waitForEvent: before... ");

      int lWaitTimeout = 5;
      boolean lWaitSuccess = false;
      while (!lWaitSuccess && mStopIfFalse)
      {
        lWaitSuccess = (mDcamAcquisition.mDcamDevice.getDcamWait().waitForEvent(lDcamcapEventToWaitFor, lWaitTimeout));

      }
      if (mDcamAcquisition.mDebug) System.out.println("DcamJ(Runnable): ...after.");
      ...
      if (!lWaitSuccess)
      {
        if (!mDcamAcquisition.isExternalTriggering() && !mDcamAcquisition.isSoftwareTriggering())
        {
          ...
          System.err.format("DcamJ(Runnable): frame index = %d (local index = %d) out of %d frames to capture (%s acquisition)  \n", mDcamAcquisition.mAcquiredFrameIndex, lReceivedFrameIndexInBufferList, mNumberOfFramesToCapture, mStackAcquisition ? "stack" : "single plane");
      ...
      final long lDcamWaitEvent = mDcamAcquisition.mDcamDevice.getDcamWait().getEvent();
      ...
      {
        if (mDcamAcquisition.mDebug) System.out.println("DcamJ(Runnable): Received Stop Event");
        if (mStackAcquisition)
        {
          ...
          mDcamAcquisition.notifyListeners(mDcamAcquisition.mAcquiredFrameIndex, lAcquisitionTimeStampInNanoseconds, 0, lDcamFrame);
      ...
      if (!mStackAcquisition && lReceivedFrameReadyEvent)
      {
        ...
        while (lRingBufferFrameIndex < 0) lRingBufferFrameIndex += lNumberOfBuffers;
        ...
        {
          lDcamFrame = mDcamAcquisition.getBufferControl().getDcamFrameForIndex(lRingBufferFrameIndex);
          lDcamFrame.setIndex(mDcamAcquisition.mAcquiredFrameIndex);
          lDcamFrame.setTimeStampInNs(lAcquisitionTimeStampInNanoseconds);
          mDcamAcquisition.notifyListeners(lFrameIndex, lAcquisitionTimeStampInNanoseconds, lRingBufferFrameIndex, lDcamFrame);
          mDcamAcquisition.mAcquiredFrameIndex++;
      ...
      {
        if (mDcamAcquisition.mDebug) System.out.println("DcamJ(Runnable): Received frame ready Event");

        if (!mContinuousAcquisition && !mStackAcquisition && lReceivedFrameIndexInBufferList >= mNumberOfFramesToCapture - 1)
        {
          mStopIfFalse = false;
        }
