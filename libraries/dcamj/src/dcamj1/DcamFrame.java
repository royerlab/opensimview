...
public class DcamFrame
{
  ...
  public static DcamFrame requestFrame(final long pBytesPerPixel, final long pWidth, final long pHeight, final long pDepth)
  {
    DcamFrame lDcamFrame;
    do
    {
      lDcamFrame = mAvailableFramesQueue.poll();
    } while (lDcamFrame != null && (lDcamFrame.getPixelSizeInBytes() != pBytesPerPixel || lDcamFrame.getWidth() != pWidth || lDcamFrame.getHeight() != pHeight || lDcamFrame.getDepth() != pDepth));

    if (lDcamFrame == null)
    {
      lDcamFrame = new DcamFrame(pBytesPerPixel, pWidth, pHeight, pDepth);
    }
  ...
  public static void preallocateFrames(final long pNumberOfFramesToAllocate, final long pBytesPerPixel, final long pWidth, final long pHeight, final long pDepth)
  {
    clearFrames();
    for (int i = 0; i < pNumberOfFramesToAllocate; i++)
    {
      final DcamFrame lRequestedFrame = requestFrame(pBytesPerPixel, pWidth, pHeight, pDepth);
  ...
  public DcamFrame(final long pBytesPerPixel, final long pWidth, final long pHeight, final long pDepth)
  {
    ...
    {
      mPointerArray[i] = Pointer.allocateAlignedArray(byte.class, pBytesPerPixel * pWidth * pHeight, cPageAlignment);
      /*mByteBufferArray[i] = ByteBuffer.allocateDirect((int) (pBytesPerPixel * pWidth * pHeight))
      																.order(ByteOrder.nativeOrder());/**/
      mSinglePlaneDcamFrameArray[i] = new DcamFrame(getPointerForSinglePlane(i), getPixelSizeInBytes(), getWidth(), getHeight());
    }
  }

  @SuppressWarnings("unchecked")
  public DcamFrame(final Pointer<Byte> pSinglePlanePointer, final long pBytesPerPixel, final long pWidth, final long pHeight)
  {
    mBytesPerPixel = pBytesPerPixel;
    mWidth = pWidth;
    mHeight = pHeight;
  ...
  {
    return copyAllPlanesToSinglePointer(pDestinationPointer, mPointerArray.length);
  }

  public boolean copyAllPlanesToSinglePointer(final Pointer<Byte> pDestinationPointer, final long pMaxNumberOfPlanes)
  {
    final long lTotalSizeInBytes = getTotalSizeInBytesForAllPlanes();
    if (pDestinationPointer.getValidBytes() != lTotalSizeInBytes) return false;

    Pointer<Byte> lDestinationPointer = pDestinationPointer;

    for (int i = 0; i < Math.min(mPointerArray.length, pMaxNumberOfPlanes); i++)
    {
  ...
  {
    if (mSinglePlaneDcamFrameArray == null) return this;
    else return mSinglePlaneDcamFrameArray[Math.toIntExact(pIndex)];
  }
  ...
  {
    return String.format("DcamFrame [mBytesPerPixel=%s, mWidth=%s, mHeight=%s, mDepth=%s, mIndex=%s, mTimeStampInNs=%s]", mBytesPerPixel, mWidth, mHeight, mDepth, mIndex, mTimeStampInNs);
  }
}
