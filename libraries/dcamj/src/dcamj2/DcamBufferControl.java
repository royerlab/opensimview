...
public class DcamBufferControl extends DcamBase
{
   ...
   *
   * @param pNumberOfBuffers number of internal buffers to allocate
  ...
  public final boolean allocateInternalBuffers(final int pNumberOfBuffers)
  {
    ...
    final IntValuedEnum<DCAMERR> lError = DcamapiLibrary.dcambufAlloc(mDcamDevice.getHDCAMPointer(), pNumberOfBuffers);
  ...
  {
    if (mInternalDcamFramePointer == null) mInternalDcamFramePointer = Pointer.allocate(DCAM_FRAME.class);

    final IntValuedEnum<DCAMERR> lError = DcamapiLibrary.dcambufLockframe(mDcamDevice.getHDCAMPointer(), mInternalDcamFramePointer);
    final boolean lSuccess = addErrorToListAndCheckHasSucceeded(lError);
    if (!lSuccess)
    {
      return null;
  ...
  public final Pointer<DCAM_FRAME> copyFrame()
  {
    ...
    final IntValuedEnum<DCAMERR> lError = DcamapiLibrary.dcambufCopyframe(mDcamDevice.getHDCAMPointer(), mInternalDcamFramePointer);
    final boolean lSuccess = addErrorToListAndCheckHasSucceeded(lError);
    if (!lSuccess)
    {
      return null;
   ...
   *
   * @param pImageSequence Dcam frame
   * @return true: success, false otherwise.
   */
  public final boolean attachExternalBuffers(DcamImageSequence pImageSequence)
  {
    ...
    {
      if (mPointerToPointerArray != null) mPointerToPointerArray.release();
    ...
    {
      Pointer<Byte> lPointerToIndividualBuffer = pImageSequence.getPointerForPlane(i);
  ...
  private boolean attachBuffersInternal(final long lNumberOfBuffers)
  {
    ...
    @SuppressWarnings("deprecation") final IntValuedEnum<DCAMERR> lError = DcamapiLibrary.dcambufAttach(mDcamDevice.getHDCAMPointer(), pointerTo(mDCAMBUF_ATTACH));
    final boolean lSuccess = addErrorToListAndCheckHasSucceeded(lError);
    return lSuccess;
  }
   ...
   *
   * @param pNumberOfBuffers number of buffers
   * @return total required memory in bytes
   */
  public long computeTotalRequiredMemoryInBytes(int pNumberOfBuffers)
  {
    final long lImageSizeInBytes = (long) mDcamDevice.getProperties().getDoublePropertyValue(DCAMIDPROP.DCAM_IDPROP_BUFFER_FRAMEBYTES);
  ...
  {
    final IntValuedEnum<DCAMERR> lError = DcamapiLibrary.dcambufRelease(mDcamDevice.getHDCAMPointer(), 0);
    final boolean lSuccess = addErrorToListAndCheckHasSucceeded(lError);
