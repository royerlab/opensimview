 ...
 * @author royer
 */
public class DcamImageSequence implements RecyclableInterface<DcamImageSequence, DcamImageSequenceRequest>, SizedInBytes, Freeable
{
   ...
   * this constructor.
   *
   * @param pDcamDevice    device to use for acquisition, this is used to adjust height and
   *                       width
   ...
   * @param pDepth         depth
   */
  public DcamImageSequence(DcamDevice pDcamDevice, final long pBytesPerPixel, final long pWidth, final long pHeight, final long pDepth)
  {
   ...
   * @param pHeight        height
   * @param pDepth         depth
   * @param pFragmented    true: allocates multiple independent buffers, false: allocates a
   *                       single contiguous buffer
   */
  public DcamImageSequence(DcamDevice pDcamDevice, final long pBytesPerPixel, final long pWidth, final long pHeight, final long pDepth, boolean pFragmented)
  {
    ...
    if (pFragmented)
    {
      ...
      for (int i = 0; i < mDepth; i++)
      {
        ...
        OffHeapMemory lAllocatedMemory = OffHeapMemory.allocateAlignedBytes("DcamImageSequence" + i, lNumberOfBytes, cPageAlignment);
        mFragmentedMemory.add(lAllocatedMemory);
      }
    } else
    {
      long lNumberOfBytes = pBytesPerPixel * mWidth * mHeight * mDepth;
      OffHeapMemory lAllocatedMemory = OffHeapMemory.allocateAlignedBytes("DcamImageSequence", lNumberOfBytes, cPageAlignment);
      mFragmentedMemory = FragmentedMemory.split(lAllocatedMemory, mDepth);
    }
   ...
   * @param pDcamDevice       device to use for acquisition, this is used to adjust height and
   *                          width
   * @param pFragmentedMemory fragmented memory object
   * @param pBytesPerPixel    bytes per pixel
   * @param pWidth            width
   * @param pHeight           height
   * @param pDepth            depth
   */
  public DcamImageSequence(DcamDevice pDcamDevice, final FragmentedMemoryInterface pFragmentedMemory, final long pBytesPerPixel, final long pWidth, final long pHeight, final long pDepth)
  {
    mDcamDevice = pDcamDevice;
    mBytesPerPixel = pBytesPerPixel;
    mWidth = pDcamDevice.adjustWidthHeight(pWidth, 4 / pDcamDevice.getBinning());
    mHeight = pDcamDevice.adjustWidthHeight(pHeight, 4 / pDcamDevice.getBinning());
    mDepth = pDepth;
   ...
   *
   * @param pTimeStampInNs image sequence time stamp.
   */
   ...
   *
   * @param pIndex plane index
   * @return BridK pointer
   */
  public Pointer<Byte> getPointerForPlane(final int pIndex)
  {
    ContiguousMemoryInterface lMemoryForPlane = getMemoryForPlane(pIndex);
  ...
  public DcamImageSequence getSinglePlaneImageSequence(final int pIndex)
  {
    ...
    DcamImageSequence lDcamImageSequence = new DcamImageSequence(mDcamDevice, FragmentedMemory.wrap(lMemory), getBytesPerPixel(), getWidth(), getDepth(), 1L);
   ...
   *
   * @param pKeepPlaneList     list of boolean flags indicating which planes to keep
   * @param pDestinationMemory destination memory
   */
  public void consolidateTo(ArrayList<Boolean> pKeepPlaneList, final ContiguousMemoryInterface pDestinationMemory)
  {
    if (pKeepPlaneList == null || pKeepPlaneList.isEmpty() || allPlanesKept(pKeepPlaneList))
    ...
    else
    {
      ...
      int lNumberOfFragments = mFragmentedMemory.getNumberOfFragments();

      for (int i = 0; i < lNumberOfFragments; i++)
        if (pKeepPlaneList.get(i))
        {
          ContiguousMemoryInterface lContiguousMemoryInterface = mFragmentedMemory.get(i);
  ...
  {
    return String.format("DcamImageSequence [mBytesPerPixel=%d, mWidth=%d, mHeight=%d, mDepth=%d, mTimeStampInNs=%d]", mBytesPerPixel, mWidth, mHeight, mDepth, mTimeStampInNs);
  }
  ...
  public void release()
  {
    if (mRecycler != null) mRecycler.release(this);
  }
