...
public class ECC100Axis
{
  ...
  private boolean waitToArriveAt(double pTargetPositionInMicrons, double pEpsilonInMicrons, long pTimeOut, TimeUnit pTimeUnit)
  {
    ...
    while (!isReady() && !hasArrived())
    {
      ...
      if (System.nanoTime() > lDeadLine) return false;
    }
    return true;
  }
  ...
  public void continuous(boolean pEnable, boolean pForward)
  {
    ...
    if (pForward) EccLibrary.ECC_controlContinousFwd(mPointerToDeviceHandle.getInt(), mAxisIndex, lPointerEnable, 1);
    else EccLibrary.ECC_controlContinousBkwd(mPointerToDeviceHandle.getInt(), mAxisIndex, lPointerEnable, 1);
  ...
  {
    if (!isReferencePositionValid()) return 0;
