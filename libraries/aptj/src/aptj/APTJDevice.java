...
public class APTJDevice
{
   ...
   */
  public APTJDevice(APTJDeviceFactory pAPTJDeviceFactory, APTJDeviceType pAPTJDeviceType, long pDeviceSerialNum) throws APTJExeption
  {
...

    {
      ...
      APTJDeviceFactory.checkError(APTLibrary.GetHWInfo(getSerialNumber(), lPointerModelString, 1024, lPointerSWVerString, 1024, lPointerNotesString, 1024));
...

    {
      ...
      APTJDeviceFactory.checkError(APTLibrary.MOT_GetStageAxisInfo(getSerialNumber(), lMinPosPointer, lMaxPosPointer, lUnitPointer, lPitchPointer));/**/

      mMinPosition = lMinPosPointer.get();
      mMaxPosition = lMaxPosPointer.get();
      mPitch = lPitchPointer.get();
      mUnits = (((lUnitPointer.get().intValue()) == 1) ? APTJUnits.mm : APTJUnits.deg);
...

    {
      ...
      APTJDeviceFactory.checkError(APTLibrary.MOT_GetVelParamLimits(getSerialNumber(), lMaxAccnLimitPointer, lMaxVelLimitPointer));/**/
  ...
  {
    stopIfNeeded();
    APTJDeviceFactory.checkError(APTLibrary.MOT_MoveHome(getSerialNumber(), 0));
  }
  ...
  {
    APTJDeviceFactory.checkError(APTLibrary.MOT_SetVelParams(getSerialNumber(), 0, (float) getAcceleration(), (float) abs(pVelocity)));

    int lDirection = signum(pVelocity) > 0 ? 1 : 2;

    APTJDeviceFactory.checkError(APTLibrary.MOT_MoveVelocity(getSerialNumber(), lDirection));/**/
  }
  ...
  {
    APTJDeviceFactory.checkError(APTLibrary.MOT_SetVelParams(getSerialNumber(), 0, (float) getAcceleration(), (float) pSpeed));
  }
  ...
  public double getSpeed() throws APTJExeption
  {
    ...
    APTJDeviceFactory.checkError(APTLibrary.MOT_GetVelParams(getSerialNumber(), lMinVelPointer, lAccnPointer, lMaxVelPointer));/**/
  ...
  {
    APTJDeviceFactory.checkError(APTLibrary.MOT_SetVelParams(getSerialNumber(), 0, (float) pAcceleration, (float) getSpeed()));
  }
  ...
  {
    stopIfNeeded();
    APTJDeviceFactory.checkError(APTLibrary.MOT_MoveAbsoluteEx(getSerialNumber(), (float) pNewPosition, 0));
  }
  ...
  {
    stopIfNeeded();
    APTJDeviceFactory.checkError(APTLibrary.MOT_MoveRelativeEx(getSerialNumber(), (float) pDeltaPosition, 0));
  }
  ...
  public double getCurrentPosition() throws APTJExeption
  {
    ...
    APTJDeviceFactory.checkError(APTLibrary.MOT_GetPosition(getSerialNumber(), lPositionPointer));
   ...
   * @throws APTJExeption exception
   */
  public boolean waitWhileMoving(long pPollPeriod, long pTimeOut, TimeUnit pTimeUnit) throws APTJExeption
  {
    long lPeriodInMilliseconds = TimeUnit.MILLISECONDS.convert(pPollPeriod, pTimeUnit);
  ...
  public boolean isLimitAttained() throws APTJExeption
  {
    ...
    boolean lIsLimitAttained = lCheckBit1 || lCheckBit2 || lCheckBit3 || lCheckBit4;
  ...
  public boolean isMoving() throws APTJExeption
  {
    ...
    boolean lIsMoving = lCheckBit1 || lCheckBit2 || lCheckBit3 || lCheckBit4 || lCheckBit5;
  ...
  private long getStatusBits() throws APTJExeption
  {
    ...
    APTJDeviceFactory.checkError(APTLibrary.MOT_GetStatusBits(getSerialNumber(), lStatusBitsPointer));
  ...
  {
    return new StringBuilder(Long.toBinaryString(pLongValue)).reverse().toString();
  }
  ...
  {
    return String.format("APTJDevice [mAPTJDeviceType=%s, mDeviceSerialNum=%s, mModelString=%s, mSWVerString=%s, mNotesString=%s, mLowPosition=%s, mHighPosition=%s, mPitch=%s, mUnits=%s, mMaxAccel=%s, mMaxVelocity=%s]", mAPTJDeviceType, mDeviceSerialNum, mModelString, mSWVerString, mNotesString, mMinPosition, mMaxPosition, mPitch, mUnits, mMaxAccel, mMaxVelocity);
  }
