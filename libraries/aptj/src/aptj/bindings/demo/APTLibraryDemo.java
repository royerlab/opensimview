...
public class APTLibraryDemo
{
  ...
  public void test() throws InterruptedException
  {
    ...
    APTLibrary.GetNumHWUnitsEx(APTLibrary.HWTYPE_TST001, lPointerNumDevices);
    ...
    for (int i = 0; i < lNumberOfDevices; i++)
    {
      ...
      APTLibrary.GetHWSerialNumEx(APTLibrary.HWTYPE_TST001, i, lPointerSerialNum);
      ...
      APTLibrary.GetHWInfo(lSerialNumber, lPointerModelString, 1024, lPointerSWVerString, 1024, lPointerNotesString, 1024);
      ...
      checkError(APTLibrary.MOT_GetStageAxisInfo(lSerialNumber, lMinPosPointer, lMaxPosPointer, lUnitPointer, lPitchPointer));/**/

      System.out.println("Minimum pos: " + lMinPosPointer.get());
      System.out.println("Maximum pos: " + lMaxPosPointer.get());
      System.out.println("Units      : " + (((lUnitPointer.get().intValue()) == 1) ? "mm" : "deg"));
      ...
      checkError(APTLibrary.MOT_GetVelParams(lSerialNumber, lMinVelPointer, lAccnPointer, lMaxVelPointer));/**/
      ...
      checkError(APTLibrary.MOT_GetVelParamLimits(lSerialNumber, lMaxAccnLimitPointer, lMaxVelLimitPointer));/**/
    ...
    for (int i = 0; i < lNumberOfDevices; i++)
    {
      ...
      checkError(APTLibrary.MOT_MoveVelocity(lSerialNumber, 1 + (((i % 2) == 1) ? 1 : -1)));/**/
    }
    Thread.sleep(3 * 1000);
    ...
    for (int i = 0; i < lNumberOfDevices; i++)
    {
      ...
      checkError(APTLibrary.MOT_SetVelParams(lSerialNumber, 0, 1.5f, 1.5f));
    }/**/
    Thread.sleep(3 * 1000);
    ...
    for (int i = 0; i < lNumberOfDevices; i++)
    {
      ...
      checkError(APTLibrary.MOT_MoveVelocity(lSerialNumber, 1 + (((i % 2) == 0) ? 1 : -1)));/**/
    }
    Thread.sleep(2 * 1000);
    ...
    for (int t = 0; t < 40; t++)
    {
      for (int i = 0; i < lNumberOfDevices; i++)
      {
        ...
        long lSerialNumber = lIndexToSerialBidiMap.get(i);
        checkError(APTLibrary.MOT_GetPosition(lSerialNumber, lPositionPointer));
        System.out.print("position: " + lPositionPointer.get());

        checkError(APTLibrary.MOT_GetStatusBits(lSerialNumber, lStatusBitsPointer));
        System.out.println("position: " + toBinaryString(lStatusBitsPointer.get().longValue()));
      }
      Thread.sleep(500);
    }
  ...
  {
    if (pReturnCode != 0) System.out.println("Return code=" + pReturnCode);
  }
}
