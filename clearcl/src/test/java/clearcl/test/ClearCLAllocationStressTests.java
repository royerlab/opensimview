...
public class ClearCLAllocationStressTests
{
   ...
   * @throws Exception NA
   */
  @Test public void testRGC() throws Exception
  {
    final ClearCLBackendInterface
        lClearCLBackendInterface =
        ClearCLBackends.getBestBackend();

    testWithBackend(lClearCLBackendInterface);
  }

  private void testWithBackend(final ClearCLBackendInterface pClearCLBackendInterface) throws
                                                                                       Exception
  {
  ...
  private void allocateImages(ClearCLDevice pDevice, ClearCLContext pMainContext, int pI)
  {
    for (int i = 0; i < 10; i++)
    {
      ClearCLImage
          lImage =
          pMainContext.createSingleChannelImage(ImageChannelDataType.Float, 1000, 1000);
      if (i % 10 == 0)
        lImage.close();
    }
  }
  ...
  private void allocateBuffers(ClearCLDevice pDevice, ClearCLContext pMainContext, int pI)
  {
    for (int i = 0; i < 10; i++)
    {
      ClearCLBuffer
          lBuffer =
          pMainContext.createBuffer(NativeTypeEnum.Float, 1000 * 1000);
      if (i % 10 == 0)
        lBuffer.close();
    }
  }
