...
public class Utils
{
    ...
    boolean needsAdditionalSynchronization = false;
    {
      ...
      try
      {
        ...
        if (!OpenCLProbeLibrary.isValid())
        {
          ...
          {
            log(Level.INFO,
                "Hacking around ATI's weird driver bugs (using atiocl/amdocl library instead of OpenCL)",
                null);
        ...
        if (OpenCLProbeLibrary.hasOpenCL1_0())
        {
          ...
          log(Level.WARNING,
              "At least one OpenCL platform uses OpenCL 1.0, which is not thread-safe: will use (slower) synchronized low-level bindings.");
        }
      }
      finally
      {
   ...
   * @return The value
   */
  public static int getInt(OpenCLLibrary pOpenCLLibrary,
   ...
   * @return The value
   */
  public static Integer[] getInts(OpenCLLibrary pOpenCLLibrary,
                                  OpenCLLibrary.cl_device_id device,
                                  int paramName,
                                  int numValues)
  {
   ...
   * @return The value
   */
  public static long getLong(OpenCLLibrary pOpenCLLibrary,
                             OpenCLLibrary.cl_device_id device,
                             int paramName)
  {
   ...
   * @return The value
   */
  public static Long[] getLongs(OpenCLLibrary pOpenCLLibrary,
                                OpenCLLibrary.cl_device_id device,
                                int paramName,
                                int numValues)
  {
   ...
   * @return The value
   */
  public static String getString(OpenCLLibrary pOpenCLLibrary,
                                 OpenCLLibrary.cl_device_id pDevice,
                                 int pParamName)
  {
    // Obtain the length of the string that will be queried
    Pointer<SizeT> lSize = Pointer.allocateSizeT();
    BackendUtils.checkOpenCLError(pOpenCLLibrary.clGetDeviceInfo(pDevice,
                                                                 pParamName,
                                                                 0,
                                                                 null,
                                                                 lSize));
   ...
   */
  public static String getString(OpenCLLibrary pOpenCLLibrary,
                                 OpenCLLibrary.cl_platform_id platform,
                                 int paramName)
  {
    // Obtain the length of the string that will be queried
    Pointer<SizeT> lSizeT = Pointer.allocateSizeT();
    BackendUtils.checkOpenCLError(pOpenCLLibrary.clGetPlatformInfo(platform,
                                                                   paramName,
                                                                   0,
                                                                   null,
                                                                   lSizeT));
   ...
   * @return The value
   */
  public static long getSize(OpenCLLibrary pOpenCLLibrary,
                             OpenCLLibrary.cl_device_id device,
                             int paramName)
  {
  ...
  public static long[] getSizes(OpenCLLibrary pOpenCLLibrary,
                                OpenCLLibrary.cl_device_id device,
                                int paramName,
                                int numValues)
  {
  ...
  public static Pointer<OpenCLLibrary.cl_device_id> convertDevicePointers(ClearCLPeerPointer... pDevicePointers)
  {
    ...
    for (int i = 0; i < pDevicePointers.length; i++)
      lDevicesArrayPointer.set(i,
                               (OpenCLLibrary.cl_device_id) pDevicePointers[i].getPointer());
