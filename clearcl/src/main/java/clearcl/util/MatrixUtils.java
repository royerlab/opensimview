...
public class MatrixUtils
{
   ...
   * @param pContext
   *          clearcl context
   * @param pClearCLBuffer
   *          buffer
   * @param pMatrix
   *          matrix
   * @return buffer
   */
  public static ClearCLBuffer matrixToBuffer(ClearCLContext pContext,
                                             ClearCLBuffer pClearCLBuffer,
                                             Matrix4f pMatrix)
  {
    if (pClearCLBuffer == null
        || pClearCLBuffer.getSizeInBytes() != 16)
    {
      if (pClearCLBuffer != null)
        pClearCLBuffer.close();

      pClearCLBuffer =
                     pContext.createBuffer(MemAllocMode.Best,
