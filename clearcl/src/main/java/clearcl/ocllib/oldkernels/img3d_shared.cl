...
inline int localIndex(int i, int j, int k,
					  const int locX,const int locY,const int locZ, const int FSIZE){
									...
									__local short *pixLoc)
{
  ...
  int val0 = pixLoc[localIndex(iGroup,jGroup,kGroup,
									  localSizeX,localSizeY,localSizeZ,FSIZE)];

  for(int i2 = -FSIZE;i2<=FSIZE;i2++){
    for(int j2 = -FSIZE;j2<=FSIZE;j2++){
  	  for(int k2 = -FSIZE;k2<=FSIZE;k2++){

  		int val1 = pixLoc[localIndex(iGroup+i2,jGroup+j2,kGroup+k2,
									  localSizeX,localSizeY,localSizeZ,FSIZE)];
  ...
  //output[i+j*Nx+k*Nx*Ny] = (short)(pixLoc[indexGroup]);
